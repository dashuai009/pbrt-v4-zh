...
#parec[
  ...
  相机的投影或镜头系统在胶片上形成场景的像之后，还需要模拟胶片测量光的过程，才能得到渲染器的最终图像。本节先概述胶片测光的辐射度学原理，再讨论如何将光谱能量转换为三刺激颜色（通常为 RGB）。由此引出 #link(<PixelSensor>)[`PixelSensor`] 类，它既模拟这一转换，也模拟相机通常会执行的后续处理。接着，我们讨论如何将胶片上的图像样本累积为最终像素值，然后介绍 #link(<Film>)[`Film`] 接口及其两个实现。
]
...
#parec[
  ...
  既然要模拟真实的成像过程，就值得更精确地定义胶片或相机传感器测量中的辐射度学关系。从镜头后部传播到胶片的光线携带着场景的辐亮度。从胶片平面上的一点看去，辐亮度从一组方向入射。该点所见的散焦模糊程度会影响离开镜头的辐亮度分布。@fig:exit-pupil-images 展示了从胶片上两个位置看去的镜头辐亮度图像。
]
...
#parec[
  ...
  给定入射辐亮度函数，就能定义胶片平面上一点的辐照度。从@eqt:irradiance-from-radiance 中用辐亮度表示辐照度的定义出发，利用@eqt:dw-dA-eqt 将立体角积分转换为面积积分。这里的积分区域 $A_e$ 位于最后一片透镜的切平面上。由此得到胶片平面上一点 $p$ 的辐照度：
]
...
#figure(
  ...
  caption: [
    #ez_caption[Geometric setting for the irradiance measurement
      ...
      辐照度测量方程（@eqt:measurement-irradiance）的几何配置。光线通过最后一片透镜切平面上的点 $p'$，到达胶片平面上的点 $p$，可以沿这一传播方向测量辐亮度。$z$ 是胶片平面与最后一片透镜切平面之间的轴向距离，$theta$ 是从 $p'$ 指向 $p$ 的向量与光轴的夹角。
    ]
  ],
) <measurement-setting>
...
#parec[
  ...
  在快门开启的时间内，对胶片上一点的辐照度积分，得到#emph[辐射曝光量]，即单位面积接收的能量，单位为 $upright("J") / upright("m")^2$。
]
...
#parec[
  ...
  （辐射曝光量也称为#emph[fluence]。）在一点测量辐射曝光量，就能体现胶片平面接收的能量在一定程度上取决于快门开启时长这一关系。
]
...
#parec[
  ...
  传统胶片基于一种化学过程：卤化银晶体受光后产生溴化银。卤化银主要对蓝光敏感；要记录彩色图像，可以设置多层晶体，在层间加入彩色滤光片，并使用染料增强卤化银对其他波长的响应。
]
...
#parec[
  ...
  数字传感器的像素通常以#emph[马赛克]形式排列。由于人类视觉系统对绿色更敏感，绿色像素的数量分别是红色像素和蓝色像素的两倍。这种排列需要用#emph[去马赛克]算法重建图像像素，使每个位置都具有红、绿、蓝三个分量。直接取相邻的四个马赛克像素，将其颜色值原样组合，效果并不好，因为这些传感器像素对应的位置略有不同。
]
...
#parec[
  ...
  `pbrt`包括一个现实的相机模型以及基于投影矩阵的理想化模型。 由于针孔相机的光圈面积无穷小，我们在`PixelSensor`的实现中做出了一些实际的权衡，以便使用针孔模型渲染的图像不会完全是黑色的。 我们将光圈大小的效果建模的责任留给`Camera`。理想化模型完全不考虑这一点，而`RealisticCamera`在`<<Compute weighting for RealisticCamera ray>>`片段中考虑了这一点。 然后`PixelSensor`仅考虑快门时间和ISO设置。这两个因素被收集到一个称为“成像比例因子”的单一数值中。
]
...
#heading(level: 4, numbering: none)[#ez_caption[Chromatic Adaptation and White Balance][色适应与白平衡]]
...
#parec[
  ...
  对 LMS 空间中的颜色进行白平衡时，先逐分量除以场景照明体的颜色，再乘以目标照明体的颜色。这一步可用对角矩阵表示，结合空间变换即可得到作用于 XYZ 颜色的完整白平衡矩阵。
]
...
#heading(level: 4, numbering: none)[#ez_caption[Sampling Sensor Response][传感器响应采样]]
...
#parec[
  ...
  `PixelSensor` 的传感器响应函数描述了对辐亮度的响应如何随波长变化。因此，在采样射线所携带的波长时，值得至少近似地考虑这种变化。最起码，不应选取所有响应函数都为零的波长，因为它对最终图像没有贡献。更一般地，依据传感器响应函数进行重要性采样，有望减小@eqt:mc-sensor-response 的估计误差。
]
...
#parec[
  Like `SampledWavelengths::SampleUniform()`, `SampleVisible()` uses a single random sample to generate all wavelength samples. It uses a slightly different approach, taking uniform steps across the $lr([0, 1))$ sample space before sampling each wavelength.
][
  与 `SampledWavelengths::SampleUniform()` 类似，`SampleVisible()` 用一个随机样本生成全部波长样本。这里的方法略有不同：先在 $lr([0, 1))$ 样本空间中按等间距步进，再由各位置采样相应波长。
]
...
#figure(
  ...
  caption: [
    #ez_caption[
      ...
      (b)~以相同的采样率渲染，但使用 `SampledWavelengths::SampleVisible()` 进行波长采样。图像中的色彩噪声明显减少，额外的计算开销几乎可以忽略不计。
      (模型由Yasutoshi Mori提供。)
    ]
  ],
  kind: image,
...
#parec[
  ...
  滤波重要性采样还有其他优点。它使并行渲染更容易：若不同线程负责不同像素，就不会需要同时修改同一个像素值。此外，对被积函数采样不充分时，方差尖峰可能使某些样本远亮于其余样本；这些样本只影响一个像素，不会扩散到多个像素。修复这样的单像素伪影，比修复一片受影响的邻域更容易。
]
...
#parec[
  ...
  `Film` 实现通过 `UsesVisibleSurface()` 告知调用方，它是否使用传给 `AddSample()` 的 `VisibleSurface *`。这样，积分器在用不到这些信息时，就可以省去初始化 `VisibleSurface` 的开销。
]
...
#parec[
  ...
  有了 #link("https://pbr-book.org/4ed/Sampling_and_Reconstruction/Image_Reconstruction.html#Filter")[`Filter`]，#link(<Film::SampleBounds>)[`Film::SampleBounds()`] 也容易实现：计算样本范围时，既要按滤波器半径向外扩展，也要计入 `pbrt` 像素坐标约定所要求的半像素偏移。第 8.1.4 节将详细说明这些约定。
]
...
#parec[
  ...
  构造函数根据（可能经过裁剪的）图像分辨率，分配一个 `Pixel` 结构的二维数组，每个像素对应一个结构。`rgbSum` 以 RGB 颜色表示不断累积的像素贡献加权和；`weightSum` 保存这些样本贡献的滤波权重之和。二者分别对应@eqt:pixel-contribution-weighted-sum 的分子和分母。最后，`rgbSplat` 保存 splat 贡献的（未加权）和。
]
...
#parec[
  ...
  `RGBFilm` 的 `maxComponentValue` 参数指定限幅阈值。它默认为无穷大，此时不进行限幅。
]
