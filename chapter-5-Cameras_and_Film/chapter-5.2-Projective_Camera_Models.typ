...
#parec[
  ...
  正交投影和透视投影都需要指定两个与观察方向垂直的平面：#emph[近]平面和#emph[远]平面。当使用光栅化进行渲染时，不在这两个平面之间的对象会被剔除，不包含在最终图像中。（剔除近平面前的对象尤为重要，以避免深度为0的奇点，因为否则投影矩阵会将相机后面的点映射为在其前面。）在光线追踪器中，投影矩阵仅用于确定离开相机的光线，这些问题不适用；因此在这种情况下，不需要过多担心这些平面的深度设置。
]
...
#block(sticky: true)[#raw("<<ProjectiveCamera Protected Members>>=")] <fragment-ProjectiveCameraProtectedMembers-0>
```cpp
...
```
#block(sticky: true)[#raw("<<ProjectiveCamera Protected Members>>+=")] <fragment-ProjectiveCameraProtectedMembers-1>
```cpp
...
#block(sticky: true)[#raw("<<Transform Function Definitions>>+=")] <fragment-TransformFunctionDefinitions-4>
```cpp
...
#parec[
  ...
  由于正交投影的简单性，可以在 `GenerateRayDifferential()` 方法中直接计算 $x$ 和 $y$ 方向的微分射线。微分射线的方向将与主射线相同（正交相机生成的所有射线皆如此），并且所有射线的原点差异将相同。因此，这里的构造函数预先计算了由于在胶片平面上 $x$ 和 $y$ 方向上的单个像素偏移而导致的相机空间坐标中射线原点的移动量。
]
...
#block(sticky: true)[#raw("<<OrthographicCamera Method Definitions>>=")] <fragment-OrthographicCameraMethodDefinitions-0>
```cpp
...
#block(sticky: true)[#raw("<<OrthographicCamera Method Definitions>>+=")] <fragment-OrthographicCameraMethodDefinitions-1>
```cpp
...
#parec[
  ...
  透视投影与正交投影相似，因为二者都把三维空间中的一个体积区域投影到二维胶片平面上。然而，它包括近大远小的透视缩短效应：远处的物体被投影得比近处同样大小的物体小。与正交投影不同，透视投影不保留真实距离或角度，平行线在投影中不再保持平行。透视投影与眼睛或相机镜头生成三维世界图像的方式非常接近。
]
...
#block(sticky: true)[#raw("<<Transform Function Definitions>>+=")] <fragment-TransformFunctionDefinitions-5>
```cpp
...
#parec[
  ...
  2. 用户指定的视场角(`fov`)通过缩放投影平面上的$(x, y)$值来考虑，以便视野内的点投影到观察平面上的$[-1, 1]$坐标之间。对于方形图像，$x$和$y$都在屏幕空间的$[-1, 1]$之间。否则，图像较窄的方向映射到$[-1, 1]$，较宽的方向映射到屏幕空间值的比例较大的范围。回想一下，正切等于直角三角形对边与邻边的比率。这里邻边的长度为1，因此对边的长度为$tan ( "fov" \/ 2 )$。通过该长度的倒数进行缩放，将视野映射到范围$[-1, 1]$。
]
...
#block(sticky: true)[#raw("<<PerspectiveCamera Private Members>>=")] <fragment-PerspectiveCameraPrivateMembers-0>
```cpp
Vector3f dxCamera, dyCamera;
```
...
#block(sticky: true)[#raw("<<PerspectiveCamera Private Members>>+=")] <fragment-PerspectiveCameraPrivateMembers-1>
```cpp
Float cosTotalWidth;
```
...
#parec[
  ...
  @fig:dof-images 显示了这种效果，在 #emph[水彩] 场景中。 随着透镜光圈的增大，离对焦平面越远的点模糊度越大。 注意，中心的铅笔杯在所有图像中始终保持对焦，因为对焦平面已放置在其深度处。 @fig:ecosys-dof 显示了用于渲染景观场景的景深。 注意这种效果如何将观众的目光吸引到图像中心对焦的草地上。
]
...
#parec[
  ...
  注意，弥散圆的直径与透镜的直径成比例。透镜直径常用_光圈数_ $n$ 表示，它给出直径相对于焦距的比例：$d_l = f / n$。
]
...
#parec[
  The `SampleUniformDiskConcentric()` function, which is defined in #link("https://pbr-book.org/4ed/Sampling_Algorithms/Sampling_Multidimensional_Functions.html#sec:unit-disk-sample")[Section A.5.1], takes a $(u , v)$ sample position in $[0, 1)^2$ and maps it to a 2D unit disk centered at the origin $(0 , 0)$. To turn this into a point on the lens, these coordinates are scaled by the lens radius. The `CameraSample` class provides the $(u , v)$ lens-sampling parameters in the `pLens` member variable.
][
  `SampleUniformDiskConcentric()`函数在 #link("https://pbr-book.org/4ed/Sampling_Algorithms/Sampling_Multidimensional_Functions.html#sec:unit-disk-sample")[A.5.1 节]中定义，接受 $[0, 1)^2$ 中的 $(u , v)$ 样本位置并将其映射到以原点 $(0 , 0)$ 为中心的二维单位圆盘。为了将其转换为透镜上的一个点，这些坐标按透镜半径缩放。`CameraSample`类在`pLens`成员变量中提供 $(u , v)$ 透镜采样参数。
]
