...
== #ez_caption[Projective Camera Models][投影相机模型]
...
viewing problem:] how to project a 3D scene onto a 2D image for display. Most of the classic approaches can be expressed by a $4 times 4$ projective transformation matrix. Therefore, we will introduce a projection matrix camera class, `ProjectiveCamera`, and then define two camera models based on it. The first implements an orthographic projection, and the other implements a perspective projection—two classic and widely used projections.
  ...
  三维计算机图形学的基本问题之一是#emph[三维观察问题]：如何把三维场景投影到二维图像上显示。大多数经典方法可以用一个 $4 times 4$ 的投影变换矩阵表示。因此，我们将介绍一个投影矩阵相机类，`ProjectiveCamera`，然后基于它定义两种相机模型。我们先实现正交投影，然后实现透视投影——两种经典且广泛使用的投影。
]

#block(sticky: true)[#raw("<<ProjectiveCamera Definition>>=")] <fragment-ProjectiveCameraDefinition-0>
...
#parec[
  ...
  正交投影和透视投影都需要指定两个与观察方向垂直的平面：#emph[近]平面和#emph[远]平面。使用光栅化渲染时，不在这两个平面之间的对象会被剔除，不出现在最终图像中。（剔除近平面前方的对象尤为重要：这既能避开深度为 0 处的奇点，也能防止相机后方的点经投影矩阵变换后看起来位于相机前方。）在光线追踪器中，投影矩阵仅用于确定从相机发出的射线，并不存在上述问题，因此不必像光栅化那样仔细设置这两个平面的深度。
]
...
#parec[
  ...
  - #emph[归一化设备坐标（NDC）空间：] 这是实际渲染图像的坐标系统。在$(x , y)$中，这个空间范围从$(0 , 0)$到$(1 , 1)$，其中$(0 , 0)$是图像的左上角。深度值与屏幕空间相同，线性变换将屏幕空间转换为NDC空间。
]
...
#parec[
  ...
  - #emph[光栅空间：] 它与 NDC 空间几乎相同，区别是 $x$、$y$ 坐标从 $(0 , 0)$ 延伸到图像在这两个方向上的像素分辨率。
]
...
#parec[
  ...
  除了`CameraBase`类所需的参数外，#link(<ProjectiveCamera>)[`ProjectiveCamera`]还需要投影变换矩阵、图像的屏幕空间范围，以及与相机对焦距离和镜头光圈大小有关的附加参数。如果镜头光圈不是无穷小的针孔，那么图像的某些部分可能会模糊，就像真实镜头系统中失焦的对象一样。对此效果的模拟将在本节后面讨论。
]

#block(sticky: true)[#raw("<<ProjectiveCamera Public Methods>>=")] <fragment-ProjectiveCameraPublicMethods-0>
#block(breakable: false)[
```cpp
...
#parec[
  ...
  各种 #link(<ProjectiveCamera>)[`ProjectiveCamera`] 实现把投影变换传给这里所示的基类构造函数。该变换将相机空间映射到屏幕空间；有了它，构造函数就能方便地计算其余变换，一直连接到光栅空间与相机空间之间。
]

#block(sticky: true)[#raw("<<Compute projective camera transformations>>=")] <fragment-Computeprojectivecameratransformations-0>
#block(breakable: false)[
```cpp
...
#block(sticky: true)[#raw("<<ProjectiveCamera Protected Members>>=") #link(<fragment-ProjectiveCameraProtectedMembers-1>)[↓]] <fragment-ProjectiveCameraProtectedMembers-0>
#block(breakable: false)[
```cpp
...
#parec[
  ...
  构造函数中唯一不那么直接的变换是从屏幕到光栅的投影。先计算屏幕到 NDC、NDC 到光栅的变换，再将二者复合。这里一个重要的细节是，最终变换会反转 $y$ 坐标；这是必要的，因为在屏幕坐标中增加 $y$ 值向上移动图像，而在光栅坐标中向下移动。
]

#block(sticky: true)[#raw("<<Compute projective camera screen transformations>>=")] <fragment-Computeprojectivecamerascreentransformations-0>
#block(breakable: false)[
```cpp
...
#parec[Editorial note: for a nondegenerate screen window with increasing bounds, the intermediate `NDCFromScreen` transform above maps screen $y$ to $[-1,0]$, although the preceding prose defines NDC $y$ in $[0,1]$. The negative scale in `rasterFromNDC` then maps this to the correct raster range from 0 to the image height. The fixed source code is preserved; its intermediate coordinate convention differs from the prose definition.][校订说明：对于上下界递增的非退化屏幕窗口，上述中间变换 `NDCFromScreen` 将屏幕 $y$ 映射到 $[-1,0]$，而前文对 NDC 的定义是 $[0,1]$。随后 `rasterFromNDC` 中的负缩放将其变为从 0 到图像高度的正确光栅范围。此处保留固定原代码；其中间坐标约定与正文定义不同。]
#block(sticky: true)[#raw("<<ProjectiveCamera Protected Members>>+=") #link(<fragment-ProjectiveCameraProtectedMembers-0>)[↑] #link(<fragment-ProjectiveCameraProtectedMembers-2>)[↓]] <fragment-ProjectiveCameraProtectedMembers-1>
#block(breakable: false)[
```cpp
...
=== #ez_caption[Orthographic Camera][正交相机]
<orthographic-camera>
#parec[
  ...
  正交相机基于正交投影变换。正交变换取场景中的一个长方体区域，将其投影到包围该区域的盒子前表面。它不会产生#emph[透视缩短]的效果——即物体在图像平面上随着距离的增加而变小——但它保持平行线之间的平行关系，并保留物体之间的相对距离。@fig:ortho-volume 显示了这个长方体视域如何定义场景的可见区域。
]


#block(sticky: true)[#raw("<<OrthographicCamera Definition>>=")] <fragment-OrthographicCameraDefinition-0>
#block(breakable: false)[
```cpp
...
#figure(
  ...
  caption: [
    ...
    ][ 正交视域是相机空间中的一个轴对齐盒子，定义为使区域内的物体投影到盒子的 $z = "near"$ 面上。 ]
  ],
)<ortho-volume>
#figure(
  table(
    ...
    [#ez_caption[(a) orthographic][(a) 正交投影]], [#ez_caption[(b) perspective][(b) 透视投影]],
  ),
  caption: [
    #ez_caption[
      ...
      使用不同相机模型渲染的 Kroken 场景。图像从相同的视点渲染，使用 (a) 正交相机和 (b) 透视相机。缺少近大远小的透视缩短效应，使正交视图的深度感较弱，尽管它保持了平行线，这可能是一个有用的特性。#emph[(场景由Angelo Ferretti 提供。)]
    ]
  ],
  kind: image,
...
#block(sticky: true)[#raw("<<OrthographicCamera Public Methods>>=")] <fragment-OrthographicCameraPublicMethods-0>
#block(breakable: false)[
```cpp
...
#block(sticky: true)[#raw("<<Transform Function Definitions>>+=") #link("https://pbr-book.org/4ed/Geometry_and_Transformations/Transformations.html#fragment-TransformFunctionDefinitions-3")[↑] #link(<fragment-TransformFunctionDefinitions-5>)[↓]] <fragment-TransformFunctionDefinitions-4>
#block(breakable: false)[
```cpp
...
#parec[
  ...
  正交投影很简单，因此可以在 `GenerateRayDifferential()` 方法中直接计算 $x$ 和 $y$ 方向的微分射线。微分射线与主射线方向相同（正交相机生成的所有射线皆如此），相应的原点偏移量也不随主射线变化。因此，这里的构造函数预先计算：胶片平面上的位置沿 $x$ 或 $y$ 方向移动一个像素时，射线原点在相机空间中移动多少。
]

#block(sticky: true)[#raw("<<Compute differential changes in origin for orthographic camera rays>>=")] <fragment-Computedifferentialchangesinoriginfororthographiccamerarays-0>
#block(breakable: false)[
```cpp
...
#block(sticky: true)[#raw("<<OrthographicCamera Private Members>>=")] <fragment-OrthographicCameraPrivateMembers-0>
#block(breakable: false)[
```cpp
...
#parec[
  ...
  现在我们可以通过代码，将光栅空间中的采样点转换为相机射线。该过程在@fig:ortho-raster-to-ray 总结。首先，光栅空间采样位置被转换为相机空间中的一个点，给出位于近平面上的一个点，这就是相机射线的原点。相机空间的观察方向沿 $z$ 轴正方向，因此射线方向为 $(0 , 0 , 1)$。
]
#figure(
...
#block(sticky: true)[#raw("<<OrthographicCamera Method Definitions>>=") #link(<fragment-OrthographicCameraMethodDefinitions-1>)[↓]] <fragment-OrthographicCameraMethodDefinitions-0>
#block(breakable: false)[
```cpp
...
#block(sticky: true)[#raw("<<Compute raster and camera sample positions>>=")] <fragment-Computerasterandcamerasamplepositions-0>
#block(breakable: false)[
```cpp
...
#block(sticky: true)[#raw("<<OrthographicCamera Method Definitions>>+=") #link(<fragment-OrthographicCameraMethodDefinitions-0>)[↑]] <fragment-OrthographicCameraMethodDefinitions-1>
#block(breakable: false)[
```cpp
...
#block(sticky: true)[#raw("<<Compute ray differentials for OrthographicCamera>>=")] <fragment-ComputeraydifferentialsformonoOrthographicCamera-0>
#block(breakable: false)[
```cpp
if (lensRadius > 0) {
...
=== #ez_caption[Perspective Camera][透视相机]
<perspective-camera>
#parec[
  ...
  透视投影与正交投影相似，因为二者都把三维空间中的一个体积区域投影到二维胶片平面上。然而，它包括近大远小的透视缩短效应：远处的物体被投影得比近处同样大小的物体小。与正交投影不同，透视投影不保留真实距离或角度，平行线在投影中不再保持平行。透视投影与眼睛或相机镜头形成三维世界图像的方式相当接近。
]

#block(sticky: true)[#raw("<<PerspectiveCamera Definition>>=")] <fragment-PerspectiveCameraDefinition-0>
#block(breakable: false)[
```cpp
...
#block(sticky: true)[#raw("<<PerspectiveCamera Public Methods>>=")] <fragment-PerspectiveCameraPublicMethods-0>
#block(breakable: false)[
```cpp
...
#parec[
  ...
  透视投影描述了场景的透视视图。场景中的点被投影到垂直于 $z$ 轴的观察平面上。#link(<Perspective>)[`Perspective()`]函数计算此变换；它需要`fov`中的视场角以及到近 $z$ 平面和远 $z$ 平面的距离（@fig:perspective-projection）。
]
...
#block(sticky: true)[#raw("<<Transform Function Definitions>>+=") #link(<fragment-TransformFunctionDefinitions-4>)[↑]] <fragment-TransformFunctionDefinitions-5>
#block(breakable: false)[
```cpp
...
#block(sticky: true)[#raw("<<Perform projective divide for perspective projection>>=")] <fragment-Performprojectivedivideforperspectiveprojection-0>
#block(breakable: false)[
```cpp
...
#parec[
  ...
  2. 用户指定的视场角(`fov`)通过缩放投影平面上的$(x, y)$值来考虑，以便视野内的点投影到观察平面上的$[-1, 1]$坐标之间。对于方形图像，$x$和$y$都在屏幕空间的$[-1, 1]$之间。否则，图像较窄的方向映射到$[-1, 1]$，较宽的方向则映射到按宽高比例相应扩大的屏幕空间范围。回想一下，正切等于直角三角形对边与邻边的比率。这里邻边的长度为1，因此对边的长度为$tan ( "fov" \/ 2 )$。通过该长度的倒数进行缩放，将视野映射到范围$[-1, 1]$。
]
#block(sticky: true)[#raw("<<Scale canonical perspective view to specified field of view>>=")] <fragment-Scalecanonicalperspectiveviewtospecifiedfieldofview-0>
#block(breakable: false)[
```cpp
...
#parec[
  ...
  与 `OrthographicCamera` 一样，`PerspectiveCamera`的构造函数计算关于其生成的射线如何随像素移动而变化的信息。在这种情况下，射线的原点不变，射线的微分仅在方向上不同。这里计算的是：像素位置偏移时，相机空间中近平面上的对应点发生多大位移。
]
#block(sticky: true)[#raw("<<Compute differential changes in origin for perspective camera rays>>=")] <fragment-Computedifferentialchangesinoriginforperspectivecamerarays-0>
#block(breakable: false)[
```cpp
...
#block(sticky: true)[#raw("<<PerspectiveCamera Private Members>>=") #link(<fragment-PerspectiveCameraPrivateMembers-1>)[↓]] <fragment-PerspectiveCameraPrivateMembers-0>
#block(breakable: false)[
```cpp
Vector3f dxCamera, dyCamera;
```
...
#parec[
  ...
  透视相机视场最大角度的余弦有时会很有用。特别是，可以用指向待测点的方向与观察方向作点积，再与此余弦值比较，从而快速剔除视场外的点。可以通过计算相机视图向量与图像某个角落的向量之间的角度来找到这个余弦（见@fig:perspective-view-max-cosine）。这里需要对这个角落进行小的调整，以考虑用于根据位置对图像样本加权的每个像素中心的滤波函数的宽度（这个主题将在@image-reconstruction 中讨论）。
]
...
#figure(
  ...
  caption: [
    #ez_caption[
      ...
      计算透视相机最大视角的余弦。一个包围透视相机观察方向的方向锥可以通过将相机的观看方向作为中心轴，并计算该轴与指向图像某个角落的向量之间的角度 $theta$ 的余弦值来得到。在相机空间中，只需先将该向量归一化，再取其 $z$ 分量。
    ]
  ],
)<perspective-view-max-cosine>

#block(sticky: true)[#raw("<<Compute cosTotalWidth for perspective camera>>=")] <fragment-ComputemonocosTotalWidthforperspectivecamera-0>
#block(breakable: false)[
```cpp
...
#block(sticky: true)[#raw("<<PerspectiveCamera Private Members>>+=") #link(<fragment-PerspectiveCameraPrivateMembers-0>)[↑]] <fragment-PerspectiveCameraPrivateMembers-1>
#block(breakable: false)[
```cpp
Float cosTotalWidth;
```
...
#block(sticky: true)[#raw("<<PerspectiveCamera Method Definitions>>=")] <fragment-PerspectiveCameraMethodDefinitions-0>
#block(breakable: false)[
```cpp
...
#parec[
  ...
  `GenerateRayDifferential()` 沿用 `GenerateRay()` 的实现，另外加入下面的片段来计算微分射线。
]


#block(sticky: true)[#raw("<<Compute offset rays for PerspectiveCamera ray differentials>>=")] <fragment-ComputeoffsetraysforusePerspectiveCameraraydifferentials-0>
#block(breakable: false)[
```cpp
if (lensRadius > 0) {
...
#block(breakable: false)[
=== #ez_caption[The Thin Lens Model and Depth of Field][薄透镜模型与景深]
<thin-lens-model-and-depth-of-field>
#parec[
...
#parec[
  ...
  真实的相机有透镜系统，通过有限大小的光圈将光线聚焦到胶片平面上。 相机设计师（以及使用可调光圈相机的摄影师）面临一个权衡：光圈越大，到达胶片的光线越多，所需的曝光时间越短。 然而，透镜只能聚焦在一个平面上（_对焦平面_），场景中离这个平面越远的物体就越模糊。 随着光圈的增大，这种效果会更加明显。
]
...
#parec[
  ...
  在薄透镜近似下，与光轴平行并通过透镜的入射光线在透镜后面的一个点聚焦，该点称为焦点。 透镜到其后方焦点的距离 $f$ 称为透镜的#emph[焦距]。 如果胶片平面放置在透镜后面等于焦距的距离处，则无限远的物体将聚焦，因为它们在胶片上成像为一个点。
]
...
#parec[
  ...
  考虑一个焦距为 $f$ 的薄透镜，以及场景中相对于它位于深度 $z$ 的点。高斯透镜方程给出物体到透镜的距离与透镜到该点像的距离之间的关系：
]
...
#parec[
  ...
  若要在深度 $z$ 处对焦，即将 $z$ 设为#emph[对焦距离]，可用高斯透镜方程求出所需的透镜与胶片间距（@fig:thin-lens-focus）：
]
...
field].
  ...
  不在对焦平面上的点在胶片平面上成像为一个圆盘，而不是一个点。 这个圆盘的边界称为弥散圆。 弥散圆的大小受光线通过的光圈直径、对焦距离以及物体与透镜之间的距离的影响。 虽然弥散圆只有在单个深度处半径为零，但附近的一些深度的弥散圆足够小，看起来仍然是对焦的。 （只要其弥散圆小于像素间距，点就会有效地看起来是对焦的。） 看起来清晰的深度范围称为景深。
]
...
#parec[
  ...
  @fig:dof-images 展示了 #emph[Watercolor] 场景中的这一效果。透镜光圈越大，远离对焦平面的点就越模糊。图像中央的笔筒在所有图中都保持清晰，因为对焦平面正好位于它所在的深度。@fig:ecosys-dof 展示了景观场景中的景深效果，它将读者的视线引向图像中央清晰的草地。
]
...
#figure(
  table(
    ...
    [#ez_caption[(a) No defocus][(a) 无散焦]], [#ez_caption[(b) small aperture][(b) 小光圈]], [#ez_caption[(c) large Lens aperture][(c) 大光圈]],
  ),
  caption: [
    #ez_caption[ (a) Scene rendered with no defocus blur, (b) extensive depth of field due to a relatively small lens aperture, which gives only a small amount of blurriness in the out-of-focus regions, and (c) a very large aperture, giving a larger circle of confusion in the out-of-focus areas, resulting in a greater amount of blur on the film plane. (Scene courtesy of Angelo Ferretti.)][ (a) 没有散焦模糊渲染的场景，(b) 由于相对较小的镜头光圈而产生的广阔景深，这使得失焦区域只有少量模糊，以及 (c) 一个非常大的光圈，在失焦区域形成更大的弥散圆，导致胶片平面上出现更多的模糊。（场景由 Angelo Ferretti 提供。）]
  ],
  kind: image,
)<dof-images>
...
#parec[
  The Gaussian lens equation also lets us compute the size of the circle of confusion; given a lens with focal length $f$ that is focused at a distance $z_f$, the film plane is at $z'_f$. Given another point at depth $z$, the Gaussian lens equation gives the distance $z prime$ that the lens focuses the point to. This point is either in front of or behind the film plane; @fig:circle-of-confusion-area (a) shows the case where it is behind.
][
  高斯透镜方程还让我们可以计算弥散圆的大小；给定一个焦距为 $f$ 的透镜，聚焦在距离 $z_f$ 处，胶片平面位于 $z'_f$。 给定另一个深度为 $z$ 的点，高斯透镜方程给出了透镜将该点聚焦到的距离 $z prime$。 这个点要么在胶片平面前面，要么在后面；@fig:circle-of-confusion-area (a) 显示了它在后面的情况。
]
...
#parec[
  ...
  弥散圆的直径由 $z prime$ 和透镜之间的锥体与胶片平面的截交圆决定。 如果我们知道透镜的直径 $d_l$，那么我们可以使用相似三角形来求解弥散圆的直径 $d_c$ （@fig:circle-of-confusion-area(b)）：
]


$ frac(d_l, z prime) = frac(d_c, lr(|z prime - z'_f|)) . $
...
$ d_c = lr(|frac(d_l (z prime - z'_f), z prime)|) . $
...
#parec[
  Note that the diameter of the circle of confusion is proportional to the diameter of the lens. The lens diameter is often expressed as the lens's _f-number_ $n$, which expresses diameter as a fraction of focal length, $d_l = f / n$.
][
  注意，弥散圆的直径与透镜的直径成比例。透镜直径常借助_光圈数_ $n$ 表示，两者与焦距的关系是 $d_l = f / n$。
]
...
#parec[
  ...
  @fig:circle-of-confusion-graph 显示了该函数的图形，使用50毫米焦距的透镜和25毫米的光圈，聚焦在 $z_f = 1 upright(" m")$。注意，对焦平面前后的模糊程度是不对称的，前方物体的模糊增长速度比后方更快。
]
...
#parec[
  ...
  为获得平滑的散焦模糊，通常需要为每个像素追踪许多射线，对透镜进行充分采样。@fig:field-depth-11 展示了@fig:ecosys-dof 中的景观场景，但每像素仅使用 4 个样本（@fig:ecosys-dof 每像素使用 2048 个样本）。
]
...
#block(sticky: true)[#raw("<<Modify ray for depth of field>>=")] <fragment-Modifyrayfordepthoffield-0>
#block(breakable: false)[
```cpp
if (lensRadius > 0) {
...
#parec[
  The `SampleUniformDiskConcentric()` function, which is defined in #link("https://pbr-book.org/4ed/Sampling_Algorithms/Sampling_Multidimensional_Functions.html#sec:unit-disk-sample")[Section A.5.1], takes a $(u , v)$ sample position in $lr([0,1))^2$ and maps it to a 2D unit disk centered at the origin $(0 , 0)$. To turn this into a point on the lens, these coordinates are scaled by the lens radius. The `CameraSample` class provides the $(u , v)$ lens-sampling parameters in the `pLens` member variable.
][
  `SampleUniformDiskConcentric()`函数在 #link("https://pbr-book.org/4ed/Sampling_Algorithms/Sampling_Multidimensional_Functions.html#sec:unit-disk-sample")[A.5.1 节]中定义，接受 $lr([0,1))^2$ 中的 $(u , v)$ 样本位置并将其映射到以原点 $(0 , 0)$ 为中心的二维单位圆盘。为了将其转换为透镜上的一个点，这些坐标按透镜半径缩放。`CameraSample`类在`pLens`成员变量中提供 $(u , v)$ 透镜采样参数。
]

#block(sticky: true)[#raw("<<Sample point on lens>>=")] <fragment-Samplepointonlens-0>
#block(breakable: false)[
```cpp
...
#parec[
  ...
  光线的起点是透镜上的这个点。现在需要确定新光线的正确方向。我们知道，_所有_通过透镜的给定图像样本的光线必须在对焦平面的同一点汇聚。此外，我们知道光线穿过透镜中心时方向不变，因此找到适当的汇聚点就是将针孔模型的未扰动光线与对焦平面相交，然后将新光线的方向设置为从透镜上的点到交点的向量。
]
...
#parec[
  ...
  对于这个简单模型，对焦平面垂直于 $z$ 轴，光线从原点开始，因此将穿过透镜中心的光线与对焦平面相交是直接的。交点的 $t$ 值由以下公式给出
]

$ t = frac("focalDistance", upright(bold(d))_z) . $


#block(sticky: true)[#raw("<<Compute point on plane of focus>>=")] <fragment-Computepointonplaneoffocus-0>
#block(breakable: false)[
```cpp
...
#parec[
  ...
  现在可以初始化光线。起点设置为透镜上的采样点，方向设置为光线穿过对焦平面上的点`pFocus`。
]

#block(sticky: true)[#raw("<<Update ray for effect of lens>>=")] <fragment-Updaterayforeffectoflens-0>
#block(breakable: false)[
```cpp
...
#parec[
  ...
  为了使用薄透镜计算射线微分，⟨Update ray for effect of lens⟩片段中使用的方法应用于在胶片平面上 $x$ 和 $y$ 方向上偏移一个像素的光线。实现此功能的片段⟨Compute OrthographicCamera ray differentials accounting for lens⟩和⟨Compute PerspectiveCamera ray differentials accounting for lens⟩未在此处包含。
]


#include "supplements/5.2-expanded.typ"

#parec[Editorial note: the fixed source’s collapsed panels `fragbit-247` and `fragbit-252`, for computing the image-plane area at $z = 1$, contain no code. The fragment names are retained above; no implementation is invented here. The lens diagrams use signed scene coordinates along negative $z$, while the later 1-m focus example states a positive distance; that original notation is retained.][校订说明：固定原文中，计算 $z = 1$ 处像平面面积的折叠面板 `fragbit-247` 和 `fragbit-252` 均无代码。上文保留片段名称，不补写无来源的实现。透镜示意图将场景置于负 $z$ 侧，后面的 1 m 对焦示例则以正值描述距离；此处保留原文记法。]
