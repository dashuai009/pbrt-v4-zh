...
### 隔离构建与原文范围分类
- 检查点af664ff已推送到codex/book-audit-shiroa，草稿PR #20：https://github.com/dashuai009/pbrt-v4-zh/pull/20 。远端Linux检查已启动；本地另在/tmp/pbrt-v4-zh-validation固定该提交构建。Pages仍未部署，不把草稿PR视为发布。
- 书后三文件/inline helper增量经另一agent独立核对后恢复指纹：5433条正文/顺序等逐项核对通过，References新增标签剥离后精确恢复原复核哈希；修复CSS Unicode负号造成的数学基线失效，真实Web原生样式解析及视觉已验证。
- 7.0–7.2与补充代码已独立复核通过；7.3继续初校。7.1固定源的fig:spinning-spheres只是未解析字串，独立检索全源未找到图，不虚构图号或图形。
- 原始候选清单仍保留全部21993项，明确将HTML nav内1081项分为原站导航，contents/index辅助页168项单列，正文候选20744项。没有删除候选或按数量判定完整；章首背景图补记实际资源URL。原文逐项本地位置映射仍待证据。

### 第六、七章与公共编号验收
- 第六章全部6.0–6.10、第七章全部7.0–7.5已独立复核。6.8独立复核覆盖全文、57代码、14fold、320源SVG和72显示式；真实三语全书编译与中文500–529全30页/英双语复杂页通过。6.7补充915重复项已由该独立复核者改为6.8精确引用，另附增量证据。
- 原书数字与本地别名不一致曾使分节重编号。新增41明确源别名、出处与固定编号，由另一agent逐条独立读对并实际查询41/41正确；有source-file却缺映射现在直接编译失败。9.1图9.3资源及n_g/n_s、B.2被转换成空数学的±√项是限定修复，已另做独立原文/视觉确认，不能宣称这些未完成章节已精校。
- 公共门禁独立复核揭示的报告修改/路径别名、公共JSON遗漏、PNG/生成入口快照漏项、零宽高/空站点等反例已补回归测试。公共语义依赖跟随template/styles/数据，报告内容也封存哈希；73条原记录先保留原有正文证据补报告指纹，后按独立公共集成报告补共享依赖，未扩大正文审校范围。此前遗漏入账的A.2/A.3按已有完整独立报告补记。
- af664ff的远端Linux检查成功：https://github.com/dashuai009/pbrt-v4-zh/actions/runs/34167898211 。新增修复尚须新提交/CI，不复用旧成功当新版本验收。
- 原HTML另有62个Jeri互动图、128个图像资源。新增明确候选项及资源位置，清单总候选22055；仍不把数量或文件存在当完整性证明。固定SVG中可能包含原书合法静态图版，不能只因HTML用EXR就判定SVG错误。8.20已派生PNG并保留EXR/参数/版本/hash，另一agent实际Jeri比较构图、亮度、颜色和噪声差异通过，最终8.0–8.3整节复核仍进行中。
- 真实全书引用与目标归属导出已试验合并：固定af664ff的中文/英文各1584引用、1201目标与旧两个导出器逐字段完全相同，可少做两次重复全书排版。旧导出器保留作诊断，完整构建改用合并导出。
