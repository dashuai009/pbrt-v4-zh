...
### 6.8 内部检查点 A（in_progress，未释放）

导言至6.8.1 Utility Routines末尾已依固定HTML重建：对应原p索引1–125中56个非空段（按源DOM序，禁止当段数完整证明）、3图6.38–6.40及图注、浮点表示/算术规则/辅助函数小标题、18辅助片段和Infinity。修复原公式s*m缺失隐含1.m、二进制/十六进制展示、最大有限值说明被中文注释吞掉、NaN检测结尾和fast-math脚注、机器epsilon完整数值与C/C++两倍定义区别、有限精度能实现正确舍入的遗漏句、舍入运算中FMA与分离乘加混淆/LaTeX字面串、FloatBits及原始片段结构。源代码未改算法。NaN比较概括、负值区间端点、Exponent负输入符号位及指针重解释读取风险均保留源文并明确校注。独立复核尚未进行；6.8.1 Error Propagation之后仍待处理，不因本阶段编译通过判整节完成。

当前局部编译入口audit-rounding-first-preview.typ生成/tmp/audit-rounding-first-bi.pdf，完成内部检查后清理入口。复杂后续式计划使用固定源SVG保真，保留math.equation标签；根agent已同意。6.8仍独占，尚未释放。

## 批次6释放：6.8（awaiting_review）

固定Managing_Rounding_Error.html八子节已全部逐段读对：206个非空原正文段、三组列表/类型说明所涉及内容、57可见代码、72个展示数学式（其中16编号式6.17–6.32）、9图6.38–6.46、10原书脚注及14折叠面板。范围包括导言、浮点表示/算术/辅助函数、误差传播/运行误差分析、保守包围盒求交、精确判别式、水密三角形、交点误差与重投影/三角形/双线性/曲线/变换、新射线起点、排除负t及讨论。清单见audit/reports/edit-shapes-6.8-map.json。无表/书目列表/习题；文献引用使用固定source-cite。原始转换稿保存audit/quarantine/chapter-6.8-before-initial-review.typ.txt。

除检查点A外，完整补回旧稿遗漏的四数求和第二方案误差界、前向/后向分析、gamma定义与分式区间不能约消、球面/圆柱精确判别式全推导及两张对比渲染、所有重投影/插值误差推导、Transform的分组求和、阴影终点限制及讨论。原来\\gamma混入C++、公式置入中文/英文错位、缺失编号、注释吞掉最大有限数段、损坏LaTeX字面串等均以源恢复。57/57可见代码体逐token与固定源一致，忽略空白和文学片段名称的数学朗读串转写，不修改算法或标识符。

复杂公式采用同一正文中的固定原SVG，共320份存chapter-6-Shapes/supplements/6.8-math，独占helper为6.8-source-math.typ。72个展示式全部保持源SVG字符/上下标/圈运算符，仍放在带原标签的math.equation中参与统一编号；前段简单行内式及中文行内式用Typst。320/320资源经XML规范化比较，与固定原HTML中实际SVG完全相等；只修复HTML解析对大小写敏感viewBox造成的小写属性，未改几何/文字。首次视觉预览确实显示该大小写错误导致缺字，已修复并重渲确认；这再次说明编译成功不代表排版正确。浮点减号在MathJax朗读标题中仅称minus，但实际字符为圈减U+2296，已以原SVG恢复，中文对应浮点运算同步使用圈减。

折叠覆盖：951/952、954/955/957、959/960、961–964为本节可见片段或组合。953四坐标变换与956/958带输入误差计算，已对照chapter-3-Geometry_and_Transformations/supplements/3.9-expanded.typ第50及63行的实际完整实现，只有映射，不复制。Curve误差界已在本节可见，6.7补充仍是独立前文引用版本，由根agent在全书集成时决定是否需消除跨章展示重复；本批未回写已释放6.7。

源技术限制与校注：NaN所有比较为假的概括不含!=；次正规零值示意指数-127与实际-126的说明保留；机器epsilon与C/C++的2倍差别保留；负值区间端点及相对误差分母绝对值条件说明；x/x等恒等式不涵盖零/NaN/无穷；指针转换本身与不兼容类型访问区分；源Exponent未屏蔽负数符号位；重投影界不捕获已有切向偏差；投影矩阵w≠1误差代码bug明确保留；OffsetRayOrigin误差界本身未向上舍入而期望1ulp覆盖的原脚注完整补回；ShadowEpsilon仍必要、阴影终点问题未解决；三角形推导忽略高阶项、双线性epsilon为输入量级方案均照实呈现。没有把这些限制处理成“已证明所有输入稳健”。

验证：最终Typst0.13.1+fonts三语局部构建通过/tmp/audit-rounding-first-{zh,en,bi}.pdf。实际查看四数误差传播18页（修复SVG后/tmp/audit-rounding-first-18-final.png）、两种判别式对比及推导25页、浮点边函数28页、球面重投影32页、重心插值34页、带输入误差变换39页、偏移点/关键原脚注42页、三角形误差推导48页，公式/图像/代码均完整可见且无裁切。初期辅助函数14/16页亦已查看。局部编号不作为全书编号验收，Web响应布局仍需根agent全书构建与手机复核。临时audit-rounding-first-preview.typ已清理，diff检查通过。

6.8正文、6.8专属SVG/helper及本报告全部释放，不再编辑。至此本任务负责的6.7–6.10均完成初校、等待另一agent独立原文复核；没有将初校或编译成功当作已精校。
