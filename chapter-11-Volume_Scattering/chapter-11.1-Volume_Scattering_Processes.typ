...
#parec[
  ...
  本章中的所有模型都基于粒子位置不相关的假设——换句话说，尽管它们的密度可能在空间上变化，但它们的位置是独立的。 （在@spectral-analysis-of-sampling-patterns 中引入的噪声颜色的背景下，这个假设是它们位置具有白噪声分布。） 这个假设对许多类型的物理介质并不成立；例如，不可能两个粒子都在空间中的同一点，因此真正的白噪声分布是不可能的。 请参阅本章末尾的“进一步阅读”部分，了解放宽此假设的最新研究。
]
...
#parec[
  Absorption is described by the medium's #emph[absorption coefficient];, $sigma_a$, which is the probability density that light is absorbed per unit distance traveled in the medium. (Note that the medium absorption is distinct from the absorption coefficient used in specifying indices of refraction of conductors, as introduced in @the-fresnel-equations-for-conductors.) It is usually a spectrally varying quantity, though we will neglect the implications of that detail in this chapter and return to them in @improving-the-sampling-techniques . Its units are reciprocal distance ($upright(m)^(- 1)$), which means that $sigma_a$ can take on any nonnegative value; it is not required to be between $0$ and $1$, for instance. In general, the absorption coefficient may vary with both position $upright(p)$ and direction $omega$, although the volume scattering code in `pbrt` models it as purely a function of position. We will therefore sometimes simplify notation by not including $omega$ in the use of $sigma_a$ and other related scattering properties, though it is easy enough to reintroduce when it is relevant.
][
  吸收由介质的#emph[吸收系数（absorption coefficient）] $sigma_a$ 描述，它是光在介质中每单位距离被吸收的概率密度。 （注意，介质吸收与在@the-fresnel-equations-for-conductors 中介绍的用于指定导体折射率的吸收系数不同。） 它通常是一个关于光谱的变化量，尽管我们将在本章中忽略这一细节的影响，并在@improving-the-sampling-techniques 中继续讨论。 其单位是距离的倒数（$upright(m)^(- 1)$），这意味着 $sigma_a$ 可以取任何非负值；例如，它不必在 $0$ 和 $1$ 之间。 一般来说，吸收系数可能随位置 $upright(p)$ 和方向 $omega$ 变化，尽管`pbrt`中的体积散射代码将其建模为纯粹是位置的函数。 因此，我们有时会通过不在使用 $sigma_a$ 和其他相关散射特性时包含 $omega$ 来简化符号，尽管在相关时重新引入它很容易。
]

#parec[
  @fig:absorption-basic shows the effect of absorption along a very short segment of a ray. Some amount of radiance $L_upright(i) (upright(p) , - omega)$ is arriving at point $upright(p)$, and we would like to find the exitant radiance $L_o (upright(p) , omega)$ after absorption in the differential volume. This change in radiance along the differential ray length $dif t$ is described by the differential equation. #footnote[The position for the $L_upright(i)$ functions should actually be $upright(p) + dif t omega$, though in a slight abuse of notation we will here and elsewhere use $upright(p)$.
  ]
][
  @fig:absorption-basic 展示了沿着光线上一段非常短的段的吸收效果。某些量的辐射度 $L_upright(i) (upright(p) , - omega)$ 到达点 $upright(p)$，我们希望在微分体积吸收后找到出射辐射度 $L_o (upright(p) , omega)$。 这种沿微分光线长度 $dif t$ 的辐射度变化由微分方程描述。#footnote[对于 $L_upright(i)$ 函数的位置实际上应该是 $upright(p) + dif t omega$，不过在这里以及其他地方，我们会稍微滥用符号，直接使用 $upright(p)$。]
]



$
  L_o (upright(p) , omega) - L_upright(i) (upright(p) , - omega) = dif L_o (upright(p) , omega) = - sigma_a (upright(p) , omega) L_upright(i) (upright(p) , - omega) dif t ,
$
...
#figure(
  ...
  caption: [
    #ez_caption[
      ...
      radiance at a point $upright(p)$ from direction $- omega$. If the ray passes
      ...
      $d L_o (p , omega) = - sigma_a (p , omega) L_upright(i) (p , - omega) dif t .$
    ][
      图11.3：吸收减少了通过参与介质的光线沿线的辐射度。考虑一条在点$upright(p)$从方向$- omega$携带入射辐射度的光线。如果光线穿过一个充满吸收粒子的微小圆柱体，由于这些粒子的吸收导致的辐射度变化为
      $d L_o (p , omega) = - sigma_a (p , omega) L_upright(i) (p , - omega) dif t .$
    ]
  ],
)<absorption-basic>
...
#parec[
  This differential equation can be solved to give the integral equation describing the total fraction of light absorbed for a ray. If we assume that the ray travels a distance $d$ in direction $omega$ through the medium starting at point $upright(p)$, the surviving portion of the original radiance is given by
][
  这个微分方程可以求解得出描述光线吸收的总比例的积分方程。如果我们假设光线从点 $upright(p)$ 开始沿方向 $omega$ 穿过介质行进距离 $d$，则原始辐射度的存活部分为
]

$ e^(- integral_0^d sigma_a (upright(p) + t omega , omega) dif t) . $
=== Emission
<emission>


#parec[
  While absorption reduces the amount of radiance along a ray as it passes through a medium, emission increases it due to chemical, thermal, or nuclear processes that convert energy into visible light. @fig:vol-emission shows emission in a differential volume, where we denote emitted radiance added to a ray per unit distance at a point $upright(p)$ in direction $omega$ by $sigma_a (upright(p) , omega) L_e (upright(p) , omega)$. @fig:explosion-emission shows the effect of emission with a data set from a physical simulation of an explosion.
][
  虽然吸收减少了光线穿过介质时的辐射度，但由于化学、热或核等过程将能量转化为可见光，发射增加了辐射度。@fig:vol-emission 显示了微分体积中的发射，我们用 $sigma_a (upright(p) , omega) L_e (upright(p) , omega)$ 表示在点 $upright(p)$ 沿方向 $omega$ 每单位距离增加到光线的发射辐射度。@fig:explosion-emission 显示了发射的效果，使用了物理模拟爆炸的数据集。
]
...
#figure(
  ...
  caption: [
    #ez_caption[
      The volume emission function $L_e (upright(p) , omega)$ gives
      ...
      due to emission per differential distance is given by @eqt:volume-emission-diff-eq.
    ][
      体积发射函数$L_e (upright(p) , omega)$给出了光线穿过发射粒子的微分体积时辐射度的变化。由于发射每微分距离的辐射度变化由 @eqt:volume-emission-diff-eq 给出。
    ]
  ],
)<vol-emission>
...
#figure(
  ...
  caption: [
    #ez_caption[
      A Participating Medium Where the Dominant Volumetric Effect Is Emission. #emph[(Scene courtesy of Jim Price.)]
    ][
      参与介质中主要的体积效应是发射。#emph[(场景由Jim Price提供。)]
    ]
  ],
)<explosion-emission>
...
$
  dif L_o (upright(p) , omega) = sigma_a (upright(p) , omega) L_e (upright(p) , omega) dif t .
$<volume-emission-diff-eq>
...
#parec[
  Note that this equation incorporates the assumption that the emitted light $L_e$ is not dependent on the incoming light $L_upright(i)$. This is always true under the linear optics assumptions that `pbrt` is based on.
][
  注意，这个方程包含了假设发射光 $L_e$ 不依赖于入射光 $L_upright(i)$。在`pbrt`基于的线性光学假设下，这始终成立。
]
...
#figure(
  ...
  caption: [
    #ez_caption[
      Like absorption, out scattering also reduces the radiance along a ray. Light that hits particles may be scattered in another direction such that the radiance exiting the region in the original direction is reduced.
    ][
      与吸收类似，外散射也减少了沿光线的辐射度。光线击中粒子可能会被散射到另一个方向，从而减少了原方向上离开区域的辐射度。
    ]
  ],
)<vol-outscatter>

#parec[
  The probability of an out-scattering event occurring per unit distance is given by the scattering coefficient, $sigma_s$. Similar to absorption, the reduction in radiance along a differential length $dif t$ due to out scattering is given by
][
  单位距离内发生外散射事件的概率由散射系数 $sigma_s$ 给出。类似于吸收，沿微分长度 $dif t$ 由于外散射导致的辐射度减少可表示为：
]

$
  dif L_o (upright(p) , omega) = - sigma_s (upright(p) , omega) L_upright(i) (upright(p) , - omega) dif t.
$
...
#parec[
  ...
  由于吸收和外散射导致的辐射减少总量由 $sigma_a + sigma_s$ 给出。这种吸收和外散射的综合效应称为#emph[衰减（attenuation）];或#emph[消光（extinction）];。这两个系数的和表示为衰减系数 $sigma_t$ ：
]

$
  sigma_t (upright(p) , omega) = sigma_a (upright(p) , omega) + sigma_s (upright(p) , omega).
$
...
$
  rho (upright(p) , omega) = frac(sigma_s (upright(p) , omega), sigma_t (upright(p) , omega)).
$



#parec[
  Under the assumptions of radiometry, the single-scattering albedo is always between 0 and 1. It describes the probability of scattering (versus absorption) at a scattering event. The second is the #emph[mean free path];, $1 \/ sigma_t (upright(p), omega)$, which gives the average distance that a ray travels in a medium with attenuation coefficient $sigma_t (upright(p), omega)$ before interacting with a particle.
][
  在辐射测量的假设下，单次散射反照率总是在0到1之间。它描述了在散射事件中散射（相对于吸收）的概率。第二个是#emph[平均自由路径（mean free path）];， $1 \/ sigma_t (upright(p), omega)$，它表示在具有衰减系数 $sigma_t (upright(p), omega)$ 的介质中，光线在与粒子相互作用之前行进的平均距离。
]
=== In Scattering
<in-scattering>
#parec[
  ...
  虽然外散射会由于在不同方向的散射而减少沿光线的辐射，但#emph[内散射（in scattering）];则考虑了由于从其他方向散射而增加的辐射（@fig:vol-inscatter）。@fig:cloud-inscattering 显示了使用云模型的内散射效果。对于没有吸收的地方，对应于单次散射反照率为1。因此光在云中多次散射，使其呈现出非常不同的外观。
]
...
#figure(
  ...
  caption: [
    #ez_caption[
      In scattering accounts for the increase in radiance along a ray due to scattering of light from other directions. Radiance from outside the differential volume is scattered along the direction of the ray and added to the incoming radiance.
    ][
...
#figure(
  ...
  caption: [
    #ez_caption[
      In Scattering with the Cloud Model. For these scenes, there is no absorption and only scattering, which gives a substantially different result than the clouds in @fig:cloud-absorption. (a) Relatively dense cloud. (b) Thinner cloud._ (Cloud model courtesy of Walt Disney Animation Studios.)_
    ][
      在使用云模型进行散射的情况下，这些场景中不存在吸收，只有散射，这与@fig:cloud-absorption 中的云呈现出显著不同的结果。(a) 相对密集的云层。(b) 较薄的云层。_(云模型由华特迪士尼动画工作室提供。)_
    ]
  ],
)<cloud-inscattering>
...
#parec[
  ...
  假设粒子之间的间隔至少是它们半径的几倍，则在描述特定位置的散射时可以忽略粒子间的相互作用。在此假设下，#emph[相位函数] $p(omega, omega prime)$ 描述了在某一点的散射辐射的角分布；它是BSDF的体积模拟。然而，用BSDF类比并不精确。例如，相位函数有一个归一化约束：对于所有 $omega$，条件
]


$
  integral_(S^2) p(omega ,omega prime) dif omega prime = 1
$<phase-function-normalization>

#parec[
  must hold. #footnote[This difference is purely due to convention; the phase function could have equally well been defined to include the albedo, like the BSDF.] This constraint means that phase functions are probability distributions for scattering in a particular direction.
][
  必须成立。#footnote[这种差异纯粹是由于约定造成的；相位函数同样可以被定义为包含反照率，就像 BSDF 一样。] 这一约束意味着相位函数是在特定方向上散射的概率分布。
]

#parec[
  The total added radiance per unit distance is given by the #emph[source function] $L_s$ :
][
  每单位距离的总增加辐射由#emph[源函数（source function）] $L_s$ 给出：
]

$ dif L_o (upright(p) , omega) = sigma_t (upright(p) , omega) L_s (upright(p) , omega) dif t $
...
$
  L_s (upright(p), omega) = frac(sigma_a (upright(p) comma omega), sigma_t (upright(p) comma omega)) L_e (upright(p), omega ) + frac(sigma_s (upright(p) comma omega), sigma_t (upright(p) comma omega)) integral_(S^2) p(upright(p), omega_upright(i), omega) L_(upright(i)) (upright(p) , omega_upright(i) ) thin dif omega_upright(i) .
