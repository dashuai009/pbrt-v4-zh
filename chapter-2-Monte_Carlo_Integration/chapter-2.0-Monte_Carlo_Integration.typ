...
= #ez_caption[Monte Carlo Integration][蒙特卡洛积分]
...
#parec[
  ...
  渲染充满了积分问题。除了光传输方程@eqt:rendering-equation 之外，在接下来的章节中我们还会看到，积分方程还描述了与光相关的多种其他量，包括相机中的传感器响应、参与介质中光的衰减和散射，以及皮肤等材质的散射。这些积分方程通常没有解析解，因此我们必须求助于数值方法。尽管标准的数值积分技术，如梯形积分或高斯求积，在解决低维平滑积分方面很有效，但它们计算渲染中常见的高维、不连续积分时，收敛较慢。蒙特卡洛积分技术提供了解决这一问题的一种方法。它们使用随机采样来计算积分，其收敛速度与被积函数的维数无关。
]


#parec[
  Monte Carlo integration #footnote[For brevity, we will refer to Monte Carlo integration simply as “Monte Carlo.”] has the useful property that it only requires the ability to evaluate an integrand $f(x)$ at arbitrary points in the domain in order to estimate the value of its integral $integral f(x) d x$. This property not only makes Monte Carlo easy to implement but also makes the technique applicable to a broad variety of integrands. It has a natural extension to multidimensional functions; in @light-transport-i-surface-reflection, we will see that the light transport algorithm implemented in the `RandomWalkIntegrator` can be shown to be estimating the value of an infinite-dimensional integral.
][
  蒙特卡洛积分#footnote("为简洁起见，我们将蒙特卡洛积分简称为“蒙特卡洛”。")具有一个有用的特性，即只需能够在定义域内任意点计算被积函数 $f(x)$ 的值，就能估计其积分 $integral f(x) d x$ 的值。这一特性不仅使蒙特卡洛易于实现，而且使该技术适用于广泛的被积函数。它自然地扩展到多维函数；在 @light-transport-i-surface-reflection 中，我们将看到在 `RandomWalkIntegrator` 中实现的光传输算法可以被证明是在估计一个无限维积分的值。
]
...
#parec[
  ...
  明智地使用随机性已经彻底改变了算法设计领域。随机算法大致分为两类：_拉斯维加斯_和_蒙特卡洛_。拉斯维加斯算法是那些使用随机性但最终总是给出相同结果的算法（例如，在快速排序中选择一个随机数组元素作为枢轴）。另一方面，蒙特卡洛算法随运行中使用的具体随机数给出不同的结果，但_平均而言_能给出正确答案。因此，通过平均多次运行蒙特卡洛算法（使用相同的输入）的结果，可以找到一个从统计意义上看很可能接近真实答案的结果。
]
...
#parec[
  ...
  以下各节将讨论蒙特卡洛积分的基本原理，特别关注那些在 `pbrt` 中广泛应用的原理。另请参见@Appendix-A，其中包含了在系统中较少使用的其他蒙特卡洛采样函数的实现。
]
