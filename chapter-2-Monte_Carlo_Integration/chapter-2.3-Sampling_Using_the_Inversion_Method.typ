...
== #ez_caption[Sampling Using the Inversion Method][使用逆变换法采样]
...
#parec[
  ...
  为了计算 @eqt:MC-estimator 中的蒙特卡洛估计量，必须能够从选定的概率分布中抽取随机样本。有多种技术可以实现这一点，但在渲染领域，最重要的方法之一是#emph[逆变换法]，它通过对分布的累积分布函数（CDF）求逆将 $\[ 0 , 1 \)$ 中的均匀样本映射为服从给定一维概率分布的样本。（在@sampling-with-multidimensional-transformations 中，我们将看到如何通过考虑一系列一维分布将这种方法应用于更高维的函数。）当与第 8 章中定义的采样器生成的分布良好的样本一起使用时，逆变换法可以特别有效。在本书的其余部分，我们将看到逆变换法应用于从 BSDF、光源、相机和散射介质所定义的分布中生成样本。
]

=== #ez_caption[Discrete Case][离散情形]

#parec[
  @eqt:sampling-discrete-random leads to an algorithm for sampling from a set of discrete probabilities using a uniform random variable. Suppose we have a process with four possible outcomes where the probabilities of each of the four outcomes are given by $p_1$, $p_2$, $p_3$, and $p_4$, with $sum_i p_i = 1$. The corresponding PMF is shown in @fig:discrete-pdf.
][
  @eqt:sampling-discrete-random 给出了一个利用均匀随机变量按一组离散概率进行采样的算法。假设我们有一个具有四种可能结果的过程，其中每种结果的概率分别为 $p_1$ 、 $p_2$ 、 $p_3$ 和 $p_4$，且 $sum_i p_i = 1$。相应的 PMF 如@fig:discrete-pdf 所示。
]
...
#parec[
  There is a direct connection between the sums in @eqt:sampling-discrete-random and the definition of the CDF. The discrete CDF is given by
][
  @eqt:sampling-discrete-random 中的和与 CDF 的定义之间有直接联系。离散 CDF 由下式给出：
]

$ P_i = sum_(j = 1)^i p_j, $

#parec[
  which can be interpreted graphically by stacking the bars of the PMF on top of each other, starting at the left. This idea is shown in @fig:discrete-cdf.
][
  从图形上看，可以从左到右依次将 PMF 的条形竖直叠加，以理解这个公式。这一想法如@fig:discrete-cdf 所示。
]
...
#figure(
  ...
  caption: [
    #ez_caption[
      A Discrete CDF, Corresponding to the PMF in @fig:discrete-pdf. Each column's height is given by the PMF for the event that it represents plus the sum of the PMFs for the previous events, $P_i = sum_(j=1)^i p_j$.
    ][
      离散累积分布函数 (CDF)，对应于@fig:discrete-pdf 中的概率质量函数 (PMF)。每个条形的高度等于其所代表事件的 PMF 值，加上它之前所有事件的 PMF 值之和，$P_i = sum_(j=1)^i p_j$。
    ]
  ],
) <discrete-cdf>

#parec[
  The sampling operation of @eqt:sampling-discrete-random can be expressed as finding $i$ such that
][
  @eqt:sampling-discrete-random 的采样操作可以表示为找到满足以下条件的 $i$ ：
]

$ P_(i - 1) lt.eq xi < P_i, $ <discrete-sample-cdf>

#parec[
  which can be interpreted as inverting the CDF $P$, and thus, the name of the technique. Continuing the graphical interpretation, this sampling operation can be considered in terms of projecting the events' probabilities onto the vertical axis where they cover the range $[0 , 1]$ and using a random variable $xi$ to select among them (see @fig:discrete-inversion). It should be clear that this draws from the correct distribution—the probability of the uniform sample hitting any particular bar is exactly equal to the height of that bar.
][
  这可以解释为对 CDF $P$ 求逆，方法名称也由此而来。沿用图形解释，可以将这一采样操作看作将事件的概率投影到垂直轴上，覆盖范围 $[0 , 1]$，并使用随机变量 $xi$ 在其中选择（见@fig:discrete-inversion）。显然，这样抽取的样本服从所需分布——均匀样本击中任何特定条的概率正好等于该条的高度。
]
...
#parec[
  ...
  `SampleDiscrete()` 函数实现了这个算法。它接受一组不一定归一化的非负权重、一个均匀随机样本 `u`，并返回其中一个权重的索引，其概率与权重成比例。它执行的采样操作对应于找到满足以下条件的 $i$ ：
]

$ sum_(j = 1)^(i - 1) w_j lt.eq xi sum w_i < sum_(j = 1)^i w_j, $ <discrete-sample-cdf-weights>

#parec[
  which corresponds to multiplying @eqt:discrete-sample-cdf by $sum w_i$. (Not requiring a normalized PMF is a convenience for calling code and not much more work in the function's implementation.) Two optional parameters are provided to return the value of the PMF for the sample as well as a new uniform random sample that is derived from `u`.
][
  这对应于将@eqt:discrete-sample-cdf 乘以 $sum w_i$。（不要求 PMF 归一化，可以方便调用方，且几乎不增加函数实现的工作量。）提供了两个可选参数，以返回样本的 PMF 值以及从 `u` 派生的新均匀随机样本。
]
...
#block(sticky: true)[#raw("<<Sampling Inline Functions>>+=") #link(<fragment-SamplingInlineFunctions-1>)[▲] #link(<fragment-SamplingInlineFunctions-3>)[▼]] <fragment-SamplingInlineFunctions-2>
...
#block(sticky: true)[#raw("<<Handle empty weights for discrete sampling>>=")] <fragment-Handleemptymonoweightsfordiscretesampling-0>
#block(breakable: false)[
```cpp
if (weights.empty()) {
    if (pmf)
...
#block(sticky: true)[#raw("<<Compute sum of weights>>=")] <fragment-Computesumofmonoweights-0>
#block(breakable: false)[
```cpp
Float sumWeights = 0;
for (Float w : weights)
...
#parec[
  Following @eqt:discrete-sample-cdf-weights, the uniform sample `u` is scaled by the sum of the weights to get a value $u'$ that will be used to sample from them. Even though the provided `u` value should be in the range $[0, 1)$, it is possible that `u * sumWeights` will be equal to `sumWeights` due to floating-point round-off. In that rare case, `up` is bumped down to the next lower floating-point value so that subsequent code can assume that `up < sumWeights`.
][
  根据@eqt:discrete-sample-cdf-weights，均匀样本 `u` 通过权重总和进行缩放，得到一个值 $u'$，用于从这些权重中采样。尽管提供的 `u` 值应在范围 $lr([0, 1))$ 内，但由于浮点舍入，`u * sumWeights` 可能等于 `sumWeights`。在这种罕见情况下，`up` 被降低到下一个较低的浮点值，以便后续代码可以假设 `up < sumWeights`。
]

#block(sticky: true)[#raw("<<Compute rescaled u' sample>>=")] <fragment-Computerescaledusample-0>
#block(breakable: false)[
```cpp
...
#parec[
  ...
  现在要在权重数组中找出最后一个满足条件的偏移量 $i$：随机样本 `up` 大于截至该位置前的累计权重。采样使用从数组开始进行的线性搜索，累积权重总和，直到总和大于 $u'$。
]

#block(sticky: true)[#raw("<<Find offset in weights corresponding to u'>>=")] <fragment-Findoffsetinmonoweightscorrespondingtou-0>
#block(breakable: false)[
```cpp
int offset = 0;
Float sum = 0;
...
#parec[
  ...
  `while` 循环结束后，提供的样本 `u` 中的随机性仅用于选择数组中的一个元素——一个离散选择。样本在夹住它的 CDF 值之间的偏移量本身是一个均匀随机值，可以很容易地重新映射到 $lr([0, 1))$。如果请求，这个值在 `uRemapped` 中返回给调用者。
]
...
#block(sticky: true)[#raw("<<Compute PMF and remapped u value, if necessary>>=")] <fragment-ComputePMFandremappedmonouvalueifnecessary-0>
#block(breakable: false)[
```cpp
if (pmf)
...
#figure(
  ...
  caption: [
    #ez_caption[
      To use the inversion method to draw a sample from the distribution described by the PMF in @fig:discrete-pdf, a canonical uniform random variable is plotted on the vertical axis. By construction, the horizontal extension of $xi$ will intersect the box representing the $i$th outcome with probability $p_i$. If the corresponding event is chosen for a set of random variables $xi$, then the resulting distribution of events will be distributed according to the PMF.
    ][
      要使用逆变换法从@fig:discrete-pdf 中的 PMF 描述的分布中抽取样本，将一个标准均匀随机变量绘制在垂直轴上。通过构造，$xi$ 的水平延伸将以概率 $p_i$ 与表示第 $i$ 个结果的框相交。如果为一组随机变量 $xi$ 选择了相应的事件，那么事件的结果分布将按照 PMF 分布。
    ]
  ],
) <discrete-inversion>


=== #ez_caption[Continuous Case][连续情形]
<continuous-case>

#parec[
  In order to generalize this technique to continuous distributions, consider what happens as the number of discrete possibilities approaches infinity. The PMF from @fig:discrete-pdf becomes a PDF, and the CDF from @fig:discrete-cdf becomes its integral. The projection process is still the same, but it has a convenient mathematical interpretation—it represents inverting the CDF and evaluating the inverse at $xi$.
][
  为了将这种技术推广到连续分布，考虑离散可能性数量趋近无穷时的情况。@fig:discrete-pdf 中的概率质量函数（PMF）变成了概率密度函数（PDF），@fig:discrete-cdf 中的累积分布函数（CDF）变成了它的积分。投影过程并未改变，但现在有了简便的数学解释——它表示对 CDF 求逆并在 $xi$ 处求其反函数的值。
]
...
#parec[
  ...
  更准确地说，我们可以从 PDF $p (x)$ 中抽取一个样本 $X_i$，步骤如下：
]



#parec[
  1. Integrate the PDF to find the CDF#footnote[In general, the lower limit of integration should be $-infinity$, although if $p(x) = 0$ for $x < 0$, this equation is equivalent.] $P(x) = integral_0^x p(x') d x'$.
][
  1. 对 PDF 积分，求得 CDF#footnote[一般而言，积分下限应为 $-infinity$；但若 $x < 0$ 时 $p(x) = 0$，则此公式与之等价。] $P(x) = integral_0^x p(x') d x'$。
]
...
#parec[
  ...
  3. 通过求解方程 $xi = P(X)$ 中的 $X$ 来生成样本；换句话说，求得 $X = P^(- 1) (xi)$。
]
...
#parec[
  ...
  我们将通过一个简单例子说明此算法；更多应用示例请参见第 A.4 节。
]



==== #ez_caption[Sampling a Linear Function][线性函数采样]
...
#parec[
  ...
  在区间 $[0 , 1]$ 上定义的函数 $f (x) = (1 - x) a + x b$ 在 $x = 0$ 处为 $a$，在 $x = 1$ 处为 $b$，进行线性插值。此处我们假设 $a , b gt.eq 0$ ；本章末尾的一道习题将讨论更一般的情况。
]

#block(sticky: true)[#raw("<<Math Inline Functions>>=") #link("https://pbr-book.org/4ed/Shapes/Managing_Rounding_Error.html#fragment-MathInlineFunctions-1")[▼]] <fragment-MathInlineFunctions-0>
#block(breakable: false)[
```cpp
...
#block(sticky: true)[#raw("<<Sampling Inline Functions>>+=") #link(<fragment-SamplingInlineFunctions-2>)[▲] #link(<fragment-SamplingInlineFunctions-4>)[▼]] <fragment-SamplingInlineFunctions-3>
#block(breakable: false)[
```cpp
...
#block(sticky: true)[#raw("<<Sampling Inline Functions>>+=") #link(<fragment-SamplingInlineFunctions-3>)[▲] #link(<fragment-SamplingInlineFunctions-5>)[▼]] <fragment-SamplingInlineFunctions-4>
#block(breakable: false)[
```cpp
...
#parec[
  ...
  需注意返回语句中的 `std::min` 调用，确保返回值在区间 $lr([0, 1))$ 内。尽管采样算法在给定 $xi in lr([0, 1))$ 的情况下在该范围内生成值，但舍入误差可能导致结果等于 1。因为调用采样例程的一些代码依赖于返回值在指定范围内，所以采样例程必须确保这一点。
]
...
#parec[
  ...
  除了提供从分布中采样和计算样本 PDF 的函数外，`pbrt` 通常还提供对采样操作求逆的函数，返回对应于值 $x$ 的随机样本 $xi$。在一维情况下，这相当于计算 CDF。
]

#block(sticky: true)[#raw("<<Sampling Inline Functions>>+=") #link(<fragment-SamplingInlineFunctions-4>)[▲] #link(<fragment-SamplingInlineFunctions-6>)[▼]] <fragment-SamplingInlineFunctions-5>
#block(breakable: false)[
```cpp
