...
== #ez_caption[Improving Efficiency][提高效率]
...
#parec[
  ...
  对于无偏的蒙特卡洛估计量，样本数量与方差（进而与误差）之间存在可靠的关系，这为我们提供了便利。如果我们得到一个噪声过大的渲染图像，增加样本数量将以可预测的方式减少误差，并且——在足够的计算资源下——可以生成质量足够好的图像。
]
...
#parec[
  ...
  然而，计算需要时间，而且往往时间不够用。电影的截止日期可能迫在眉睫，或者实时渲染器中的六十分之一秒时间片可能即将结束。样本数量因此受到限制，降低方差的唯一途径就是设法更充分地利用能够取得的样本。幸运的是，已经开发了多种技术来通过充分利用已有样本来改进基本的蒙特卡洛估计量；在这里，我们将讨论在 `pbrt` 中使用的一些最重要的技术。
]


=== #ez_caption[Stratified Sampling][分层采样]
<Stratified-Sampling>
...
#parec[
  ...
  `pbrt` 大量使用了一类经典且有效的方差缩减技术——基于精心放置采样点以更好地捕捉被积函数特征（或者更准确地说，减少错过重要特征的可能性）。分层采样将积分域分解为多个层，并在每个层中放置采样点；在这里，我们将从方差减少的角度分析这种方法。稍后，在@fourier-analysis-of-variance 中，我们将借助傅里叶分析工具再次讨论这一方法，以获得更深入的理解。
]
...
#parec[
  ...
  分层采样将积分域 $Lambda$ 细分为 $n$ 个不重叠的层 $Lambda_1 , Lambda_2 , dots.h , Lambda_n$ 。每个区域称为一个#emph[层]，它们必须完全覆盖原始域：
]
...
#parec[
  ...
  为了从 $Lambda$ 中抽取样本，我们按各层内部的密度 $p_i$，从每个 $Lambda_i$ 中抽取 $n_i$ 个样本。一个简单的例子是像素超采样。使用分层采样，像素周围的区域被划分为一个 $k times k$ 的网格，并在每个网格单元内均匀地抽取一个样本。这比取 $k^2$ 个随机样本要好，因为样本位置不太可能聚集在一起。在这里，我们将解释为什么这种技术能减少方差。
]
...
#parec[
  ...
  其中 $X_(i , j)$ 是从密度 $p_i$ 中抽取的第 $j$ 个样本。总体估计为 $F = sum_i v_i F_i$ ，其中 $v_i$ 是第 $i$ 层的体积比例（ $v_i in \(0 , 1\]$ ）。
]
...
#parec[
  ...
  因此，当该层有 $n_i$ 个样本时，层内估计量的方差为 $sigma_i^2 \/ n_i$ 。这表明总体估计量的方差为
]
...
#parec[
  ...
  如果我们合理地假设样本数量 $n_i$ 与体积 $v_i$ 成正比，那么我们有 $n_i = v_i n$ ，并且总体估计量的方差为
]
$
...
#parec[
  ...
  为了比较此结果与不分层采样的方差，我们注意到抽取一个未分层样本相当于根据体积 $v_i$ 定义的离散概率分布随机选择一个层 $I$ ，然后在 $Lambda_I$ 中选择一个随机样本 $X$ 。从这个意义上说， $X$ 是以 $I$ 为条件选择的，因此可以使用条件概率来证明
]
...
#parec[
  where $Q$ is the mean of $f$ over the whole domain $Lambda$ .#footnote[ See Veach (@veach1997robust) for a derivation of this result.
]
][
  其中 $Q$ 是函数 $f$ 在整个定义域 $Lambda$ 上的均值。#footnote[ 关于此结果的详细推导，参见 Veach（@veach1997robust）。 ]
]
...
#parec[
  ...
  关于@eqt:strat-variance，有两点需要注意。首先，我们知道右侧第二个求和项必须是非负的，因为方差总是非负的。其次，它表明分层采样永远不会增加方差。分层总是减少方差，除非右侧第二个求和项恰好为 0。只有当函数 $f$ 在每个层 $Lambda_i$ 上的均值相同时，右侧第二个求和项才可能为 0。为了使分层采样的效果达到最佳，我们应尽量使右侧第二个求和项最大化，因此最好使各层的均值尽可能不同。这解释了为何在缺乏函数 $f$ 信息的情况下，选择紧凑的层更为可取。如果层很宽，它们将包含更多的变化，并且 $mu_i$ 会更接近真实均值 $Q$ 。
]
...
#parec[
  ...
  @fig:mc-strat-images 展示了在渲染包含光泽反射的图像时，使用分层采样与独立随机分布采样的效果。几乎无需增加运行时间，方差就能得到相当程度的降低。
]
...
#figure(
  table(
    ...
    [#ez_caption[(a) Independent][(a) 独立采样]], [#ez_caption[(b) Stratified][(b) 分层采样]],
  ),
  caption: [
...
#parec[
  The main downside of stratified sampling is that it suffers from the same "curse of dimensionality" as standard numerical quadrature. Full stratification in $D$ dimensions with $S$ strata per dimension requires $S^D$ samples, which quickly becomes prohibitive. Fortunately, it is often possible to stratify some of the dimensions independently and then randomly associate samples from different dimensions; this approach will be used in @stratified-sampler. Choosing which dimensions are stratified should be done in a way that stratifies dimensions that tend to be most highly correlated in their effect on the value of the integrand (Owen @owen1998latin).
][
  分层采样的主要缺点是它与标准数值积分一样，受到“维数灾难”的影响。在 $D$ 维空间中，每维划分 $S$ 个层并进行完全分层，需要 $S^D$ 个样本，这很快变得不可行。幸运的是，通常可以独立地对某些维度进行分层，然后随机关联来自不同维度的样本；这种方法将在 @stratified-sampler 中使用。选择联合分层的维度时，应优先选择那些对被积函数值的影响往往具有最高相关性的维度（Owen @owen1998latin）。
]


=== #ez_caption[Importance Sampling][重要性采样] <importance-sampling>


#parec[
  Importance sampling is a powerful variance reduction technique that exploits the fact that the Monte Carlo estimator
][
  重要性采样是一种强大的方差减少技术，它利用了以下事实：如果样本是从与被积函数 $f (x)$ 相似的分布 $p (x)$ 中抽取的，那么蒙特卡洛估计量
]
...
#parec[
  To see why such sampling distributions reduce error, first consider the effect of using a distribution $p (x) prop f (x)$ , or $p (x) = c f (x)$ .#footnote[We will generally assume that $f(x) gt.eq 0$; if it is negative, we might set $p(x) prop |f(x)|$. See the "Further Reading" section for more discussion of this topic.] It is trivial to show that normalization of the PDF requires that
][
  要了解为什么这种采样分布能减少误差，首先考虑使用分布 $p (x) prop f (x)$ ，或 $p (x) = c f (x)$ 的效果。#footnote[我们通常假设 $f(x) gt.eq 0$；如果它是负数，我们可能设 $p(x) prop |f(x)|$。有关此主题的更多讨论，请参阅“延伸阅读”部分。] 很容易证明，PDF 的归一化要求
]
...
#parec[
  ...
  估计量的方差为零！当然，这是荒谬的，因为如果我们能直接积分 $f$ ，就不会费心使用蒙特卡洛方法。然而，如果可以找到一个形状与 $f (x)$ 相似的密度 $p (x)$ ，方差就会减少。
]



#parec[
  As a more realistic example, consider the Gaussian function $f (x) = e^(- 1000 (x - 1 \/ 2)^2)$ , which is plotted in @fig:gaussian-pdf-and-samples (a) over $[0 , 1]$ . Its value is close to zero over most of the domain. Samples $X$ with $X < 0.2$ or $X > 0.3$ are of little help in estimating the value of the integral since they give no information about the magnitude of the bump in the function's value around $1 \/ 4$ . With uniform sampling and the basic Monte Carlo estimator, variance is approximately $0.0365$ .
][
作为一个更现实的例子，考虑高斯函数 $f (x) = e^(- 1000 (x - 1 \/ 2)^2)$ ，它在 $[0 , 1]$ 上绘制在@fig:gaussian-pdf-and-samples (a) 中。它的值在大部分定义域上接近于零。如果在 $X < 0.2$ 或 $X > 0.3$ 上对 $X$ 采样，对于估计积分值几乎没有帮助，因为它们没有提供关于函数在 $1 \/ 4$  附近峰值幅值的信息。使用均匀采样和基本的蒙特卡洛估计量，方差大约为 $0.0365$ 。
  #translator[固定上游此处写作 $1/4$，并给出 $X<0.2$、$X>0.3$，但上面的高斯公式中心是 $1/2$；原书数值不一致，待核实，未擅改。]
]
...
$
  ...
  cases(
    delim: "{",
     0.1 & quad x in \[0 comma 0.45\) ,
     9.1 & quad x in \[0.45 comma 0.55\),
     0.1 & quad x in \[0.55 comma 1\)
  )
$
...
#parec[
  which is plotted in @fig:gaussian-pdf-and-samples (b), and the estimator from @eqt:MC-estimator is used instead, then variance is reduced by a factor of approximately 6.7 times. A representative set of 6 points from this distribution is shown in @fig:gaussian-pdf-and-samples (c); we can see that most of the evaluations of $f(x)$ are in the interesting region where it is not nearly zero.
][
  该函数绘制于 @fig:gaussian-pdf-and-samples (b)，若采用@eqt:MC-estimator 的估计量，方差将降至原来的约 $1/6.7$。从该分布中抽取的一组有代表性的 6 个点如 @fig:gaussian-pdf-and-samples (c) 所示；可见，$f(x)$ 的多数求值点都位于其值不接近零、值得关注的区域。
]
...
#figure(
  table(
    ...
    [
      #image("../pbr-book-website/4ed/Monte_Carlo_Integration/piecewise-gaussian-pdf.svg")
    ],
    [
...
$ p (x) = cases(delim: "{", 1.2 & quad x in \[ 0 comma 0.4\), 0.2 & quad x in \[ 0.4 \, 0.6\), 1.2 & quad x in \[ 0.6 \, 1\)) $
...
#parec[
  ...
  来估计高斯函数的积分。这个 PDF 增加了函数值接近零处的采样概率，并减少了其幅值较大处的采样概率。
]
...
这不仅减少了在被积函数值较大区域生成的样本数量，而且在这些区域采样时，蒙特卡洛估计量中的 $f(x)\/p(x)$ 幅值会特别高，因为该区域 $p(x) = 0.4$。结果是方差约为均匀采样时的 $5.4$ 倍、上述较好 PDF 对应方差的近 $36$ 倍。在渲染中进行蒙特卡洛积分的背景下，计算被积函数通常需要付出追踪光线的开销，因此应尽量减少样本数量；采用较差的采样分布，再靠计算更多样本来弥补，并不是理想的选择。
  #translator[固定上游正文为 $0.4$，而紧邻分段公式的中段值为 $0.2$；此处保留原书并记录待核实。]
]


=== #ez_caption[Multiple Importance Sampling][多重重要性采样]
...
#parec[
  ...
  我们经常遇到被积函数是两个或多个函数乘积的积分： $integral f_a (x) f_b (x) d x $ 。通常可以为单个因子分别推导出重要性采样策略，却未必能得到与这些因子的乘积相匹配的采样策略。这种情况在涉及光传输的积分中尤其常见，例如光传输方程@eqt:rendering-equation 中的 BSDF、入射辐亮度和余弦因子的乘积。
]
...
#parec[
  ...
  为了理解将蒙特卡洛应用于此类乘积的难点，假设目前幸运地拥有两个采样分布 $p_a$ 和 $p_b$ ，它们完全匹配 $f_a$ 和 $f_b$ 的分布。（实际上，这种情况通常不会发生。）使用@eqt:MC-estimator 的蒙特卡洛估计量，我们有两个选择：我们可以使用 $p_a$ 抽取样本，给出估计量
]
...
#parec[
  where $c$ is a constant equal to the integral of $f_a$ , since $ p_a (x) prop f_a (x)$ . The variance of this estimator is proportional to the variance of $f_b$, which may itself be high.#footnote[Note that the definition of variance in @eqt:variance-initial does not preclude computing the variance of a function itself.] Conversely, we might sample from $p_b$ , though doing so gives us an estimator with variance proportional to the variance of $f_a$ , which may similarly be high. In the more common case where the sampling distributions only approximately match one of the factors, the situation is usually even worse.
][
  其中 $c$ 是一个常数，等于 $f_a$ 的积分，因为 $ p_a (x) prop f_a (x)$ 。这个估计量的方差与 $f_b$ 的方差成正比，而后者本身可能很高。#footnote[注意，@eqt:variance-initial 中的方差定义也允许计算函数本身的方差。]相反，我们可以从 $p_b$ 采样，尽管这样做会给我们一个方差与 $f_a$ 的方差成正比的估计量，这同样可能很高。在采样分布仅近似匹配其中一个因子的情况下，情况通常更糟。
]
...
#parec[
  ...
  不幸的是，显而易见的解决方案是从每个分布中抽取一些样本并平均两个估计量，但这并没有显著改善。因为方差是可加的，一旦方差进入估计量，我们就无法通过将其添加到另一个低方差估计量来消除它。
]
...
#parec[
  ...
  多重重要性采样（MIS）正是针对这一问题，提供了一种易于实施的方差减少技术。基本思想是，在估计积分时，我们应该从多个采样分布中抽取样本，希望至少有一个能合理地匹配被积函数的形状，即使我们不知道是哪一个。MIS 随后提供了一种方法来加权来自每种技术的样本，可以消除由于被积函数值与采样密度不匹配而导致的大方差峰值。鼓励编写仅针对特殊情况进行特殊采样的专用代码，因为当这些情况发生时，它们会减少方差，而在一般情况下成本相对较低。
]



#parec[
  With two sampling distributions $p_a$ and $p_b$ and a single sample taken from each one, $X ~ p_a$ and $Y ~ p_b$ , the MIS Monte Carlo estimator is
][
  对于两个采样分布 $p_a$ 和 $p_b$ ，以及从每个分布中抽取的一个样本 $X ~ p_a$ 和 $Y ~ p_b$ ，MIS 蒙特卡洛估计量为
]
...
#parec[
  ...
  其中 $w_a$ 和 $w_b$ 是权重函数，其选取应使这个估计量的期望值等于 $f(x)$ 的积分值。
]
...
#parec[
  ...
  （使估计量无偏的权重函数须满足以下全部条件：当 $f(x) eq.not 0$ 时，权重函数之和为 1，即 $sum_(i = 1)^n w_i (x) = 1$，并且如果 $p_i (x) = 0$ ，则 $w_i (x) = 0$ 。）
]
...
#parec[
  ...
  每次计算得到的 $f$ 值，都除以所有 PDF 在该样本处的值之和，而不仅是生成该样本的 PDF 值。因此，如果 $p_a$ 在自身采样密度较低、而 $p_b$ 的采样密度较高的点生成样本，则除以 $p_a (X) + p_b (X)$ 会减少该样本的贡献。这实际上降低了从 $p_a$ 取得的这些样本的权重，因为 $p_b$ 对应的采样技术在积分域中的这些点上更有效。只要有一种采样技术有合理的概率在函数值较大的点采样，MIS 权重就能显著降低方差。
]
...
#block(sticky: true)[#raw("<<Sampling Inline Functions>>=") #link(<fragment-SamplingInlineFunctions-1>)[▼]] <fragment-SamplingInlineFunctions-0>
```cpp
...
#block(sticky: true)[#raw("<<Sampling Inline Functions>>+=") #link(<fragment-SamplingInlineFunctions-0>)[▲] #link("https://pbr-book.org/4ed/Monte_Carlo_Integration/Sampling_Using_the_Inversion_Method.html#fragment-SamplingInlineFunctions-2")[▼]] <fragment-SamplingInlineFunctions-1>
```cpp
...
#parec[
  ...
  给出积分的无偏估计。在单样本模型中，平衡启发式已被证明是最优方法。
]
...
#parec[
  ...
  多重重要性采样的一个缺点是，如果其中一种采样技术与被积函数非常匹配，MIS 可能会稍微增加方差。在渲染应用中，MIS 能降低那些本来可能具有高方差的情形的方差，因此几乎总是值得采用。
]

==== #ez_caption[MIS Compensation][MIS 补偿]
...
#parec[
  ...
  多重重要性采样（MIS）通常应用于使用单独对被积函数进行重要性采样时有效的概率分布，这些分布在被积函数为非零的地方都有非零的生成样本的概率。然而，当使用 MIS 时，并不要求所有的概率密度函数（PDF）在函数值为非零的地方都为非零；只需在每个这样的点上，至少有一个 PDF 非零即可。
]
...
=== #ez_caption[Russian Roulette][俄罗斯轮盘赌]
<russian-roulette>
...
#parec[
  ...
  其中被积函数由一些容易计算的因子 $f (X)$ （例如，与表面如何散射光有关的因子）和其他计算开销更大的因子组成，例如需要追踪光线的二元可见性因子 $v (X)$ 。在这些情况下，评估估计量的计算开销主要在于 $v$ 。
]
...
#parec[
  ...
  俄罗斯轮盘赌从不减少方差。事实上，除非恰好有 $c = F$ ，否则它总是会增加方差。然而，如果选择概率使得对最终结果贡献可能很小的样本被跳过，它确实提高了蒙特卡洛的效率。
]

=== #ez_caption[Splitting][分裂]
<mc-splitting>
...
#parec[
  ...
  俄罗斯轮盘赌减少样本数量，而分裂则增加多维积分中某些维度上的样本数量，以提高效率。例如，考虑以下一般形式的积分：
]
...
#parec[
  ...
  采用标准重要性采样估计方法，我们可能从独立分布中抽取 $n$ 个样本， $X_i ~ p_x$ 和 $Y_i ~ p_y$ ，并计算
]
...
#parec[
  ...
  分裂使我们能够将针对 $A$ 中每个样本在 $B$ 上抽取多个样本的想法形式化。通过分裂，我们可能为每个样本 $X_i$ 抽取 $m$ 个样本 $Y_(i,j)$ ，给出估计量
]

$ frac(1, n) sum_(i = 1)^n frac(1, m) sum_(j = 1)^m frac(f(X_i comma Y_(i comma j)), p_x (X_i) p_y (Y_(i comma j))) . $

#parec[
  If it is possible to partially evaluate $f(X_i, dot)$ for each $X_i$ , then we can compute a total of $n m$ samples more efficiently than we had taken $n m$ independent $X_i$ values using @eqt:splitting-std-estimator.
][
  如果能针对每个 $X_i$ 预先完成 $f(X_i, dot)$ 的部分计算，那么计算总共 $n m$ 个样本时，就能比使用 @eqt:splitting-std-estimator 并抽取 $n m$ 个独立的 $X_i$ 值更高效。
]
...
#parec[
  ...
  以渲染为例，计算图像的像素颜色时，需要求取形如 @eqt:splitting-candidate-integral 的积分：在像素区域 $A$ 上积分；对于像素内的每个点 $x$，向场景中追踪一条光线，再通过半球（这里记为 $B$）上的积分计算交点处的反射辐亮度，这又需要追踪一条或多条光线。通过分裂，我们可以为每个光照积分抽取多个样本，通过在这些样本上分摊从相机追踪初始光线的成本，从而提高效率。
]
