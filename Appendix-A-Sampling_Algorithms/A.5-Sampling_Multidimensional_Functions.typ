...
下面使用球坐标推导采样算法。根据第 #link("https://pbr-book.org/4ed/Monte_Carlo_Integration/Transforming_between_Distributions.html#sec:mc-transform-multiple-dimensions")[2.4.1] 节的公式 #link("https://pbr-book.org/4ed/Monte_Carlo_Integration/Transforming_between_Distributions.html#eq:spherical-cartesian-pdf-relation")[(2.23)]，有 $p(theta,phi)=frac(sin theta,2 pi)$。该密度可分离。由于 $phi$ 从 0 到 $2 pi$，且其 PDF 为常数，所以 $p(phi)=1/(2 pi)$，进而有 $p(theta)=sin theta$。两个 CDF 为
]
...
#parec[
Why does this work? Let $(r,phi)$ be the polar coordinates of the point chosen on the disk (note that we are using $phi$ instead of the usual $theta$ for the polar angle here). From Section #link("https://pbr-book.org/4ed/Sampling_Algorithms/Sampling_Multidimensional_Functions.html#sec:unit-disk-sample")[A.5.1], we know that the joint density #source-math("/Appendix-A-Sampling_Algorithms/supplements/math/A5-f0bb023d945f8397.svg",13.154,2.843,0.838,"p left-parenthesis r comma phi right-parenthesis equals r slash pi") gives the density of a point sampled on the disk.
][
为什么这样做有效？用 $(r,phi)$ 表示圆盘采样点的极坐标。注意，这里用 $phi$ 而非通常的 $theta$ 表示极角。由第 #link("https://pbr-book.org/4ed/Sampling_Algorithms/Sampling_Multidimensional_Functions.html#sec:unit-disk-sample")[A.5.1] 节可知，圆盘采样点的联合密度为 $p(r,phi)=r/pi$。
]
...
现在把该点映射到半球。如 @fig:malley 所示，竖直投影给出 $sin theta=r$。为完成 $(r,phi)=(sin theta,phi) arrow.r (theta,phi)$ 的变换，需要计算雅可比矩阵的行列式：
]
...
#parec[
It is sometimes useful to be able to uniformly sample rays in a cone of directions. This distribution is separable in $(theta,phi)$, with #source-math("/Appendix-A-Sampling_Algorithms/supplements/math/A5-727246f9ad3cf314.svg",14.157,2.843,0.838,"p left-parenthesis phi right-parenthesis equals 1 slash left-parenthesis 2 pi right-parenthesis"), and so we therefore need to derive a method to sample a direction $theta$ up to the maximum angle of the cone, $theta_("max")$. Incorporating the #source-math("/Appendix-A-Sampling_Algorithms/supplements/math/A5-0c4f565ef8ce03ec.svg",4.333,2.176,0.338,"sine theta") term from the measure on the unit sphere from Equation (#link("https://pbr-book.org/4ed/Radiometry,_Spectra,_and_Color/Working_with_Radiometric_Integrals.html#eq:sintheta-dtheta-dphi")[4.8]), we have
][
有时需要在一个圆锥方向范围内均匀采样射线。这一分布在 $(theta,phi)$ 上可分离，且 $p(phi)=1/(2 pi)$。因此，只需推导如何采样不超过圆锥最大角度 $theta_("max")$ 的 $theta$。计入公式 #link("https://pbr-book.org/4ed/Radiometry,_Spectra,_and_Color/Working_with_Radiometric_Integrals.html#eq:sintheta-dtheta-dphi")[(4.8)] 中单位球面测度的 $sin theta$ 因子，可得
]
