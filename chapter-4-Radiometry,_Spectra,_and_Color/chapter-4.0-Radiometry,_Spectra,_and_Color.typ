...
= #ez_caption[Radiometry, Spectra, and Color][辐射度量学、光谱与颜色]
...
#parec[
  ...
  在本章中，我们将首先介绍描述电磁辐射的四个关键量：辐射通量（Flux）、辐射强度（Intensity）、辐照度（Irradiance）和辐亮度（Radiance）。
]
...
#parec[
  ...
  这些辐射量通常是波长的函数。
  每个量的变化由其 #emph[光谱分布（Spectral Distribution）]描述——给出各波长上光的量的分布函数。
  （我们会将 #emph[光谱（Spectrum）]作为光谱分布的同义词使用，并使用 #emph[spectra（spectrum 的复数形式）]表示多个光谱分布。）
  在渲染领域，特别关注波长（wavelength）（ $lambda$ ）大约在 380 纳米至 780 纳米之间的电磁辐射，这一范围对应人类可见光。
  #footnote[可感知的完整波长范围略微超出了这个区间，尽管在这些波长下人眼的感光能力要低许多个数量级。在光谱曲线制表时，通常使用 360–830 纳米（nm）的范围作为保守界限。]
...
#parec[
  ...
  较短波长的光（$lambda approx 400 "nm"$）呈偏蓝的颜色，中间波长（$lambda approx 550 "nm"$）被感知为绿色，而较长波长（$lambda approx 650 "nm"$）则被感知为红色。
  准确建模颜色在渲染中有两个重要原因：
  首先，显示设备（如显示器）需要用颜色而非光谱来描述像素值，因此准确地将光谱转换为适当的颜色对于显示渲染图像很重要。
  其次，场景中物体的发光与反射属性通常以颜色的形式进行定义，而在渲染计算中，需要将这些颜色转换为光谱用于渲染。
