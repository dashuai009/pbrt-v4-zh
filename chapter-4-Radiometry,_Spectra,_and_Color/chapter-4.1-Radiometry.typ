...
#parec[
  ...
  在一些情况下，也会引入源自波动光学（Wave Optics）的光学现象，但这些结果需要用辐射传输的基本抽象语言进行表述。
]
...
#parec[
  ...
  - #emph[无偏振（No Polarization）：]包括可见光在内的电磁辐射是#emph[偏振]的。一个形象的类比是沿着拉紧的琴弦传播的振动。晃动琴弦一端会产生垂直方向的波动向另一端传播。然而，除了简单的线性振动外，琴弦还能够传导其他形式的振动，如顺时针或逆时针的圆形或椭圆形振动。光的偏振行为也存在类似的情况。然而，人类在没有辅助工具（如专用相机或偏振墨镜）的情况下，基本无法直接感知这种额外的偏振状态。因此，在 `pbrt` 中，我们采用一种常见的假设，即光是非偏振光，即由多种偏振方向的波叠加而成，人眼仅能感知其平均行为。因此，光在渲染中唯一需要关注的就是它在各个波长上的能量分布（或等效的频率）。
]
...
#parec[
  ...
  - #emph[稳态（Steady State）：]假设环境中的光已经达到平衡状态，其辐亮度分布随时间不再变化。在现实场景中，光传播到平衡状态几乎是瞬间完成的，因此这一假设在实践中不构成限制。不过需要注意，磷光现象也违反了稳态假设。
]
...
#parec[
  ...
  采用几何光学模型（Geometric Optics Model）带来的最显著限制是其与衍射（Diffraction）和干涉（Interference）效应的不兼容性。即使这种不兼容性可以被规避——例如，通过用#emph[维格纳分布函数（Wigner Distribution Function）]（@oh2010，@Cuypers2012）替代辐亮度，但此类扩展超出了本书的讨论范围。
]

=== #ez_caption[Basic Quantities][基本物理量]
<basic-quantities>
#parec[
  ...
  有四个辐射量是渲染的核心：通量（Flux）、辐照度/辐射出射度（Irradiance / Radiant Exitance）、辐射强度（Intensity）和辐亮度（Radiance）。
  它们可以通过依次对时间、面积和方向取极限，从能量中推导出来。
  所有这些辐射量通常都依赖于波长，尽管我们会将这一话题推迟到 @radiometric-spectral-distributions 部分讨论。
]

==== #ez_caption[Energy][能量]
<energy>
#parec[
...
#parec[
  #emph[Editorial note:] The pinned source prints the speed of light as 299,472,458 m/s. #link("https://physics.nist.gov/cuu/Constants/Value/c.html")[NIST] gives the exact vacuum value as 299,792,458 m/s. The original text is retained above; this is a verified source typographical error.
][
  #translator[固定原书将光速写作 299,472,458 m/s。已对照 #link("https://physics.nist.gov/cuu/Constants/Value/c.html")[NIST 官方常数表]核实，真空光速的精确值为 299,792,458 m/s。上方保留原文数值，并在此明确指出源文排印错误。]
]

==== #ez_caption[Flux][辐射通量]
<sec:flux>
#parec[
...
#parec[
  For example, given a light that emitted $Q = 200,000 thin "J"$ over the course of an hour, if the same amount of energy was emitted at all times over the hour, we can find that the light source's flux was
][
  例如，某光源在一小时内共发射了 $Q = 200,000 thin "J"$ 的能量，若这一小时内发射功率保持不变，则光源的通量为
]
...
#parec[
  ...
  请注意，我们这里的符号稍微有些不正式。其中一个原因是，光子是离散的能量量子，因此在微分时间趋近于零时取极限并不严格符合物理实际。然而，在渲染的情况下，由于我们关注的测量所涉及的光子数量极其庞大，这个细节并不会造成问题。
]
...
==== #ez_caption[Irradiance and Radiant Exitance][辐照度与辐射出射度]
...
#parec[
  ...
  更一般地，我们可以通过在点 $p$ 处对功率增量与面积增量之比取极限来定义辐照度和辐射出射度：
]
...
#parec[
  ...
  该定律指出，到达表面的光能量与光照方向和表面法向量之间夹角的余弦成正比（@fig:irradiance ）。
...
==== #ez_caption[Intensity][辐射强度]
<intensity>

#parec[
  Consider now an infinitesimal light source emitting photons. If we center this light source within the unit sphere, we can compute the angular density of emitted power. #emph[Intensity], denoted by $I$, is this quantity; it has units $"W"/"sr"$. Over the entire sphere of directions, we have
][
  现在考虑一个无限小的光源，其以各个方向发射光子。
  如果将这个光源放置在单位球体的中心，我们可以计算其辐射功率在角度上的密度。
  这一物理量被称为#emph[辐射强度（Intensity）]，用符号 $I$ 表示，单位为瓦特每球面度（$"W" \/ "sr"$）。
  把总通量按整个球面的立体角平均，得到
]
...
#parec[
  ...
  更一般地，如果我们感兴趣的是沿着一个微分立体角（Differential Solid Angle）的辐射功率密度，则可定义辐射强度为：
]
...
#parec[
  ...
  像往常一样，我们可以通过对辐射强度进行积分来求得辐射通量（功率）。
  已知辐射强度 $I(omega)$ 是方向 $omega$ 的函数，我们可以在一个有限的方向集合 $Omega$ 上进行积分，从而恢复出对应的辐射通量（功率）：
]
...
#parec[
  ...
  辐射强度用于描述光的方向分布，但它仅对点光源有物理意义。
]

==== #ez_caption[Radiance][辐亮度]
...
#parec[
  ...
  辐亮度进一步引入了方向维度，测量了相对于立体角的辐照度或辐射出射度。
  它的定义是
]

$
  L(p, omega) = lim_(Delta omega arrow.r 0) frac(Delta E_(omega) (p), Delta omega) = frac(d E_(omega) (p), d omega),
$

#parec[
  where we have used $E_(omega)$ to denote irradiance at the surface that is perpendicular to the direction $omega$.
  ...
  其中，$E_(omega)$ 表示在垂直于方向 $omega$ 的表面上的辐照度（Irradiance）。
  换句话说，辐亮度（Radiance）并不是相对于点 $p$ 所在表面接收到的辐照度进行测量的。
  实际上，这种测量面积的变化消除了辐亮度定义中朗伯定律的 $cos theta$ 因子。
]
...
#parec[
  ...
  辐亮度（Radiance）是单位面积、单位立体角内的通量密度。
  用辐射通量可定义为：
]
...
#parec[
  ...
  因此，辐亮度可以理解为，当感兴趣的入射方向立体角 $d omega$ 变得极小时，以及表面的局部微分面积 $d A$ 也趋于极小时，对表面上入射光的测量极限。
]

#figure(
  image("../pbr-book-website/4ed/Radiometry,_Spectra,_and_Color/pha04f03.svg"),
  caption: [
    #ez_caption[Radiance $L$ is defined as flux per unit solid angle $d omega$ per unit projected area $d A^(perp)$.][辐亮度 $L$ 定义为每单位立体角 $d omega$、每单位投影面积 $d A^(perp)$ 的辐射通量。]
  ],
) <pha04f03>
...
#parec[
  ...
  在所有这些辐射量中，辐亮度（Radiance）将是本书余下部分中使用最频繁的一个。
  这种频繁使用的直观原因在于，辐亮度在某种意义上是最基本的辐射量之一。
  如果已知辐亮度，则所有其他辐射量都可以通过对辐亮度在面积和方向上的积分计算得到。
  辐亮度的另一个优良性质是，在真空（空旷空间）中沿光线传播时，它保持不变。
  因此，辐亮度非常适合用于光线追踪（Ray Tracing）的计算，是描述光传播的自然选择。
]

=== #ez_caption[Incident and Exitant Radiance Functions][入射与出射辐亮度函数]
#parec[
...
$
  L^+ (p, omega) & = lim_(t arrow.r 0^+) L(p + t upright(bold(n))_p, omega),\
  L^- (p, omega) & = lim_(t arrow.r 0^-) L(p + t upright(bold(n))_p, omega) .
$ <radiance-limits-onesided>
#parec[
  where $upright(bold(n))_p$ is the surface normal at $p$. However, keeping track of one-sided limits throughout the text is unnecessarily cumbersome.
][
  其中 $upright(bold(n))_p$ 是点 $p$ 处的表面法向量。
  然而，在全文中始终显式跟踪这些单侧极限，会显得不必要地繁琐。
]
...
#parec[
  ...
  我们倾向于通过区分到达该点的辐亮度（如由光源照射产生的入射光）与从该点离开的辐亮度（如表面反射产生的出射光）来消除这种歧义。
]
...
#parec[
  ...
  这些更直观的入射与出射辐亮度函数，与 @eqt:radiance-limits-onesided 中的单侧极限之间存在简单关系：
]
...
#parec[
  ...
  在本书中，我们将使用入射与出射辐亮度函数的概念，以消除在边界处描述辐亮度时的歧义。
]
...
#parec[
  ...
  另一个需要牢记的性质是，在空间中没有表面的点（即自由空间内），辐亮度 $L$ 是连续的，因此 $L^+ = L^-$ ，这意味着
]

$
  L_(o) (p, omega) = L_(i) (p, - omega) = L(p, omega) .
$
...
#parec[
  ...
  换句话说， $L_(i)$ 和 $L_(o)$ 仅在方向上相反。
]

=== #ez_caption[Radiometric Spectral Distributions][辐射量的光谱分布]
...
#parec[
  ...
  到目前为止，我们定义的所有辐射量都没有考虑它们在波长上的分布变化。
  因此，它们实际上是对某个（未明确说明的）波长范围内的波长依赖量的积分。
  正如我们能够通过其他物理量的极限来定义各种辐射量一样，我们也可以通过对小波长区间的极限处理，定义它们的光谱形式（Spectral Variants）。
]

#parec[
  For example, we can define #emph[spectral radiance] $L_(lambda)$ as the limit of radiance over an infinitesimal interval of wavelengths $Delta lambda$,
][
  例如，我们可以定义#emph[光谱辐亮度（Spectral Radiance）] $L_(lambda)$ ，其为辐亮度在一个无限小波长区间 $Delta lambda$ 上的极限：
]

$
  L_(lambda) = lim_(Delta lambda arrow.r 0) frac(Delta L, Delta lambda) = frac(d L, d lambda) .
$
...
#parec[
  ...
  进一步地，辐亮度可以通过对光谱辐亮度在某个波长范围上的积分来求得：
]

$
  L = integral_(lambda_0)^(lambda_1) L_(lambda) (lambda) thin d lambda .
...
#parec[
  ...
  其他辐射量的光谱形式也可以以类似方式定义。
  所有这些光谱变体的单位中都多了一个 $1\/"m"$ 因子，以反映波长的依赖性。
]

=== #ez_caption[Luminance and Photometry][亮度与光度学]
...
#parec[
  ...
  所有辐射量（如通量、辐亮度等）都有其对应的光度量（Photometric Measurements）。
  #emph[光度学（Photometry）]是研究可见电磁辐射与人类视觉系统感知之间关系的学科。
  每个光谱辐射量都可以乘以光谱响应曲线 $V(lambda)$ 后，对波长积分，转换为对应的光度量，该曲线描述了人眼对不同波长的相对敏感度。
...
#parec[
  ...
  我们将用 $Y$ 表示亮度，其与光谱辐亮度（Spectral Radiance）的关系为：
]


$
  Y = integral_lambda L_(lambda) (lambda) V (lambda) thin d lambda .
$ <luminance>
...
#parec[
  ...
  亮度与光谱响应曲线 $V(lambda)$ 都与 XYZ 颜色表示密切相关，相关内容将在 @xyz-color 介绍。
]

#parec[
  The units of luminance are candelas per meter squared ( $"cd"/"m"^2$ ), where the candela is the photometric equivalent of radiant intensity. Some representative luminance values are given in @tbl:luminance-values.
][
  亮度的单位是坎德拉每平方米（$"cd"/"m"^2$），其中坎德拉（Candela）是辐射强度（Radiant Intensity）的光度学等效单位。一些具有代表性的亮度值将在 @tbl:luminance-values 给出。
]

#figure(
  table(
    columns: (1fr, 1fr),
    table.header([#ez_caption[Condition][情景]], [#ez_caption[Luminance ($"cd"/"m"^2$, or nits)][亮度（$"cd"/"m"^2$，或尼特）]]),
    [#ez_caption[Sun at horizon][地平线附近的太阳]], [600,000],
    [#ez_caption[60-watt lightbulb][60 瓦灯泡]], [120,000],
    [#ez_caption[Clear sky][晴空]], [8,000],
    [#ez_caption[Typical office][典型办公室]], [100–1,000],
    [#ez_caption[Typical computer display][典型计算机显示器]], [1–100],
    [#ez_caption[Street lighting][街道照明]], [1–10],
    [#ez_caption[Cloudy moonlight][多云时的月光]], [0.25],
  ),
  kind: table,
  caption: [#ez_caption[Representative Luminance Values for a Number of Lighting Conditions.][多种照明情景的典型亮度值。]],
) <luminance-values>
...
#parec[
  ...
  本章介绍的所有其他辐射量（Radiometric Quantities）都有其对应的光度量（Photometric Equivalents），它们总结在 @tbl:radiometric-photometric 表格里。#footnote[各类光度量的命名相对不太直观且容易混淆。吉姆·卡吉亚（Jim Kajiya）对此曾有过幽默的总结：“一尼特（nit）等于每球面度一勒克斯（lux），是每平方米上的一坎德拉（candela per square meter），是每平方米每球面度上的一流明（lumen per square meter per steradian）。明白了吗？”]
]

#figure(
  table(
    columns: (1fr, 1fr, 1fr, 1.8fr),
    table.header([#ez_caption[Radiometric][辐射度量]], [#ez_caption[Unit][单位]], [#ez_caption[Photometric][光度量]], [#ez_caption[Unit][单位]]),
    [#ez_caption[Radiant energy][辐射能量]], [#ez_caption[joule (J)][焦耳（J）]], [#ez_caption[Luminous energy][光能量]], [#ez_caption[talbot (T)][塔尔博（T）]],
    [#ez_caption[Radiant flux][辐射通量]], [#ez_caption[watt (W)][瓦特（W）]], [#ez_caption[Luminous flux][光通量]], [#ez_caption[lumen (lm)][流明（lm）]],
    [#ez_caption[Intensity][辐射强度]], [$"W"/"sr"$], [#ez_caption[Luminous intensity][发光强度]], [$"lm"/"sr"$ = #ez_caption[candela (cd)][坎德拉（cd）]],
    [#ez_caption[Irradiance][辐照度]], [$"W"/"m"^2$], [#ez_caption[Illuminance][照度]], [$"lm"/"m"^2$ = #ez_caption[lux (lx)][勒克斯（lx）]],
    [#ez_caption[Radiance][辐亮度]], [$"W"/("m"^2 dot "sr")$], [#ez_caption[Luminance][亮度]], [$"lm"/("m"^2 dot "sr") = "cd"/"m"^2$ = #ez_caption[nit][尼特]],
  ),
  kind: table,
  caption: [#ez_caption[Radiometric Measurements and Their Photometric Analogs.][辐射度量及其对应光度量。]],
