...
#parec[
  ...
  “光谱分布”和“颜色”看似是同一事物的两个名称，实则有别。光谱分布是一个纯粹的物理概念，而颜色描述了人类对光谱的感知。因此，颜色与人类视觉系统的生理学和大脑对视觉刺激的处理密切相关。
]
...
#parec[
  ...
  尽管 `pbrt` 中的大多数渲染计算是基于光谱分布的，但仍然必须仔细处理颜色。例如，渲染图像中每个像素的光谱分布必须转换为 RGB 颜色，以便在显示器上显示。准确执行此转换需要利用显示器的颜色特性信息。渲染器还会在场景描述中找到颜色，用于描述反射率和光发射。 虽然人类使用颜色来描述建模场景的外观很方便，但如果渲染器在光传输模拟中需要使用光谱分布，则这些颜色必须转换为光谱。不幸的是，这种转换的条件并不足以唯一确定结果。为此已经开发了多种方法；`pbrt` 中实现的方法在 @from-rgb-to-specturm 中描述。
]
...
matching functions], which are functions of wavelength that can be used to compute a tristimulus representation of a spectral distribution.
  ...
  颜色感知的#emph[三刺激理论]表明，对于人类观察者，所有可见光谱分布都可以用三个标量值准确表示。其基础是眼睛中有三种类型的视锥细胞，每种细胞对不同波长的光敏感。 自 19 世纪引入以来，该理论已在众多实验中得到验证，并导致了#emph[光谱匹配函数]的发展，这些函数是波长的函数，可用于计算光谱分布的三刺激表示。
]
...
$ v_i = integral S (lambda) m_(i) (lambda) thin d lambda . $
#parec[
  ...
  匹配函数因此定义了一个#emph[颜色空间]，即由三刺激值构成的三维向量空间：两个光谱之和的三刺激值由它们的三刺激值之和给出，并且通过常数缩放的光谱的三刺激值可以通过相同因子缩放三刺激值来找到。 请注意，根据这些定义，两个光谱分布的乘积的三刺激值#emph[不是]由它们的三刺激值的乘积给出的。 这就是为什么使用像 RGB 这样的三刺激颜色进行渲染可能不会给出准确结果的原因；我们将在@from-rgb-to-specturm 中对此主题进行更多讨论。
]
...
#figure(
  ...
  caption: [
    #ez_caption[*The XYZ Color Matching Curves.*
      ...
      按照 @eqt:xyz-matching，将给定光谱分布分别乘以三条匹配曲线并对波长积分，可得到 $x_lambda$、$y_lambda$ 和 $z_lambda$，从而转换为 XYZ。]
  ],
)<cie-xyz>
...
#parec[
  ...
  标准组织国际照明委员会（CIE）通过一系列以人为受试者的实验，确定了一组重要的颜色匹配函数。
]
...
#parec[
  ...
  它们定义了#emph[XYZ 颜色空间]，如@fig:cie-xyz 所示。XYZ 是一个#emph[与设备无关]的颜色空间，这意味着它不描述特定显示器或颜色测量设备的特性。
]

#parec[
  Given a spectral distribution $S (lambda)$, its XYZ color space coordinates $x_lambda$, $y_lambda$, and $z_lambda$ are computed by integrating its product with the $X (lambda)$, $Y (lambda)$, and $Z (lambda)$ spectral matching curves:#footnote[A variety of conventions are used to define these integrals, sometimes with other or no normalization factors.  For use in `pbrt`, the normalization by one over the integral of the $Y$ matching curve is convenient, as it causes a spectral distribution with a constant value of 1 to have $y_lambda = 1$.]
][
  给定光谱分布 $S (lambda)$，其 XYZ 颜色空间坐标 $x_lambda$ 、 $y_lambda$ 和 $z_lambda$ 是通过将其与 $X (lambda)$ 、 $Y (lambda)$ 和 $Z (lambda)$ 光谱匹配曲线的乘积积分来计算的：#footnote[使用各种约定来定义这些积分，有时使用其他归一化因子或没有归一化因子。对于`pbrt`中的使用，乘以 $Y$ 匹配曲线积分的倒数进行归一化很方便，因为它会使常数值为1的光谱分布具有$y_lambda = 1$。]
]

$
  x_lambda & = frac(1, integral_lambda Y (lambda) thin d lambda) integral_lambda S (lambda) X (lambda) thin d lambda ,\
  y_lambda & = frac(1, integral_lambda Y (lambda) thin d lambda) integral_lambda S (lambda) Y (lambda) thin d lambda ,\
  z_lambda & = frac(1, integral_lambda Y (lambda) thin d lambda) integral_lambda S (lambda) Z (lambda) thin d lambda .
$ <xyz-matching>


#parec[
  The CIE $Y (lambda)$ tristimulus curve was chosen to be proportional to the $V (lambda)$ spectral response curve used to define photometric quantities such as luminance in @eqt:luminance . Their relationship is $V(lambda) = 683 Y(lambda)$.
][
  CIE $Y(lambda)$ 三刺激曲线被选为与光谱响应曲线 $V(lambda)$ 成正比，后者用于定义光度量，例如 @eqt:luminance 中的亮度。两者的关系为 $V(lambda) = 683 Y(lambda)$。
]
...
#parec[
  ...
  值得注意的是，具有显著不同分布的光谱可能具有非常相似的 $x_lambda$ 、 $y_lambda$ 和 $z_lambda$ 值。对于人类观察者来说，这样的光谱看起来是相同的。这种光谱对称为#emph[同色异谱体（metamers）]。
]
...
#parec[
  ...
  @fig:xyz-3d-curve 显示了对应于可见范围内单个波长光的 XYZ 系数的 XYZ 空间中的曲线的三维图。 更复杂光谱分布的系数因此对应于沿此曲线的点的线性组合。虽然所有光谱分布都可以用 XYZ 系数表示，但并非所有 XYZ 系数值都对应于可实现的光谱；这样的系数集称为#emph[虚色]。
]

#figure(
  image("../pbr-book-website/4ed/Radiometry,_Spectra,_and_Color/pha04f19.svg"),
  caption: [
    #ez_caption[Plot of XYZ color coefficients for the wavelengths of light in the visible range. The curve is shaded with the RGB color associated with each wavelength.][可见波段各波长光的 XYZ 颜色系数曲线。曲线按各波长对应的 RGB 颜色着色。]
  ],
)<xyz-3d-curve>
...
#parec[
  ...
  `Spectra` 命名空间中的三个函数提供了在 360 至 830 纳米范围内以 1 纳米间隔采样的 CIE XYZ 匹配曲线。
]

#block(sticky: true)[#raw("<<Spectral Function Declarations>>+=")] <fragment-SpectralFunctionDeclarations-1>
```cpp
namespace Spectra {
...
#block(sticky: true)[#raw("<<Spectrum Constants>>+=")] <fragment-SpectrumConstants-2>
```cpp
...
#block(sticky: true)[#raw("<<XYZ Definition>>=")] <fragment-XYZDefinition-0>
```cpp
class XYZ {
  public:
    <<XYZ Public Methods>>
...
#parec[
  ...
  其实现很直接，用三个 `Float` 值表示三个颜色分量。`XYZ` 还提供了常见的算术运算，但这里不列出这些方法。
]

#block(sticky: true)[#raw("<<XYZ Public Methods>>=")] <fragment-XYZPublicMethods-0>
```cpp
XYZ(Float X, Float Y, Float Z) : X(X), Y(Y), Z(Z) {}
```

#block(sticky: true)[#raw("<<XYZ Public Members>>=")] <fragment-XYZPublicMembers-0>
```cpp
Float X = 0, Y = 0, Z = 0;
```
...
#block(sticky: true)[#raw("<<Spectrum Function Definitions>>=")] <fragment-SpectrumFunctionDefinitions-0>
```cpp
...
#parec[
  ...
  对于两个光谱的简单一维积分，蒙特卡洛方法不是必需的，因此 `InnerProduct()` 计算整数波长上的黎曼和。
]


$
  integral_(lambda_("min"))^(lambda_("max")) f (lambda) g (
    lambda
  ) thin d lambda approx sum_(lambda = lambda_("min"))^(lambda_("max")) f (lambda) g (lambda) .
$


#block(sticky: true)[#raw("<<Spectrum Inline Functions>>=")] <fragment-SpectrumInlineFunctions-0>
```cpp
...
#parec[
  It is also useful to be able to compute XYZ coefficients for a #link("https://pbr-book.org/4ed/Radiometry,_Spectra,_and_Color/Representing_Spectral_Distributions.html#SampledSpectrum")[SampledSpectrum]. Because #link("https://pbr-book.org/4ed/Radiometry,_Spectra,_and_Color/Representing_Spectral_Distributions.html#SampledSpectrum")[SampledSpectrum] only has point samples of the spectral distribution at predetermined wavelengths, they are found via a Monte Carlo estimate of @eqt:xyz-matching using the sampled spectral values $s_i$ at wavelengths $lambda_i$ and their associated PDFs:
][
  计算 `SampledSpectrum` 的XYZ系数也是有用的。`SampledSpectrum` 只保存光谱分布在预先确定波长处的点样本，因此要使用波长 $lambda_i$ 处的光谱采样值 $s_i$ 及其 PDF，对 @eqt:xyz-matching 作蒙特卡洛估计：
]
...
#parec[
  ...
  #link(<SampledSpectrum::ToXYZ>)[SampledSpectrum::ToXYZ()]计算此估计量的值。
]

#block(sticky: true)[#raw("<<Spectrum Method Definitions>>+=")] <fragment-SpectrumMethodDefinitions-2>
```cpp
...
#block(sticky: true)[#raw("<<Sample the X, Y, and Z matching curves at lambda>>=")] <fragment-SampletheXYandZmatchingcurvesatmonolambda-0>
```cpp
...
#parec[
  The summand in @eqt:xyz-mc is easily computed with values at hand. Here, we evaluate all terms of each sum with a single expression. Using #link("https://pbr-book.org/4ed/Radiometry,_Spectra,_and_Color/Representing_Spectral_Distributions.html#SampledSpectrum::SafeDiv")[`SampledSpectrum::SafeDiv()`] to divide by the PDF values handles the case of the PDF being equal to zero for some wavelengths, as can happen if #link("https://pbr-book.org/4ed/Radiometry,_Spectra,_and_Color/Representing_Spectral_Distributions.html#SampledWavelengths::TerminateSecondary")[`SampledWavelengths::TerminateSecondary()`] was called. Finally, #link("https://pbr-book.org/4ed/Radiometry,_Spectra,_and_Color/Representing_Spectral_Distributions.html#SampledSpectrum::Average")[`SampledSpectrum::Average()`] conveniently takes care of summing the individual terms and dividing by $n$ to compute the estimator's value for each coefficient.
][
  @eqt:xyz-mc 中的求和项可以通过现有的值轻松计算。在这里，我们使用一个表达式评估每个和的所有项。使用#link("https://pbr-book.org/4ed/Radiometry,_Spectra,_and_Color/Representing_Spectral_Distributions.html#SampledSpectrum::SafeDiv")[`SampledSpectrum::SafeDiv()`]来除以概率密度函数值处理了概率密度函数在某些波长上等于零的情况，这可能发生在调用#link("https://pbr-book.org/4ed/Radiometry,_Spectra,_and_Color/Representing_Spectral_Distributions.html#SampledWavelengths::TerminateSecondary")[`SampledWavelengths::TerminateSecondary()`]时。最后，#link("https://pbr-book.org/4ed/Radiometry,_Spectra,_and_Color/Representing_Spectral_Distributions.html#SampledSpectrum::Average")[`SampledSpectrum::Average()`]方便地处理了求和各个项并除以 $n$ 以计算每个系数的估计值。
]

#block(sticky: true)[#raw("<<Evaluate estimator to compute (x, y, z) coefficients>>=")] <fragment-Evaluateestimatortocomputexyzcoefficients-0>
```cpp
...
#parec[
  ...
  为了避免在只需要亮度时计算 $X$ 和 $Z$ 系数的开销，有一个`y()`方法只返回 $Y$。其实现就是 `XYZ()` 中相应的部分，故不在此列出。
]

==== #ez_caption[Chromaticity and xyY Color][色度与 xyY 颜色]
...
#parec[
  ...
  颜色可分为#emph[明度（lightness）]与#emph[彩度（chroma）]。前者描述相对于白色物体的明亮程度，后者描述相对于白色的色彩浓淡程度。一种量化彩度的方法是使用 #emph[$x y z$ 色度坐标]，其定义由 XYZ 颜色空间坐标给出：
]
...
#parec[
  Considering just $x$ and $y$, we can plot a #emph[chromaticity diagram] to visualize their values; see @fig:xy-chromaticity-diagram. Spectra with light at just a single wavelength—the pure spectral colors—lie along the curved part of the chromaticity diagram. This part corresponds to the $x y$ projection of the 3D XYZ curve that was shown in @fig:xyz-3d-curve. All the valid colors lie inside the upside-down horseshoe shape; points outside that region correspond to imaginary colors.
][
  仅考虑 $x$ 和 $y$，我们可以绘制一个#emph[色度图]来可视化它们的值；见@fig:xy-chromaticity-diagram。仅在单一波长上有光的光谱——纯光谱颜色——位于色度图的弯曲部分。这部分对应于@fig:xyz-3d-curve 中显示的3D XYZ曲线的 $x y$ 投影。所有有效的颜色都位于倒马蹄形的内部；该区域外的点对应于虚色。
]
...
#parec[
  The xyY color space separates a color's chromaticity from its lightness. It uses the $x$ and $y$ chromaticity coordinates and $y_lambda$ from XYZ, since the $Y (lambda)$ matching curve was defined to be proportional to luminance. `pbrt` makes limited use of xyY colors and therefore does not provide a class to represent them, but the #link(<XYZ>)[XYZ] class does provide a method that returns its $x y$ chromaticity coordinates as a #link("https://pbr-book.org/4ed/Geometry_and_Transformations/Points.html#Point2f")[Point2f].
][
  xyY 颜色空间将颜色的色度与明度分开。它使用 $x$ 和 $y$ 色度坐标以及来自XYZ的 $y_lambda$，因为 $Y (lambda)$ 匹配曲线被定义为与亮度成比例。`pbrt`对xyY颜色的使用有限，因此不提供表示它们的类，但#link(<XYZ>)[XYZ]类确实提供了一个方法，返回其 $x y$ 色度坐标作为#link("https://pbr-book.org/4ed/Geometry_and_Transformations/Points.html#Point2f")[Point2f]。
]

#block(sticky: true)[#raw("<<XYZ Public Methods>>+=")] <fragment-XYZPublicMethods-1>
```cpp
Point2f xy() const {
...
#parec[
  ...
  相应的逆转换方法根据给定的 $x y$ 坐标以及可选的 $y_lambda$，将 xyY 转换为 XYZ。
]

#block(sticky: true)[#raw("<<XYZ Public Methods>>+=")] <fragment-XYZPublicMethods-2>
```cpp
...
#parec[
  ...
  在渲染应用中，RGB颜色比XYZ更常用。在RGB颜色空间中，颜色由对应于红、绿、蓝三种颜色的三元组表示，通常称为#emph[RGB]。然而，单独的RGB三元组本身没有意义；它必须相对于特定的RGB颜色空间来定义。
]

#parec[
  To understand why, consider what happens when an RGB color is shown on a display: the spectrum that is displayed is given by the weighted sum of three spectral emission curves, one for each of red, green, and blue, as emitted by the display elements, be they phosphors, LED or LCD elements, or plasma cells.#footnote[This model is admittedly a simplification in that it neglects any additional processing the display does; in particular, many displays perform nonlinear remappings of the displayed values; this topic will be discussed in #link("https://pbr-book.org/4ed/Utilities/Images.html#sec:color-encodings")[Section B.5.6].] @fig:display-rgb-plots plots the red, green, and blue distributions emitted by an LCD display and an LED display; note that they are remarkably different. @fig:display-rgb-result in turn shows the spectral distributions that result from displaying the RGB color $(0.6 , 0.3 , 0.2)$ on those displays. Not surprisingly, the resulting spectra are quite different as well.
][
  为了理解原因，考虑当RGB颜色在显示器上显示时会发生什么：所显示的光谱是由三个光谱发射曲线的加权和给出的，分别对应于红、绿、蓝，由显示元素发出，无论是荧光粉、LED或LCD元素，还是等离子体单元。#footnote[这个模型作了简化，忽略了显示器的额外处理。特别是，许多显示器会对显示值进行非线性重映射；#link("https://pbr-book.org/4ed/Utilities/Images.html#sec:color-encodings")[B.5.6 节]将讨论这一点。] @fig:display-rgb-plots 绘制了LCD显示器和LED显示器发出的红、绿、蓝分布；注意它们有显著的不同。@fig:display-rgb-result 则显示了在这些显示器上显示RGB颜色 $(0.6 , 0.3 , 0.2)$ 所显示的光谱分布。不出所料，结果光谱也有很大不同。
]
...
#ez_caption[
  Red, Green, and Blue Emission Curves for an LCD Display and an LED Display. The first plot shows the curves for an LCD display, and the second shows them for an LED. These two displays have quite different emission profiles. (Data courtesy of X-Rite, Inc.)
][
  LCD 显示器与 LED 显示器的红、绿、蓝发射曲线。第一幅图对应 LCD，第二幅对应 LED。两种显示器的发射特性有很大差异。（数据由 X-Rite, Inc. 提供。）
]
...
#ez_caption[
  Spectral Distributions from Displaying the RGB Color $(0.6, 0.3, 0.2)$ on LED (red) and LCD (blue) Displays. The resulting emitted distributions are remarkably different, even given the same RGB values, due to the different emission curves illustrated in @fig:display-rgb-plots .
][
  LED（红线）与 LCD（蓝线）显示器显示 RGB 颜色 $(0.6, 0.3, 0.2)$ 时的光谱分布。由于 @fig:display-rgb-plots 所示的发射曲线不同，即使 RGB 值相同，发出的光谱分布也有显著差异。
]
...
#parec[
  ...
  如果已知显示器的 $R(lambda)$ 、 $G(lambda)$ 和 $B(lambda)$ 曲线，则可以通过对光谱分布 $S(lambda)$ 与各曲线的乘积积分，求得在该显示器上显示光谱分布的RGB系数：
]
...
#parec[
  ...
  `pbrt`还提供了一个`RGB`类，它采用直接的表示方式，并提供多种实用的算术运算（本文中未详细列出）。
]


#block(sticky: true)[#raw("<<RGB Definition>>=")] <fragment-RGBDefinition-0>
```cpp
class RGB {
  public:
    <<RGB Public Methods>>
...
#block(sticky: true)[#raw("<<RGB Public Methods>>=")] <fragment-RGBPublicMethods-0>
```cpp
RGB(Float r, Float g, Float b) : r(r), g(g), b(b) {}
```

#block(sticky: true)[#raw("<<RGB Public Members>>=")] <fragment-RGBPublicMembers-0>
```cpp
Float r = 0, g = 0, b = 0;
```

=== #ez_caption[RGB Color Spaces][RGB 颜色空间]
<rgb-color-spaces>
#parec[
...
#block(sticky: true)[#raw("<<RGBColorSpace Definition>>=")] <fragment-RGBColorSpaceDefinition-0>
```cpp
class RGBColorSpace {
  public:
...
the size of the gamut, though we will assume conventional RGB here.]
  ...
  RGB颜色空间是使用红、绿、蓝颜色基色的色度来定义的。基色定义了颜色空间的#emph[色域]，即它可以用RGB值在0到1之间表示的颜色集合。对于三个基色，色域在色度图上形成一个三角形，每个基色的色度定义了一个顶点。#footnote[一些显示器使用超过三种基色来扩大色域范围，不过这里假设使用常规的 RGB。]
]
...
#parec[
  ...
  除了基色外，还需要指定颜色空间的#emph[白点]，即当三个基色都激活到其最大发射时显示的颜色。这可能会令人惊讶，因为白色难道不应该是在每个波长上具有相同值的光谱分布吗？然而，白色是一种颜色，作为一种颜色，它是人类#emph[感知]为均匀并标记为“白色”的颜色。 白色光谱通常在蓝、绿对应的短波长处具有较高功率，在橙、红对应的长波长处则较低。D65 照明体，是在 @standard-illuminants 中描述并在 @fig:d-illuminant 中绘制的，是指定颜色空间白点的常见选择。
]
...
#parec[
  ...
  保存照明体光谱后，渲染器用户便可用 RGB 颜色指定光源的发射；所提供的照明体定义了 RGB 白色 $(1, 1, 1)$ 对应的光谱分布。
]
#block(sticky: true)[#raw("<<RGBColorSpace Method Definitions>>=")] <fragment-RGBColorSpaceMethodDefinitions-0>
```cpp
...
#parec[
  ...
  `RGBColorSpace` 用 `DenselySampledSpectrum` 表示照明体，以便高效地按波长查询。
]

#block(sticky: true)[#raw("<<RGBColorSpace Public Members>>=")] <fragment-RGBColorSpacePublicMembers-0>
```cpp
Point2f r, g, b, w;
...
#parec[
  `RGBColorSpace`s also store a pointer to an `RGBToSpectrumTable` class that stores information related to converting RGB values in the color space to full spectral distributions; it will be introduced shortly, in @from-rgb-to-specturm .
][
  `RGBColorSpace` 还保存一个指向 `RGBToSpectrumTable` 的指针。这个类存储将该颜色空间中的 RGB 值转换为完整光谱分布所需的信息，将在 @from-rgb-to-specturm 中介绍。
]

#block(sticky: true)[#raw("<<RGBColorSpace Private Members>>=")] <fragment-RGBColorSpacePrivateMembers-0>
```cpp
...
#parec[
  ...
  为了求出该颜色空间中的 RGB 值，能够在它与 XYZ 之间转换很有用。这可以用 $3 times 3$ 矩阵实现。计算这些矩阵需要各基色的色度及白点所对应的 XYZ 坐标。
]

#block(sticky: true)[#raw("<<Compute whitepoint primaries and XYZ coordinates>>=")] <fragment-ComputewhitepointprimariesandXYZcoordinates-0>
```cpp
...
#parec[
  We will first derive the matrix $M$ that transforms from RGB coefficients in the color space to XYZ:
][
  首先推导将该颜色空间中的 RGB 系数转换为 XYZ 的矩阵 $M$：
]
...
$
#parec[
  This matrix can be found by considering the relationship between the RGB triplet $(1 , 1 , 1)$ and the whitepoint in XYZ coordinates, which is available in `W`. In this case, we know that $w_(x_lambda)$ must be proportional to the sum of the $x_lambda$ coordinates of the red, green, and blue primaries, since we are considering the case of a $(1 , 1 , 1)$ RGB. The same follows for $y_lambda$ and $z_lambda$. This relationship can be expressed as
][
  这个矩阵可以通过考虑RGB三元组 $(1 , 1 , 1)$ 与XYZ坐标中的白点之间的关系来找到，这在`W`中可用。在这种情况下，我们知道 $w_(x_lambda)$ 必须与红、绿、蓝基色的 $x_lambda$ 坐标之和成比例，因为这里考虑的是 RGB 为 $(1, 1, 1)$ 的情形。$y_lambda$ 与 $z_lambda$ 也有同样的关系，可写为
]
...
#parec[
  ...
  其中只有未知数 $c_r$ 、 $c_g$ 和 $c_b$。可以通过将白点的 XYZ 坐标乘以剩余矩阵的逆矩阵来求得。对转换矩阵求逆即可得到从 XYZ 到 RGB 的矩阵。
]
#block(sticky: true)[#raw("<<Initialize XYZ color space conversion matrices>>=")] <fragment-InitializeXYZcolorspaceconversionmatrices-0>
```cpp
...
#block(sticky: true)[#raw("<<RGBColorSpace Public Members>>+=")] <fragment-RGBColorSpacePublicMembers-1>
```cpp
...
#parec[
  ...
  给定一个颜色空间的 XYZ/RGB 转换矩阵，矩阵-向量乘法足以将任何 XYZ 三元组转换为该颜色空间，并将该颜色空间中的任何 RGB 转换为 XYZ。
]

#block(sticky: true)[#raw("<<RGBColorSpace Public Methods>>=")] <fragment-RGBColorSpacePublicMethods-0>
```cpp
...
#parec[
  ...
  此外，通过使用这些矩阵进行 XYZ 颜色转换，可以很容易地计算出从一个颜色空间转换到另一个颜色空间的矩阵。
]

#block(sticky: true)[#raw("<<RGBColorSpace Method Definitions>>+=")] <fragment-RGBColorSpaceMethodDefinitions-1>
```cpp
...
#parec[
  #link("https://pbr-book.org/4ed/Radiometry,_Spectra,_and_Color/Representing_Spectral_Distributions.html#SampledSpectrum")[`SampledSpectrum`] provides a convenience method that converts to RGB in a given color space, again via XYZ.
][
  #link("https://pbr-book.org/4ed/Radiometry,_Spectra,_and_Color/Representing_Spectral_Distributions.html#SampledSpectrum")[`SampledSpectrum`] 提供了一个便捷方法，通过 XYZ 将采样光谱转换为给定颜色空间中的 RGB。
]

#block(sticky: true)[#raw("<<Spectrum Method Definitions>>+=")] <fragment-SpectrumMethodDefinitions-3>
```cpp
...
==== #ez_caption[Standard Color Spaces][标准颜色空间]
...
#parec[
  ...
  有许多广泛使用的标准颜色空间，`pbrt` 包含对它们的内置支持。例如：
]
...
#parec[
  ...
  - ACES2065-1，其基色位于可表示颜色的范围之外，位置经过选择，使其能够表示所有颜色。选择这种方式的一个原因是它适合作为长期存档存储的格式。
]

#parec[
  The gamuts of each are shown in @fig:color-space-gamuts .
][
  每个颜色空间的色域如@fig:color-space-gamuts 所示。
]
#figure(
  ...
  caption: [
    #ez_caption[
      ...
      sRGB、DCI-P3、Rec2020 和 ACES2065-1 颜色空间的色域，使用色度图可视化。sRGB 覆盖的色域最小，DCI-P3 次之，Rec2020 更大。ACES2065-1 对应于大三角形，其特点是使用对应于虚色的基色。因此，它能够表示所有有效颜色，而其他颜色空间则无法做到。
    ]
  ],
)<color-space-gamuts>
...
#parec[
  ...
  `RGBColorSpace` 类为上述每一种颜色空间提供了预初始化的 #link(<RGBColorSpace>)[`RGBColorSpace`] 实例。
]

#block(sticky: true)[#raw("<<RGBColorSpace Public Members>>+=")] <fragment-RGBColorSpacePublicMembers-2>
```cpp
...
#parec[
  ...
  还可以通过名称或通过指定基色的色度和白点来查找特定的颜色空间。
]

#block(sticky: true)[#raw("<<RGBColorSpace Public Methods>>+=")] <fragment-RGBColorSpacePublicMethods-1>
```cpp
...
=== #ez_caption[Why Spectral Rendering?][为何采用光谱渲染？]
...
#parec[
  ...
  到目前为止，我们在描述 `pbrt` 的实现时，一直假设它使用点采样光谱来表示光谱量。虽然鉴于 `pbrt` 的物理基础和对蒙特卡洛积分的普遍采用，这似乎是理所当然的，但这与当前广泛使用 RGB 颜色进行渲染中的光谱计算的实践并不一致。 在本节开始时，我们提到了这种做法的一个重大问题；在介绍了 RGB 颜色空间后，我们现在可以更深入地探讨。
]
...
#parec[
  ...
  其中 RGB 颜色的逐分量乘法由“$dot.circle$”运算符表示。
]
...
#parec[
  ...
  现在假设我们切换到广色域颜色空间 ACES2065-1。在这个颜色空间中，sRGB 颜色 $(0 , 1 , 0)$ 可以表示为 $(0.38 , 0.82 , 0.12)$ ——因此它映射到可表示颜色范围的内部。 执行相同的逐分量乘法得到结果：
]
...
#parec[
  ...
  这次，结果颜色的强度低于原来，并且由于绿色光的相对比例增加而变得更加饱和。 这导致了@fig:green-in-two-color-spaces 中所示的有些奇怪的情况：在这个新颜色空间中进行的逐分量乘法不仅产生了不同的颜色——还增加了饱和度，以至于颜色被推到了 CIE 马蹄形色域之外！
]

#figure(
  image("../pbr-book-website/4ed/Radiometry,_Spectra,_and_Color/pha04f24.svg"),
  caption: [
    #ez_caption[The same color can have very different RGB values when expressed in RGB color spaces with differently shaped gamuts. The green primary $(0, 1, 0)$ in the sRGB color gamut (inner triangle) has chromaticity coordinates $(0.3, 0.6)$ (white dot). In the wide-gamut ACES2065-1 color space (outer triangle), the same color has the RGB value $(0.38, 0.82, 0.12)$.][在具有不同形状的色域的 RGB 颜色空间中，相同的颜色可以具有非常不同的 RGB 值。在 sRGB 色域（内部三角形）中，绿色基色 $(0, 1, 0)$ 的色度坐标为 $(0.3, 0.6)$（白色圆点）。在宽色域的 ACES2065-1 颜色空间（外部三角形）中，相同的颜色具有 RGB 值 $(0.38, 0.82, 0.12)$。]
  ],
...
#parec[
  ...
  在渲染中，计算材质与光源的相互作用时，能够将光谱值相乘至关重要。同时，这个例子也说明了用 RGB 值进行这种计算的问题：这种乘法在某种意义上是任意的，因为它的结果高度依赖于所选的颜色空间。因此，即使最终需要输出 RGB 图像，像 `pbrt` 这样采用光谱模型进行渲染仍然更合适。
]
...
#parec[
  ...
  使用光谱表示进行渲染还有其他好处：它便于模拟色散；此外，为了描述薄层中的虹彩或表面微结构产生的衍射，高级反射模型往往本来就依赖于波长。
]

=== #ez_caption[Choosing the Number of Wavelength Samples][选择波长样本数]
<choosing-the-number-of-wavelength-samples>
#parec[
  Even though it uses a spectral model for light transport simulation, `pbrt`'s output is generally an image in a tristimulus color representation like RGB. Having described how those colors are computed—Monte Carlo estimates of the products of spectra and matching functions of the form of @eqt:xyz-mc —we will briefly return to the question of how many spectral samples are used for the #link("https://pbr-book.org/4ed/Radiometry,_Spectra,_and_Color/Representing_Spectral_Distributions.html#SampledSpectrum")[SampledSpectrum] class. The associated Monte Carlo estimators are easy to evaluate, but error in them leads to #emph[color noise] in images. @fig:color-noise-example shows an example of this phenomenon.
][
  尽管 `pbrt` 使用光谱模型进行光传输模拟，其输出通常是三刺激颜色表示的图像，如 RGB。我们已经描述了这些颜色是如何计算的——光谱和匹配函数的乘积的蒙特卡洛估计，如@eqt:xyz-mc 的形式——我们将简要讨论 #link("https://pbr-book.org/4ed/Radiometry,_Spectra,_and_Color/Representing_Spectral_Distributions.html#SampledSpectrum")[SampledSpectrum] 类使用多少光谱样本的问题。相关的蒙特卡洛估计量易于评估，但其中的误差会导致图像中的 #emph[色彩噪声]。@fig:color-noise-example 展示了这种现象的一个例子。
]
...
#figure(
  ...
  caption: [
    #ez_caption[
      ...
      (a) 示例场景的参考图像。
      (b)
      如果场景仅使用每像素一个图像样本渲染，每次采样仅一个波长，则蒙特卡洛估计的像素
      RGB 颜色的误差会导致显著的方差。
      (c) 使用四个波长样本（`pbrt`
      的默认值）时，这种方差大大减少，尽管色彩噪声仍然明显。在实践中，四个波长样本通常足够，因为通常会在每个像素处取多个图像样本。
...
#parec[
  ...
  @fig:color-noise-example(a) 显示了一个由点光源照明的场景，其中仅包含来自光源的直接照明。在这种简单设置中，散射光的蒙特卡洛估计量在所有波长上的方差为零，因此蒙特卡洛误差的唯一来源是颜色匹配函数的积分。每个像素只有一条光线路径，每条路径仅跟踪一个波长，图像噪声很大，如@fig:color-noise-example(b) 所示。直观地说，这种情况下的挑战可以理解为渲染器试图在每个像素处估计三个值——红色、绿色和蓝色——全部来自单个波长的光谱值。
]
...
#parec[
  ...
  增加像素样本的数量可以减少这种误差（只要它们采样不同的波长），尽管将多个波长样本与每条光线关联更为有效。光线在场景中经过的路径通常与波长无关，而在多个波长上计算照明的增量成本通常与找到光线交点和计算其他与波长无关的量相比较小。（为每条光线考虑多个波长可以看作是蒙特卡洛分裂技术的应用，该技术在@mc-splitting 中描述。）@fig:color-noise-example(c) 显示了将四个波长与每条光线关联所带来的改进；色彩噪声大大减少。
]
...
#parec[
  However, computing scattering from too many wavelengths with each ray can harm efficiency due to the increased computation required to compute spectral quantities. To investigate this trade-off, we rendered the scene from @fig:color-noise-example with a variety of numbers of wavelength samples, both with wavelengths sampled independently and with stratified sampling of wavelengths. (For both, wavelengths were sampled uniformly over the range 360–830 nm.#footnote[In Section 5.4.2, we will see that nonuniform sampling of wavelengths is beneficial but will stick to uniform sampling here for simplicity.]) @fig:mc-efficiency-vs-wavelength-samples-simple-scene shows the results.
][
  然而，使用每条光线计算过多波长的散射会因计算光谱量所需的增加而损害效率。为了研究这种权衡，我们使用各种波长样本数量渲染了@fig:color-noise-example 中的场景，包括独立采样波长和分层采样波长。（对于两者，波长在 360–830 nm 范围内均匀采样。#footnote[5.4.2 节将说明波长非均匀采样的益处；这里为简便起见仍采用均匀采样。]）@fig:mc-efficiency-vs-wavelength-samples-simple-scene 显示了渲染时间和误差的结果。
]
...
#parec[
  @fig:mc-efficiency-vs-wavelength-samples-simple-scene(c) plots Monte Carlo efficiency for both approaches (note, with a logarithmic scale for the $y$ axis). The result seems clear; 32 stratified wavelength samples are over a million times more efficient than one sample and there the curve has not yet leveled off. Why stop measuring at 32, and why is `pbrt` stuck with a default of four wavelength samples for its #link("https://pbr-book.org/4ed/Radiometry,_Spectra,_and_Color/Representing_Spectral_Distributions.html#NSpectrumSamples")[NSpectrumSamples] parameter?
][
  @fig:mc-efficiency-vs-wavelength-samples-simple-scene(c) 绘制了两种方法的蒙特卡洛效率（注意， $y$ 轴为对数刻度）。结果似乎很明确；32 个分层波长样本的蒙特卡洛效率超过单个样本的一百万倍，曲线尚未趋于平稳。为什么在 32 个样本时停止测量，而 `pbrt` 的 #link("https://pbr-book.org/4ed/Radiometry,_Spectra,_and_Color/Representing_Spectral_Distributions.html#NSpectrumSamples")[NSpectrumSamples] 参数默认为四个波长样本？
]
...
#figure(
  ...
  caption: [
    #ez_caption[
      ...
      (a)
      一个更复杂的场景，其中蒙特卡洛估计量的方差来自波长采样以外的多种来源。
      (b)
      均方误差与分层波长样本数量的关系图。波长样本超过六个后，继续增加的收益有限。
      (c)
      蒙特卡洛效率与分层波长样本数量的关系，归一化为一个波长样本的效率。对于这个场景，八个样本是最佳的。
    ]
  ],
...
#parec[
  ...
  当前设置的原因主要有三个。首先，尽管@fig:mc-efficiency-vs-wavelength-samples-simple-scene(a) 显示从 8 到 32 个波长样本的误差减少了近 $500 upright(" 倍")$，但两幅图像几乎无法区分——由于显示技术和人类视觉系统的限制，误差的差异无关紧要。其次，场景通常在每个像素中遵循多条光线路径进行渲染，以减少其他蒙特卡洛估计量的误差。随着在更少波长下采集更多像素样本，贡献到每个像素值的波长总数增加。
]
...
#parec[
  ...
  最后，也是最重要的，其他蒙特卡洛误差来源通常对总体误差的贡献比波长采样更大。@fig:mc-efficiency-vs-wavelength-samples-watercolor(a) 显示了一个更复杂的场景，其中使用蒙特卡洛采样的具有挑战性的照明。均方误差作为波长样本数量的函数的图表如@fig:mc-efficiency-vs-wavelength-samples-watercolor(b) 所示，蒙特卡洛效率如@fig:mc-efficiency-vs-wavelength-samples-watercolor(c) 所示。显然，在八个波长样本之后，更多样本的增量成本并不有利。
]

=== #ez_caption[From RGB to Spectra][从 RGB 转换为光谱]
...
#parec[
  ...
  尽管将光谱转换为 RGB 以进行图像输出是一个明确的操作，但将 RGB 颜色转换为光谱分布却并非如此。这是一项重要任务，因为渲染器的大部分输入通常是以 RGB 颜色的形式出现的。 在当前的 3D 建模工具中创建的场景通常使用 RGB 参数和纹理来指定对象的反射属性和光源的发射。在光谱渲染器中，这些 RGB 值必须以某种方式转换为等效的颜色光谱，但不幸的是，由于存在同色异谱现象，任何此类转换本质上都是模糊的。 如果问题定义得如此不明确，我们如何期望找到一个合理的解决方案？另一方面，这种模糊性也可以被视为积极的：它留下了一个包含简单且高效技术的广阔的解空间。
]
...
#figure(
  ...
  caption: [
    #ez_caption[Spectral reflectances of several color checker patches. Each curve is shaded with the associated RGB color.][色卡上几个色块的光谱反射率。每条曲线以对应的 RGB 颜色着色。]
  ],
) <color-checker-spectra>
#parec[
...
#parec[
  ...
  - _照明体光谱_，描述光源发射随波长的变化。这些是非负且无界的；它们的形状从平滑（白炽光源、LED）到极其尖锐（激光中的受激发射或氙弧灯和荧光灯中的气体放电）。
]

#parec[
  - _Reflectance spectra_, which describe reflection from absorbing surfaces. Reflectance spectra conserve the amount of energy at each wavelength, meaning that values cannot be outside of the \[0, 1\] range. They are typically smooth functions in the visible wavelength range.#footnote[Note that this observation does not always hold: in the ultraviolet and infrared wavelength range, absorption bands cause sharp spikes in reflectance spectra. Furthermore, wave-optical effects such as iridescence produce oscillatory spectral variation. These behaviors could likely be handled using specialized conversion techniques, which are beyond the scope of the simple approach discussed here.]

(@fig:color-checker-spectra shows a few examples of reflectance spectra from a color checker.)
][
  - _反射率光谱_，描述吸收性表面的反射。反射率光谱必须在每个波长上满足能量守恒，这意味着值不能超出\[0, 1\] 范围。它们通常是可见波长范围内的平滑函数。#footnote[这一观察并非总成立：在紫外和红外波段，吸收带会在反射率光谱中形成尖锐的峰。此外，虹彩等波动光学效应会产生振荡的光谱变化。这些现象或许可通过专门的转换方法处理，但超出了这里介绍的简单方法的范围。]

（@fig:color-checker-spectra 给出了色卡上几个反射率光谱的例子。）
]
...
#parec[
  ...
  本节首先介绍了一种将 RGB 颜色值（其分量在 0 和 1 之间）转换为相应反射率光谱的方法，然后推广到无界光谱和照明体光谱。 该转换利用问题的模糊性来实现以下目标：
]
...
#parec[
  ...
  - _光谱平滑性_：受之前关于真实世界反射率光谱的观察启发，输出光谱应尽可能平滑。另一种平滑性也很重要：输入 RGB 颜色的轻微扰动应导致输出光谱的相应小变化。 不连续性是不受欢迎的，因为在不同照明体下观察时，它们会导致纹理对象上出现可见的接缝。
]
...
#parec[
  ...
  尽管真实世界的反射率光谱存在多种形状，但它们通常可以通过恒定（白色、黑色）、近似线性或具有单峰（绿色、黄色）或双峰（蓝紫色）的曲线很好地近似。
]
...
#parec[
  ...
  这里选择的方法尝试使用一个设计为简单、平滑且在运行时高效评估的函数族来表示这些光谱，同时提供足够的自由度以精确再现任意 RGB 颜色值。
]
...
#parec[
  ...
  多项式通常是此类构造中的标准构建块；实际上，二次多项式可以表示恒定和线性曲线，以及在波长范围中间或端点附近达到峰值的曲线。 然而，它们缺乏能量守恒性，这个问题我们通过使用 S 形函数来解决：
]
...
#parec[
  ...
  我们将此 sigmoid 应用于由三个系数 $c_i$ 定义的二次多项式，将其输出压缩到区间 $[0, 1]$ 以确保能量守恒。
]
...
#parec[
  ...
  使用这种表示法表示理想的吸收和反射率光谱（即 $S(lambda) = 0$ 或 1）有些尴尬，因为多项式必须取值为正无穷或负无穷才能达到这两个极限。 这反过来导致@eqt:rgb-to-spectrum-polynomial 中的形式为 $plus.minus oo \/ oo$ 的分数，在 IEEE-754 算术中得到非数值（NaN）。 我们需要单独处理这种极限情况。
]

#parec[
  We begin with the definition of a class that encapsulates the coefficients $c_i$ and evaluates @eqt:rgb-to-spectrum-spd.
][
  我们从封装系数 $c_i$ 并评估@eqt:rgb-to-spectrum-spd 的类定义开始。
]

#block(sticky: true)[#raw("<<RGBSigmoidPolynomial Definition>>=")] <fragment-RGBSigmoidPolynomialDefinition-0>
```cpp
...
#block(sticky: true)[#raw("<<RGBSigmoidPolynomial Public Methods>>=")] <fragment-RGBSigmoidPolynomialPublicMethods-0>
```cpp
RGBSigmoidPolynomial(Float c0, Float c1, Float c2)
    : c0(c0), c1(c1), c2(c2) {}
```

#block(sticky: true)[#raw("<<RGBSigmoidPolynomial Private Members>>=")] <fragment-RGBSigmoidPolynomialPrivateMembers-0>
```cpp
Float c0, c1, c2;
```
...
#block(sticky: true)[#raw("<<RGBSigmoidPolynomial Public Methods>>+=")] <fragment-RGBSigmoidPolynomialPublicMethods-1>
```cpp
...
#block(sticky: true)[#raw("<<RGBSigmoidPolynomial Private Methods>>=")] <fragment-RGBSigmoidPolynomialPrivateMethods-0>
```cpp
...
#parec[
  ...
  我们保守地检查区间的端点以及通过将多项式的导数设为零并求解波长 $lambda$ 得到的极值。 如果它碰巧是局部最小值，则该值将被忽略。
]

#block(sticky: true)[#raw("<<RGBSigmoidPolynomial Public Methods>>+=")] <fragment-RGBSigmoidPolynomialPublicMethods-2>
```cpp
...
#parec[
  where $R(lambda), G(lambda), B(lambda)$ describe emission curves of the color primaries and $W(lambda)$ represents the whitepoint (e.g., D65 shown in Figure #link("https://pbr-book.org/4ed/Radiometry,_Spectra,_and_Color/Light_Emission.html#fig:d-illuminant")[4.14] in the case of the sRGB color space). Including the whitepoint in this optimization problem ensures that monochromatic RGB values map to uniform reflectance spectra.
][
  其中 $R (lambda) , G (lambda) , B (lambda)$ 描述了颜色基色的发射曲线， $W (lambda)$ 表示白点（例如，在 sRGB 颜色空间中如图 #link("https://pbr-book.org/4ed/Radiometry,_Spectra,_and_Color/Light_Emission.html#fig:d-illuminant")[4.14] 所示的 D65）。在此优化问题中包含白点确保三个分量相等的 RGB 值映射到均匀的反射率光谱。
]

#parec[
  In spaces with a relatively compact gamut like sRGB, this optimization can achieve zero error regardless of the method used to quantify color distances. In larger color spaces, particularly those including imaginary colors like ACES2065-1, zero round-trip error is clearly not achievable, and the choice of norm $norm(dot.op)$ becomes relevant. In principle, we could simply use the 2-norm—however, a problem with such a basic choice is that it is not #emph[perceptually uniform]: whether a given amount of error is actually visible depends on its position within the RGB cube. We instead use CIE76 $Delta E$, which first transforms both colors into a color space known as CIELAB before evaluating the $L_2$-distance.
][
  在 sRGB 这样相对紧凑的色域中，无论使用哪种方法量化颜色距离，这种优化都可以实现零误差。在较大的颜色空间中，特别是那些包含虚色的空间，如 ACES2065-1，显然无法实现零往返误差，选择范数 $norm(dot.op)$ 变得重要。 原则上，我们可以简单地使用 2-范数（即欧几里得范数）——然而，这样一个基本选择的问题在于它不是 #emph[感知均匀的]：给定的误差量是否实际可见取决于其在 RGB 立方体中的位置。我们选择使用 CIE76 $Delta E$，它首先将两种颜色转换为称为 CIELAB 的颜色空间，然后评估 $L_2$ -距离。
]
...
#figure(
  ...
  caption: [
    #ez_caption[
      ...
      从RGB值计算的光谱。反射率光谱的图表由RGBSigmoidPolynomial表示，针对以下RGB颜色：$(0.7, 0.5, 0.8)$（紫色线）、$(0.25, 0.44, 0.33)$（绿色线）和$(0.36, 0.275, 0.21)$（棕色线）。每条线的颜色与其对应的RGB颜色相同。
    ]
  ],
) <rgb-albedo-spectrum>
#parec[
  ...
  生成的表存储在 `pbrt` 二进制文件中。在系统启动时，为每个 RGB 颜色空间创建一个 `RGBToSpectrumTable`。
]

#block(sticky: true)[#raw("<<RGBToSpectrumTable Definition>>=")] <fragment-RGBToSpectrumTableDefinition-0>
```cpp
...
#block(sticky: true)[#raw("<<RGBToSpectrumTable Method Definitions>>=")] <fragment-RGBToSpectrumTableMethodDefinitions-0>
```cpp
...
#parec[
  If the three RGB values are equal, it is useful to ensure that the returned spectrum is exactly constant. (In some cases, a slight color shift may otherwise be evident if interpolated values from the coefficient tables are used.) A constant spectrum results if $c_0 = c_1 = 0$ in @eqt:rgb-to-spectrum-spd and the appropriate value of $c_2$ can be found by inverting the sigmoid function.
][
  如果三个 RGB 值相等，确保返回的光谱是完全恒定的是有用的。（在某些情况下，如果使用系数表中的插值值，可能会明显出现轻微的颜色偏移。）如果在@eqt:rgb-to-spectrum-spd 中 $c_0 = c_1 = 0$，则结果为恒定光谱，并且可以通过求 sigmoid 函数的反函数找到适当的 $c_2$ 值。
]

#block(sticky: true)[#raw("<<Handle uniform rgb values>>=")] <fragment-Handleuniformmonorgbvalues-0>
```cpp
...
#figure(
  ...
  caption: [#ez_caption[
      Plots of Spectrum Polynomial Coefficients $c_i$. These plots show the polynomial
      coefficients for the corresponding xy chromaticities in the sRGB color
      space. Each of (a)  $c_0$, (b)  $c_1$, and (c)  $c_2$ mostly vary smoothly,
      ...
      光谱多项式系数 $c_i$ 的图示。这些图显示了 sRGB
      颜色空间中对应 $x y$ 色度的多项式系数。每个 (a) $c_0$，(b) $c_1$ 和
      (c) $c_2$ 大多变化平滑，尽管它们表现出急剧的过渡。(d)
      根据红色、绿色或蓝色哪个具有最大幅度来划分色域，与这些过渡紧密对应；因此系数在这三个区域中独立表格化。
    ]],
) <rgb-spectrum-ci-plots>


#parec[
  The coefficients $c_i$ from the optimization are generally smoothly varying; small changes in RGB generally lead to small changes in their values. (This property also ends up being helpful for the smoothness goal.) However, there are a few regions of the RGB space where they change rapidly, which makes direct 3D tabularization of them prone to error in those regions—see @fig:rgb-spectrum-ci-plots(a), (b), and (c). A better approach is to tabularize them independently based on which of the red, green, or blue RGB coefficients has the largest magnitude. This partitioning matches the coefficient discontinuities well, as is shown in @fig:rgb-spectrum-ci-plots(d).
  ...
  优化中的系数 $c_i$ 通常是平滑变化的；RGB的微小变化通常会导致它们值的微小变化。（这一特性对于平滑性目标也很有帮助。）然而，在RGB空间的某些区域，它们的变化速度较快，这使得直接对这些区域进行3D表格化容易产生误差——见@fig:rgb-spectrum-ci-plots(a)、(b)和(c)。一种更好的方法是根据红色、绿色或蓝色RGB系数中哪个具有最大的幅度来分别对它们进行表格化。这种划分很好地匹配了系数的不连续性，如@fig:rgb-spectrum-ci-plots(d)所示。

  这三个分区内各自仍需解决三维制表问题。我们将使用红色分量 $r$ 具有最大幅度的划分来解释表格的索引方式。对于给定的 $(r,g,b)$，第一步是计算一个重新归一化的坐标。
]
$ (x , y , z) = (g / r , b / r , r) . $
#parec[
...
#block(sticky: true)[#raw("<<Find maximum component and compute remapped component values>>=")] <fragment-Findmaximumcomponentandcomputeremappedcomponentvalues-0>
```cpp
...
#block(sticky: true)[#raw("<<RGBToSpectrumTable Public Constants>>=")] <fragment-RGBToSpectrumTablePublicConstants-0>
```cpp
...
#block(sticky: true)[#raw("<<RGBToSpectrumTable Private Members>>=")] <fragment-RGBToSpectrumTablePrivateMembers-0>
```cpp
const float *zNodes;
```
...
#block(sticky: true)[#raw("<<Compute integer indices and offsets for coefficient interpolation>>=")] <fragment-Computeintegerindicesandoffsetsforcoefficientinterpolation-0>
```cpp
...
#parec[
  ...
  现在可以实现代码片段，对查找点 $(x, y, z)$ 周围的八个系数样本进行三线性插值。系数表的索引细节由 `co` lambda 函数处理，我们将在描述内存中表的布局后不久定义它。注意，尽管对 $z$ 坐标应用了非线性映射，我们在 $z$ 中的系数样本之间仍然进行线性插值。实际上，这样做的误差是很小的。
]

#block(sticky: true)[#raw("<<Trilinearly interpolate sigmoid polynomial coefficients c>>=")] <fragment-Trilinearlyinterpolatesigmoidpolynomialcoefficientsmonoc-0>
```cpp
...
#block(sticky: true)[#raw("<<RGBToSpectrumTable Public Constants>>+=")] <fragment-RGBToSpectrumTablePublicConstants-1>
```cpp
using CoefficientArray = float[3][res][res][res][3];
```

#block(sticky: true)[#raw("<<RGBToSpectrumTable Private Members>>+=")] <fragment-RGBToSpectrumTablePrivateMembers-1>
```cpp
...
#parec[
  ...
  系数查找 lambda 函数只需为数组的各个维度使用正确的索引，并将传入的整数偏移分别加到 $x$、$y$、$z$ 的索引上。
]

#block(sticky: true)[#raw("<<Define co lambda for looking up sigmoid polynomial coefficients>>=")] <fragment-Definemonocolambdaforlookingupsigmoidpolynomialcoefficients-0>
```cpp
...
#block(sticky: true)[#raw("<<RGBColorSpace Method Definitions>>+=")] <fragment-RGBColorSpaceMethodDefinitions-2>
```cpp
...
#block(sticky: true)[#raw("<<Spectrum Definitions>>+=")] <fragment-SpectrumDefinitions-4>
```cpp
...
#block(sticky: true)[#raw("<<Spectrum Method Definitions>>+=")] <fragment-SpectrumMethodDefinitions-4>
```cpp
...
#block(sticky: true)[#raw("<<RGBAlbedoSpectrum Private Members>>=")] <fragment-RGBAlbedoSpectrumPrivateMembers-0>
```cpp
...
#parec[
  Implementation of the required `Spectrum` methods is a matter of forwarding the requests on to the appropriate `RGBSigmoidPolynomial` methods. As with most `Spectrum` implementations, we will not include the `Sample()` method here since it just loops over the wavelengths and evaluates @eqt:rgb-to-spectrum-spd at each one.
][
  所需的 `Spectrum` 方法的实现是将请求转发到适当的 `RGBSigmoidPolynomial` 方法。与大多数 `Spectrum` 实现一样，我们不会在此处包含 `Sample()` 方法，因为它只是遍历波长并在每个波长处评估@eqt:rgb-to-spectrum-spd。
]

#block(sticky: true)[#raw("<<RGBAlbedoSpectrum Public Methods>>=")] <fragment-RGBAlbedoSpectrumPublicMethods-0>
```cpp
...
==== #ez_caption[Unbounded RGB][无界 RGB]
<unbounded-rgb>
#parec[
  ...
  对于无界（正值）RGB 值，仍然可以使用`RGBSigmoidPolynomial`基础——只需添加一个缩放因子，将其范围重新映射到给定RGB的必要范围即可。这种方法在`RGBUnboundedSpectrum`类中实现。
]

#block(sticky: true)[#raw("<<Spectrum Definitions>>+=")] <fragment-SpectrumDefinitions-5>
```cpp
...
#figure(
  ...
  caption: [
    #ez_caption[With the sigmoid polynomial representation, highly
      ...
      使用 S 形函数与多项式的组合表示，高饱和度颜色可能在其光谱中出现意想不到的特征。这里我们绘制了`RGBAlbedoSpectrum`为RGB颜色$(0.95 , 0.05 , 0.025)$返回的光谱，以及该颜色的所有分量除以二后的光谱。对于原始颜色，我们看到较长波长的范围接近1，而较短波长的能量比预期的多。如果将该颜色除以二，得到的光谱表现得更为稳定，尽管注意到在较长波长中其幅度超过了原始红色值$0.475$。
    ]
  ],
) <rgb-unbounded-example>


#block(sticky: true)[#raw("<<Spectrum Method Definitions>>+=")] <fragment-SpectrumMethodDefinitions-5>
```cpp
...
#block(sticky: true)[#raw("<<RGBUnboundedSpectrum Private Members>>=")] <fragment-RGBUnboundedSpectrumPrivateMembers-0>
```cpp
Float scale = 1;
RGBSigmoidPolynomial rsp;
```
...
#block(sticky: true)[#raw("<<RGBUnboundedSpectrum Public Methods>>=")] <fragment-RGBUnboundedSpectrumPublicMethods-0>
```cpp
Float operator()(Float lambda) const { return scale * rsp(lambda); }
Float MaxValue() const { return scale * rsp.MaxValue(); }
```

==== #ez_caption[RGB Illuminants][RGB 照明体]
<rgb-illuminants>
#parec[
  ...
  如 @standard-illuminants 中的照明体光谱图所示，真实的照明体通常具有复杂的光谱分布。给定使用RGB颜色指定的光源，我们不尝试推断复杂的光谱分布，而是坚持使用平滑光谱，并进行适当的缩放。详细信息由`RGBIlluminantSpectrum`类处理。
]

#block(sticky: true)[#raw("<<Spectrum Definitions>>+=")] <fragment-SpectrumDefinitions-6>
```cpp
...
#parec[
  ...
  除了与`RGBUnboundedSpectrum`中使用的等效缩放因子以允许任意最大RGB值之外，`RGBIlluminantSpectrum`还将给定波长返回的值乘以该波长处颜色空间的标准照明体的值。照明体光谱建模中一个不直观的现象是，均匀光谱通常不会在转换为RGB后映射到中性色白。颜色空间总是假设观察者适应某种类型的环境光照，这影响颜色感知和中性色的概念。例如，常用的D65白点平均了典型的日光照明条件。因此，为了重现具有期望颜色的光源，我们使用一种粗略但有效的解决方案，即将白点与合适的反射率光谱相乘。从概念上讲，这类似于通过彩色薄膜查看白色基准光源。这也确保了由白光照射的白色物体在渲染图像中产生白色像素数值。
]

#block(sticky: true)[#raw("<<Spectrum Method Definitions>>+=")] <fragment-SpectrumMethodDefinitions-6>
```cpp
...
#parec[
  ...
  因此，用一个成员变量保存指向照明体的指针。
]

#block(sticky: true)[#raw("<<RGBIlluminantSpectrum Private Members>>=")] <fragment-RGBIlluminantSpectrumPrivateMembers-0>
```cpp
Float scale;
RGBSigmoidPolynomial rsp;
...
#block(sticky: true)[#raw("<<RGBIlluminantSpectrum Public Methods>>=")] <fragment-RGBIlluminantSpectrumPublicMethods-0>
```cpp
Float operator()(Float lambda) const {
...
#parec[
  ...
  我们不会在这里包括`Sample()`或`MaxValue()`方法的实现，因为其实现可以直接由前面的代码推得。
]
...
#parec[
  Editorial notes on the fixed source: “squashing its domain” refers to bounding the output, not the input domain. The quadratic polynomial discussed with `MaxValue()` is in @eqt:rgb-to-spectrum-spd; the source points to @eqt:rgb-to-spectrum-polynomial. The error reduction discussion points to panel (a) of @fig:mc-efficiency-vs-wavelength-samples-simple-scene, although panel (b) plots error. These source wordings/references have been retained above.
][
  固定原文校注：“squashing its domain”在这里指压缩输出值域，而非输入定义域。`MaxValue()` 段讨论的二次多项式位于 @eqt:rgb-to-spectrum-spd，原文却指向 @eqt:rgb-to-spectrum-polynomial。误差降低的讨论指向 @fig:mc-efficiency-vs-wavelength-samples-simple-scene 的 (a)，实际绘制误差的是 (b)。上文英文措辞及原文引用均予保留。
]


#include "supplements/4.6-expanded.typ"
