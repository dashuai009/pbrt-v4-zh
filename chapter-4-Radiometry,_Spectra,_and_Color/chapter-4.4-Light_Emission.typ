...
== #ez_caption[Light Emission][光的发射]
<light-emission>
#parec[
  ...
  温度高于绝对零度的物体的原子在运动。根据麦克斯韦方程，带电粒子的运动导致物体在一系列波长范围内发射电磁辐射。正如我们将很快看到的，在室温下，大部分辐射位于红外频段；物体需要更高的温度才能在可见频率下发射出可观的电磁辐射。
]
...
#parec[
  ...
  为了将能量转化为发射的电磁辐射，人们发明了许多不同类型的光源。发光的物体被称为#emph[灯]或#emph[照明体]，但我们避免使用后者术语，因为我们通常用“照明体”（illuminant）专指发光的光谱分布（@standard-illuminants）。灯被装在#emph[灯具]中，灯具包括所有固定和保护灯的部件以及任何用于塑造光分布的部件，如反射器或扩散器。
]
...
#parec[
  ...
  - 白炽灯（钨丝灯）有一个小的钨丝。电流通过钨丝使其加热，从而导致其发射电磁辐射，其波长分布取决于钨丝的温度。通常有一个磨砂玻璃外壳，以便将发射扩散到比钨丝更大的区域，并吸收发射光中的某些波长成分，以实现所需的波长发射分布。对于白炽灯，大部分发射功率在红外波段，这意味着消耗的能量大部分成为热，而不是可见光。
]
...
#parec[
  ...
  - 卤素灯也有一个钨丝，但包围钨丝的外壳内充满了卤素气体。随着时间的推移，白炽灯中的一部分钨丝在加热时会蒸发；卤素气体使这些蒸发的钨返回到钨丝上，从而延长灯的寿命。由于蒸发的钨返回到钨丝上，它不会像普通白炽灯泡那样附着在灯泡表面，从而也防止了灯泡变暗。
]
...
#parec[
  ...
  - 气体放电灯让电流通过氢气、氖气、氩气或金属蒸气，从而在特定波长发射光，这取决于气体中的特定原子。（选择那些在无用的红外频率中发射相对较少电磁辐射的原子用于气体。）由于更宽的波长光谱通常比所选原子直接生成的波长更具视觉吸引力，灯泡内部通常使用荧光涂层来将发射的波长转化为更宽的范围。（荧光涂层还通过将紫外波长转化为可见波长来提高效率。）
]
...
#parec[
  ...
  - LED 灯基于电致发光：它们使用由于电流通过而发射光子的材料。
]
...
#parec[
  ...
  对于所有这些光源，基础的物理过程是电子与原子碰撞，将原子的外层电子激发到更高能级。当这样的电子返回到较低的能级时，会发射出一个光子。还有许多其他有趣的产生光的过程，包括化学发光（如在发光棒中看到的）和生物发光——一种在萤火虫中看到的化学发光形式。尽管它们本身很有趣，但我们在此不再进一步考虑它们的机制。
]
...
#parec[
  ...
  #emph[光效]衡量光源将功率转化为可见光的效率，考虑到对于人类观察者而言，在不可见波长的发射几乎没有价值。有趣的是，它是一个光度量（发射的光通量）与一个辐射量（消耗的总功率，或对所有波长计的发射总功率，即辐射通量）的比值：
]

$
  frac(integral Phi_(e) (lambda) V(lambda) d lambda, integral Phi_(i) (lambda) d lambda),
$
...
#parec[
  ...
  白炽钨丝灯泡的典型光效值约为 $15 thin upright("lm/W")$。最高可达 683，对于一个完美高效的光源，它在 $lambda = 555 thin upright("nm")$ 发射所有光，这是 $V (lambda)$ 函数的峰值。（虽然这样的光源会有很高的光效，但对于人类观察者来说不一定是令人愉悦的。）
]

=== #ez_caption[Blackbody Emitters][黑体辐射体]
<blackbody-emitters>
#parec[
  ...
  #emph[黑体]是一个完美的辐射体：它以物理上可能的最高效率将功率转化为电磁辐射。虽然真正的黑体在物理上是不可实现的，但一些辐射体表现出接近黑体的行为。黑体还具有一个有用的闭式表达式，用于根据温度计算各波长处的发射量，这对于建模非黑体辐射体很有用。
]
...
#parec[
  ...
  #emph[普朗克定律]给出了黑体发射的辐亮度作为波长 $lambda$ 和温度 $T$ （以开尔文为单位）的函数：
]

$
  L_(e) (lambda, T) = frac(2 h c^2, lambda^5 (e^(frac(h c, lambda k_b T)) - 1)),
$ <plancks-law>
...
#parec[
  ...
  其中 $c$ 是介质中的光速（在真空中为 $299 , 792 , 458 thin upright("m/s")$ ）， $h$ 是普朗克常数， $6.62606957 times 10^(- 34) thin upright("Js")$，而 $k_b$ 是玻尔兹曼常数， $1.3806488 times 10^(- 23) thin upright("J/K")$，开尔文（K）是温度单位。黑体辐射体是完美漫射的；它们在所有方向上均匀地发射辐亮度。
]

#parec[
  @fig:blackbody-plots plots the emitted radiance distributions of a blackbody for a number of temperatures.
][
  @fig:blackbody-plots 绘制了黑体在若干温度下的发射辐亮度分布。
]

#figure(
  image("../pbr-book-website/4ed/Radiometry,_Spectra,_and_Color/pha04f12.svg"),
  caption: [ #ez_caption[ Plots of emitted radiance as a function of wavelength for blackbody emitters at a few temperatures, as given by @eqt:plancks-law. Note that as temperature increases, more of the emitted light is in the visible frequencies (roughly `380 nm-780 nm`) and that the spectral distribution shifts from reddish colors to bluish colors. The total amount of emitted energy grows quickly as temperature increases, as described by the Stefan–Boltzmann law in @eqt:stefan-boltzmann.][ 根据@eqt:plancks-law，绘制了黑体辐射体在几个温度下的辐亮度随波长的分布。注意，随着温度的升高，更多的发射光在可见频率（大约`380 nm-780 nm`）范围内，并且光谱分布由偏红转向偏蓝。随着温度的升高，发射能量的总量迅速增加，如@eqt:stefan-boltzmann 中的斯特藩-玻尔兹曼定律所述。
    ]
  ],
)<blackbody-plots>
...
#parec[
  ...
  `Blackbody()` 函数计算给定温度 `T`（以开尔文为单位）和给定波长 `lambda`下的发射辐亮度。
]

#block(sticky: true)[#raw("<<Spectrum Function Declarations>>=") #link("https://pbr-book.org/4ed/Radiometry,_Spectra,_and_Color/Representing_Spectral_Distributions.html#fragment-SpectrumFunctionDeclarations-1")[▼]] <fragment-SpectrumFunctionDeclarations-0>
```cpp
...
#parec[
  ...
  传递给 `Blackbody()` 的波长以纳米为单位，但@eqt:plancks-law 的常数以米为单位。因此，首先需要将波长乘以 $10^(- 9)$，转换为米。
]

#block(sticky: true)[#raw("<<Return emitted radiance for blackbody at wavelength lambda>>=")] <fragment-Returnemittedradianceforblackbodyatwavelengthmonolambda-0>
```cpp
...
#parec[
  ...
  非黑体的发射由#emph[基尔霍夫定律]描述，该定律指出，任何频率下的发射辐亮度分布等于该频率下完美黑体的发射乘以该频率下物体吸收的入射辐亮度的比例。（这种关系源于假设物体处于热平衡状态。）吸收的辐亮度比例等于1减去反射的量，因此发射的辐亮度为
]

$ L'_(e) (T , omega , lambda) = L_(e) (T , lambda) (1 - rho_("hd") (omega)) , $ <kirchoffs-law>
#parec[
  where $L_(e) (T , lambda)$ is the emitted radiance given by Planck's law, @eqt:plancks-law, and $rho_("hd") (omega)$ is the hemispherical-directional reflectance from @eqt:rho-hd.
][
  其中 $L_(e) (T , lambda)$ 是由普朗克定律给出的发射辐亮度，@eqt:plancks-law ，而 $rho_("hd") (omega)$ 是 @eqt:rho-hd 中的半球-方向反射率。
]
...
#parec[
  ...
  #emph[斯特藩-玻尔兹曼定律]给出了黑体辐射体在点 $p$ 的辐射出射度（回想一下，这是出射辐照度）：
]
...
#parec[
  ...
  其中 $sigma$ 是斯特藩-玻尔兹曼常数， $5.67032 times 10^(- 8) thin upright(W thin m^(- 2) thin K^(- 4))$。注意，所有频率上的总发射量增长得非常快——以 $T^4$ 的速率。因此，将黑体辐射体的温度加倍会使总发射能量增至原来的 16 倍。
]
...
#parec[
  ...
  黑体发射分布通过#emph[色温]的概念为描述非黑体辐射体的发射特性提供了一个有用的指标。 如果辐射体的光谱分布形状与某一温度下的黑体分布相似，那么我们可以说该辐射体具有相应的色温。 找到色温的一种方法是取光的发射量最大的波长，并使用#emph[维恩位移定律]求出相应温度，该定律给出了黑体在给定温度下发射最大值的波长：
]

$ lambda_("max") = b / T , $ <wien-displacement>
#parec[
...
#parec[
  ...
  白炽钨灯的色温通常在 2700 K 左右，钨卤素灯约为 3000 K。 荧光灯的色温可从 2700 K 到 6500 K 不等。 一般来说，色温超过 5000 K被描述为“冷”，而 2700–3000 K被描述为“暖”。
]

=== #ez_caption[Standard Illuminants][标准照明体]
...
#parec[
  ...
  另一种分类光发射分布的有用方法是由国际照明委员会（CIE）定义的若干“标准照明体”。
]
...
#parec[
  ...
  标准照明体 A 于 1931 年引入，旨在代表平均白炽光。 它相当于约 $2856 thin upright(K)$ 的黑体辐射器。 （最初定义为 $2850 thin upright(K)$ 的黑体，但普朗克定律中使用的常数的精度后来得到了改进。 因此，规范被更新为以 1931 年的常数为准，以便照明体保持不变。）@fig:a-illuminant 显示了 A 照明体的光谱分布图。
]

#figure(
  image("../pbr-book-website/4ed/Radiometry,_Spectra,_and_Color/pha04f13.svg"),
  caption: [ #ez_caption[Plot of the CIE Standard Illuminant A's Spectral Power Distribution as a Function of Wavelength in nm. This illuminant represents incandescent illumination and is close to a blackbody at $2856 "K"$.][ CIE 标准照明体 A 的光谱功率分布随波长（nm）变化的图示。该照明体代表白炽灯照明，其光谱接近于温度为 2856 K 的黑体。
    ]
  ],
)<a-illuminant>
#parec[
  ...
  （B 和 C 照明体旨在模拟一天中两个时间的日光，并通过与特定滤光片结合的A 照明体生成。 它们不再使用。E 照明体被定义为具有恒定光谱分布，仅用于与其他照明体的比较。）
]
...
#parec[
  ...
  D 照明体描述了不同状态的日光。 它是基于对各种日光光谱的特征向量分析定义的，这使得可以用三个项（一个固定和两个加权）的线性组合来表达日光， 其中一个权重基本上对应于由于云量变化的黄蓝色变化，另一个对应于由于大气中的水（如雾霾等）导致的粉红绿色变化。 D65 大约为 $6504 thin upright(K)$ 色温（不是 $6500 thin upright(K)$ ——同样是由于普朗克定律中使用的常数值的变化）并旨在模拟欧洲正午的阳光。 （见@fig:d-illuminant。）CIE 建议，除非有特定原因，否则应使用此照明体来代表日光。
]

#figure(
  image("../pbr-book-website/4ed/Radiometry,_Spectra,_and_Color/pha04f14.svg"),
  caption: [ #ez_caption[Plot of the CIE Standard D65 Illuminant Spectral Distribution as a Function of Wavelength in nm. This illuminant represents noontime daylight at European latitudes and is commonly used to define the whitepoint of color spaces (@rgb-color-spaces).][ CIE 标准照明体 D65 的光谱分布随波长（nm）变化的图示。该照明体代表欧洲纬度的正午日光，通常用于定义颜色空间的白点（@rgb-color-spaces）。
    ]
  ],
)<d-illuminant>
...
#parec[
  ...
  最后，F 系列照明体描述了荧光灯；它基于对多种实际荧光灯的测量。
]
...
#figure(
  ...
  caption: [ #ez_caption[ *Plots of the F4 and F9 Standard Illuminants as a Function of Wavelength in nm.* These represent two fluorescent lights. Note that the distributions are quite different. Spikes in the two distributions correspond to the wavelengths directly emitted by atoms in the gas, while the other wavelengths are generated by the bulb's fluorescent coating. The F9 illuminant is a "broadband" emitter that uses multiple phosphors to achieve a more uniform spectral distribution. ][*F4 和 F9 标准照明体随波长（nm）变化的图。*这些代表了两种荧光灯。注意分布差异很大。 两个分布中的尖峰对应于气体中原子直接发射的波长，而其他波长是由灯泡的荧光涂层产生的。 F9 照明体是一个“宽带”辐射体，使用多种荧光材料来实现更均匀的光谱分布。 ]
  ],
)<f-illuminant>