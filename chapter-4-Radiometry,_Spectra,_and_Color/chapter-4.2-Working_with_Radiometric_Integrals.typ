...
== #ez_caption[Working with Radiometric Integrals][处理辐射量积分]
...
#parec[
  ...
  渲染中经常需要计算辐射量的积分。本节将介绍一些简化计算的技巧，并以一点处的辐照度为例说明其用法。在表面法向量为 $upright(bold(n))$ 的点 $p$ 处，来自方向集合 $Omega$ 的辐亮度所产生的辐照度为
]


$
  E (p , upright(bold(n))) = integral_Omega L_(i) (p , omega) lr(|cos theta|) thin d omega ,
$ <irradiance-from-radiance>

#parec[
  where $L_(i) (p , omega)$ is the incident radiance function (@fig:irradiance-sphere) and the $cos theta$ factor in the integrand is due to the $d A^tack.t$ factor in the definition of radiance. $theta$ is measured as the angle between $omega$ and the surface normal $upright(bold(n))$. Irradiance is usually computed over the hemisphere $cal(H)^2(upright(bold(n)))$ of directions about a given surface normal $upright(bold(n))$.
][
  其中 $L_(i) (p , omega)$ 是入射辐亮度函数（@fig:irradiance-sphere），积分中的 $cos theta$ 因子是来自辐亮度定义中的 $d A^tack.t$ 因子。 $theta$ 是 $omega$ 和表面法向量 $upright(bold(n))$ 之间的夹角。辐照度通常是在给定表面法向量 $upright(bold(n))$ 的半球 $cal(H)^2(upright(bold(n)))$ 上计算的。
]

#figure(
  image("../pbr-book-website/4ed/Radiometry,_Spectra,_and_Color/pha04f05.svg"),
  caption: [
    #ez_caption[Irradiance at a point $p$ is given by the integral of
      radiance times the cosine of the incident direction over the entire
      upper hemisphere above the point.][点 $p$ 处的辐照度由辐亮度乘以入射方向角的余弦在点上方整个上半球的积分给出。]
  ],
)<irradiance-sphere>
...
#parec[
  ...
  因此半球上由辐亮度求辐照度的积分可以更简单地写为
]

$ E(p, upright(bold(n))) = integral_(cal(H)^2 (upright(bold(n)))) L_(i) (p, omega) thin d omega^perp . $
#parec[
  ...
  在本书的其余部分中，我们将用立体角而不是投影立体角来编写方向上的积分。然而，在其他来源中，可能会使用投影立体角，因此了解积分所采用的实际测度始终很重要。
]


=== #ez_caption[Integrals over Spherical Coordinates][对球坐标积分]
<integrals-over-spherical-coordinates>

#parec[
  It is often convenient to transform integrals over solid angle into integrals over spherical coordinates $(theta, phi.alt)$ using @eqt:spherical-coordinates. In order to convert an integral over a solid angle to an integral over $(theta, phi.alt)$, we need to be able to express the relationship between the differential area of a set of directions $d omega$ and the differential area of a $(theta, phi.alt)$ pair (@fig:dw-to-dtdp). The differential area on the unit sphere $d omega$ is the product of the differential lengths of its sides, $sin theta d phi.alt$ and $d theta$. Therefore,
][
  利用 @eqt:spherical-coordinates 将立体角积分转换为球坐标 $(theta, phi.alt)$ 上的积分，往往更方便。为此，需要建立方向集合的微分面积 $d omega$ 与坐标对 $(theta, phi.alt)$ 的微分面积之间的关系（@fig:dw-to-dtdp）。单位球面上的微分面积 $d omega$ 等于其两边的微分长度 $sin theta d phi.alt$ 与 $d theta$ 的乘积。因此，
]
...
#figure(
  ...
  caption: [
    #ez_caption[The differential area $d omega$ subtended by a
      ...
      spherical angles.][由微小立体角所张的微分面积 $d omega$是两条边的微分长度 $sin theta d phi.alt$ 和 $d theta$的乘积。由此得到的关系式 $d omega = sin theta d theta d phi.alt$是将立体角积分转换为球面角积分的关键。]
  ],
)<dw-to-dtdp>
...
#parec[
  ...
  因此，我们可以看到在半球表面上的辐照度积分，@eqt:irradiance-from-radiance，其中 $Omega = cal(H)^2 (upright(bold(n)))$，可以等效地写为
]



$
  E (p , upright(bold(n))) = integral_0^(2 pi) integral_0^(pi \/ 2) L_(i) (
    p , theta , phi.alt
  ) cos theta sin theta d theta d phi.alt .
$

#parec[
  If the radiance is the same from all directions, the equation simplifies to $E = pi L_(i)$.
][
  如果辐亮度从所有方向都是相同的，方程简化为 $E = pi L_(i)$。
]

=== #ez_caption[Integrals over Area][对面积积分]
<integrals-over-area>
...
#parec[
  ...
  最后一个有用的变换是将方向上的积分转换为面积上的积分。再次考虑@eqt:irradiance-from-radiance 中的辐照度积分，并设想有一个具有恒定出射辐亮度的四边形，我们希望计算它在点 $p$ 处产生的辐照度。作为方向 $omega$ 或球面坐标 $(theta , phi.alt)$ 上的积分来计算这个值通常并不简单，因为给定一个特定方向，确定四边形在该方向或 $(theta , phi.alt)$ 上是否可见是复杂的。作为四边形面积上的积分来计算辐照度要容易得多。
]
...
#parec[
  ...
  其中 $theta$ 是 $d A$ 的表面法向量与指向 $p$ 的向量之间的角度， $r$ 是从 $p$ 到 $d A$ 的距离（@fig:dw-dA）。我们在此不推导这个结果，但可以直观地理解：如果 $d A$ 距离 $p$ 为1并且完全对齐并垂直于 $d omega$，那么 $d omega = d A$， $theta = 0$，@eqt:dw-dA-eqt 成立。当 $d A$ 远离 $p$ 或旋转以便不与 $d omega$ 的方向对齐时， $r^2$ 和 $cos theta$ 因子相应地调整以减少 $d omega$。
]
#figure(
  image("../pbr-book-website/4ed/Radiometry,_Spectra,_and_Color/pha04f08.svg"),
  caption: [
    #ez_caption[The differential solid angle $d omega$ subtended by a differential area $d A$ is equal to $d A cos theta \/ r^2$, where $theta$ is the angle between $d A$’s surface normal and the vector to the point $p$ and $r$ is the distance from $p$ to $d A$.][由微分面积 $d A$所张的微分立体角 $d omega$等于$d A cos theta \/ r^2$，其中 $theta$是$d A$的表面法向量与指向点$p$的向量之间的角度，$r$是从$p$到$d A$的距离。]
  ],
)<dw-dA>
...
#parec[
  where $L$ is the emitted radiance from the surface of the quadrilateral, $theta_i$ is the angle between the surface normal at $p$ and the direction from $p$ to the point $p'$ on the light, and $theta_o$ is the angle between the surface normal at $p'$ on the light and the direction from $p'$ to $p$ (@fig:quad-irradiance).
][
  其中 $L$ 是从四边形表面发出的辐亮度， $theta_i$ 是点 $p$ 处的表面法向量与从 $p$ 到光源点 $p'$ 的方向之间的角度， $theta_o$ 是光源点 $p'$ 处的表面法向量与从 $p'$ 到 $p$ 的方向之间的角度（@fig:quad-irradiance）。
]
...
#figure(
  ...
  caption: [
    #ez_caption[ To compute irradiance at a point $p$ from a quadrilateral source, it is easier to integrate over the surface area of
      ...
      为了计算四边形光源在点 $p$ 处产生的辐照度，对光源的表面积积分，比对它所张的不规则方向集合积分更容易。@eqt:dw-dA-eqt 给出的立体角和面积之间的关系让我们可以在这两种方法之间转换。
    ]
  ],
)<quad-irradiance>
