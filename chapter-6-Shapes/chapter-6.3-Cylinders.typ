...
== #ez_caption[Cylinders][圆柱体]
<cylinders>
#parec[
  Another useful quadric is the cylinder; `pbrt` provides a cylinder #link("https://pbr-book.org/4ed/Shapes/Basic_Shape_Interface.html#Shape")[`Shape`] that is centered around the $z$ axis. The user can supply a minimum and maximum $z$ value for the cylinder, as well as a radius and maximum $phi$ sweep value (@fig:cylinder-setting).
][
  另一个有用的二次曲面是圆柱体；`pbrt` 提供了一个以 $z$ 轴为中心的圆柱体#link("https://pbr-book.org/4ed/Shapes/Basic_Shape_Interface.html#Shape")[`Shape`]。用户可以为圆柱体提供最小和最大 $z$ 值，以及半径和最大 $phi$ 扫掠角度（@fig:cylinder-setting）。
]

#figure(
  image("../pbr-book-website/4ed/Shapes/pha06f07.svg"),
  caption: [
    #ez_caption[Basic Setting for the Cylinder Shape. The cylinder has a radius of~$r$ and covers a range along the $z$
      axis. A~partial cylinder may be swept by specifying a maximum $phi$
      value. ][圆柱体形状的基本设置。圆柱体的半径为 $r$，覆盖 $z$ 轴上的一个范围。可以通过指定最大 $phi$ 值来扫掠部分圆柱体。]
  ],
)<cylinder-setting>

#block(sticky: true)[#raw("<<Cylinder Definition>>=")] <fragment-CylinderDefinition-0>
```cpp
class Cylinder {
  public:
...
#figure(
  image("../pbr-book-website/4ed/Shapes/cylinders.png"),
  caption: [
    #ez_caption[ Two Cylinders. A partial cylinder is on the
...
#parec[
  Similar to the #link("https://pbr-book.org/4ed/Shapes/Spheres.html#Sphere")[`Sphere`] constructor, the #link(<Cylinder>)[`Cylinder`] constructor takes transformations that define its object space and the parameters that define the cylinder itself. Its constructor just initializes the corresponding member variables, so we will not include it here.
][
  与 #link("https://pbr-book.org/4ed/Shapes/Spheres.html#Sphere")[`Sphere`] 构造函数类似，#link(<Cylinder>)[`Cylinder`] 构造函数接受定义其对象空间的转换和定义圆柱体本身的参数。其构造函数只是初始化相应的成员变量，因此我们在此不包括它。
]

#block(sticky: true)[#raw("<<Cylinder Public Methods>>=")] <fragment-CylinderPublicMethods-0>
```cpp
...
#block(sticky: true)[#raw("<<Cylinder Private Members>>=")] <fragment-CylinderPrivateMembers-0>
```cpp
...
#parec[
  ...
  圆柱侧面可看作卷起来的矩形。将其展开后，它的高度是 $z_"max" - z_"min"$，宽度是 $r phi.alt_"max"$
]

#block(sticky: true)[#raw("<<Cylinder Public Methods>>+=")] <fragment-CylinderPublicMethods-1>
```cpp
...
#parec[
  ...
  与球体一样，圆柱体的空间包围方法使用 $z$ 范围计算保守的包围盒，但不考虑最大方位角 $phi.alt$。
]

#block(sticky: true)[#raw("<<Cylinder Method Definitions>>=")] <fragment-CylinderMethodDefinitions-0>
```cpp
...
#parec[
  Its surface normal bounding function is conservative in two ways: not only does it not account for $phi.alt_"max" < 2 pi$, but the actual set of normals of a cylinder can be described by a circle on the sphere of all directions. However, #link("https://pbr-book.org/4ed/Geometry_and_Transformations/Spherical_Geometry.html#DirectionCone")[DirectionCone]'s representation is not able to bound such a distribution more tightly than with the entire sphere of directions, and so that is the bound that is returned.
][
  其表面法向量包围函数在两个方面保持保守：不仅不考虑 $phi.alt_"max" < 2 pi$，而且圆柱体的实际法向量集合可以用一个方向球上的圆来描述。 然而，#link("https://pbr-book.org/4ed/Geometry_and_Transformations/Spherical_Geometry.html#DirectionCone")[`DirectionCone`] 这种表示方法无法比整个方向球更紧密地界定圆柱上的法向量分布，因此直接返回整个方向球。
]


#block(sticky: true)[#raw("<<Cylinder Public Methods>>+=")] <fragment-CylinderPublicMethods-2>
```cpp
...
#parec[
  Also similar to the sphere (and for similar reasons), `Cylinder` provides a `BasicIntersect()` method that returns a #link("https://pbr-book.org/4ed/Shapes/Spheres.html#QuadricIntersection")[`QuadricIntersection`] as well as an `InteractionFromIntersection()` method that converts that to a full #link("https://pbr-book.org/4ed/Geometry_and_Transformations/Interactions.html#SurfaceInteraction")[`SurfaceInteraction`]. Given these, the `Intersect()` method is again a simple composition of them. (If `pbrt` used virtual functions, a design alternative would be to have a `QuadricShape` class that provided a default `Intersect()` method and left `BasicIntersect()` and `InteractionFromIntersection()` as pure virtual functions for subclasses to implement.)
][
  同样类似于球体（出于类似的原因），`Cylinder` 提供了一个 `BasicIntersect()` 方法，该方法返回一个 #link("https://pbr-book.org/4ed/Shapes/Spheres.html#QuadricIntersection")[`QuadricIntersection`]，以及一个 `InteractionFromIntersection()` 方法，将其转换为一个完整的 #link("https://pbr-book.org/4ed/Geometry_and_Transformations/Interactions.html#SurfaceInteraction")[`SurfaceInteraction`]。 鉴于此，`Intersect()` 方法再次是它们的简单组合。（如果 `pbrt` 使用虚函数，另一种设计选择是有一个 `QuadricShape` 类，提供默认的 `Intersect()` 方法，并将 `BasicIntersect()` 和 `InteractionFromIntersection()` 作为纯虚函数供子类实现。）
]

#block(sticky: true)[#raw("<<Cylinder Public Methods>>+=")] <fragment-CylinderPublicMethods-3>
```cpp
...
#parec[
  The form of the `BasicIntersect()` method also parallels the sphere's, computing appropriate quadratic coefficients, solving the quadratic equation, and then handling the various cases for partial cylinders. A number of fragments can be reused from the #link("https://pbr-book.org/4ed/Shapes/Spheres.html#Sphere")[`Sphere`]'s implementation.
][
  `BasicIntersect()` 方法的形式也与球体的类似，计算适当的二次系数，解二次方程，然后处理部分圆柱体的各种情况。 许多代码片段可以从 #link("https://pbr-book.org/4ed/Shapes/Spheres.html#Sphere")[`Sphere`] 的实现中复用。
]

#block(sticky: true)[#raw("<<Cylinder Public Methods>>+=")] <fragment-CylinderPublicMethods-4>
```cpp
...
#parec[
  As before, the fragment that computes the quadratic discriminant, #link("https://pbr-book.org/4ed/Shapes/Managing_Rounding_Error.html#fragment-Computecylinderquadraticdiscriminantmonodiscrim-0")[`Compute cylinder quadratic discriminant`] discrim is defined in @accurate-quadratic-discriminants after topics related to floating-point accuracy have been discussed.
][
  如前所述，计算二次判别式的部分，\<\<#link("https://pbr-book.org/4ed/Shapes/Managing_Rounding_Error.html#fragment-Computecylinderquadraticdiscriminantmonodiscrim-0")[Compute cylinder quadratic discriminant]discrim\>\> 在@accurate-quadratic-discriminants 节中定义，讨论了与浮点精度相关的主题。
]

#block(sticky: true)[#raw("<<Solve quadratic equation to find cylinder t0 and t1 values>>=")] <fragment-Solvequadraticequationtofindcylindermonot0andmonot1values-0>
```cpp
Interval t0, t1;
...
#block(sticky: true)[#raw("<<Compute cylinder quadratic coefficients>>=")] <fragment-Computecylinderquadraticcoefficients-0>
```cpp
...
#parec[
  The next step in the intersection method makes sure that the hit is in the specified $z$ range and that the angle $phi.alt$ is acceptable. If not, it rejects the hit and checks $t_1$ if it has not already been considered—these tests resemble the conditional logic in #link("https://pbr-book.org/4ed/Shapes/Spheres.html#Sphere::Intersect")[`Sphere::Intersect()`].
][
  接下来，确保交点命中在指定的 $z$ 范围内并且角度 $phi.alt$ 是可接受的。否则就拒绝这个交点，并在尚未检查过 $t_1$ 时继续检查它——这些测试类似于#link("https://pbr-book.org/4ed/Shapes/Spheres.html#Sphere::Intersect")[`Sphere::Intersect()`]中的条件逻辑。
]

#block(sticky: true)[#raw("<<Test cylinder intersection against clipping parameters>>=")] <fragment-Testcylinderintersectionagainstclippingparameters-0>
```cpp
...
#parec[
  For a successful intersection, the same three values suffice to provide enough information to later compute the corresponding #link("https://pbr-book.org/4ed/Geometry_and_Transformations/Interactions.html#SurfaceInteraction")[`SurfaceInteraction`].
][
  对于成功的交点，相同的三个值足以提供足够的信息，以便稍后计算相应的#link("https://pbr-book.org/4ed/Geometry_and_Transformations/Interactions.html#SurfaceInteraction")[`SurfaceInteraction`]。
]

#block(sticky: true)[#raw("<<Return QuadricIntersection for cylinder intersection>>=")] <fragment-ReturnmonoQuadricIntersectionforcylinderintersection-0>
```cpp
...
#block(sticky: true)[#raw("<<Cylinder Public Methods>>+=")] <fragment-CylinderPublicMethods-5>
```cpp
...
#parec[
  InteractionFromIntersection() computes all the quantities needed to initialize a #link("https://pbr-book.org/4ed/Geometry_and_Transformations/Interactions.html#SurfaceInteraction")[SurfaceInteraction] from a cylinder's #link("https://pbr-book.org/4ed/Shapes/Spheres.html#QuadricIntersection")[QuadricIntersection].
][
  `InteractionFromIntersection()` 根据圆柱的 `QuadricIntersection`，计算初始化 `SurfaceInteraction` 所需的全部量。
]

#block(sticky: true)[#raw("<<Cylinder Public Methods>>+=")] <fragment-CylinderPublicMethods-6>
```cpp
...
#block(sticky: true)[#raw("<<Compute cylinder ∂p/∂u and ∂p/∂v >>=")] <fragment-Computecylinderdpduanddpdv-0>
```cpp
...
#parec[
  ...
  我们再次使用Weingarten 方程来计算圆柱法向量的参数偏导数。相关的偏导数为
]
...
#block(sticky: true)[#raw("<<Compute cylinder ∂n/∂u and ∂n/∂v >>=")] <fragment-Computecylinderdnduanddndv-0>
```cpp
...
#block(sticky: true)[#raw("<<Cylinder Public Methods>>+=")] <fragment-CylinderPublicMethods-7>
```cpp
...
#block(sticky: true)[#raw("<<Cylinder Public Methods>>+=")] <fragment-CylinderPublicMethods-8>
```cpp
...
#block(sticky: true)[#raw("<<Cylinder Public Methods>>+=")] <fragment-CylinderPublicMethods-9>
```cpp
...
#block(sticky: true)[#raw("<<Cylinder Public Methods>>+=")] <fragment-CylinderPublicMethods-10>
```cpp
...
```
#include "supplements/6.3-expanded.typ"
