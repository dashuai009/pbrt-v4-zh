...
== #ez_caption[Basic Shape Interface][基本形状接口]
...
#block(sticky: true)[#raw("<<Shape Definition>>=")] <fragment-ShapeDefinition-0>
```cpp
...
#parec[
  ...
  `pbrt`渲染的场景中一些对象的计算成本通常比较高。对于许多操作来说，一个可以囊括对象的三维#emph[包围体]是有用的。例如，如果一条光线没有穿过特定的包围体，`pbrt`可以避免处理包围体内的所有对象。
]

#parec[
  Axis-aligned bounding boxes are a convenient bounding volume, as they require only six floating-point values to store. They fit many shapes well and it is fairly inexpensive to test for the intersection of a ray with an axis-aligned bounding box. Each `Shape` implementation must therefore be capable of bounding itself with an axis-aligned bounding box represented by a #link("https://pbr-book.org/4ed/Geometry_and_Transformations/Bounding_Boxes.html#Bounds3f")[`Bounds3f`]. The returned bounding box should be in the rendering coordinate system (recall the discussion of coordinate systems in @camera-coordinate-spaces).
][
  轴对齐的包围盒（AABB）是一种方便的包围体，因为它们只需要六个浮点值来存储。它们适合许多形状，并且测试光线与轴对齐包围盒的相交消耗较低。因此，每个`Shape`的实现必须能够用一个由#link("https://pbr-book.org/4ed/Geometry_and_Transformations/Bounding_Boxes.html#Bounds3f")[`Bounds3f`]表示的轴对齐包围盒来包围自身。返回的包围盒应在渲染坐标系中（回忆在@camera-coordinate-spaces 中的坐标系讨论）。
]

#block(sticky: true)[#raw("<<Shape Interface>>=")] <fragment-ShapeInterface-0>
```cpp
...
=== #ez_caption[Ray–Bounds Intersections][射线与包围盒求交]
<raybounds-intersections>

#parec[
  Given the use of #link("https://pbr-book.org/4ed/Geometry_and_Transformations/Bounding_Boxes.html#Bounds3f")[`Bounds3f`] instances to bound shapes, we will add a #link("https://pbr-book.org/4ed/Geometry_and_Transformations/Bounding_Boxes.html#Bounds3")[`Bounds3`] method, #link(<Bounds3::IntersectP>)[`Bounds3::IntersectP()`], that checks for a ray-box intersection and returns the two parametric $t$ values of the intersection, if any.
][
  鉴于使用#link("https://pbr-book.org/4ed/Geometry_and_Transformations/Bounding_Boxes.html#Bounds3f")[`Bounds3f`] 的实例来限制形状，我们给`Bounds3`添加了一个#link(<Bounds3::IntersectP>)[`Bounds3::IntersectP()`]方法，用于计算光线与包围盒的交点，返回交点处的两个参数 $t$ 值（如果有的话）。
]
...
#parec[
  ...
  基本的光线-包围盒相交算法如下：我们从一个参数区间开始，该区间覆盖了沿光线寻找相交的位置 $t$ 的范围；通常是 $(0 , oo)$。然后我们将依次计算光线与每个轴对齐夹层相交的两个参数 $t$ 位置。我们计算每个夹层相交区间与当前相交区间的集合交集，如果发现结果区间是退化的，则返回失败。如果在检查完所有三个夹层后，区间是非退化的，我们就得到了光线在盒子内的参数范围。 @fig:ray-box 展示了这个过程，@fig:ray-slabs 展示了光线与夹层相交的基本几何。
]
...
#figure(
  ...
  caption: [
    #ez_caption[
      ...
      *光线与轴对齐包围盒相交。*我们依次计算与每个夹层的相交点，逐步缩小参数区间。这里，在2D中，沿光线的$x$和$y$范围的相交（粗线段）给出了光线在盒子内的范围。
    ]
  ],
) <ray-box>
...
#figure(
  ...
  caption: [
    #ez_caption[
      ...
      *光线与轴对齐夹层相交。*这里显示的两个平面由$x = c$描述，其中$c$为常量。每个平面的法向量是$(1 , 0 , 0)$。除非光线与平面平行，否则它将两次与夹层相交，在参数位置$t_(upright("near"))$和$t_(upright("far"))$。
    ]
  ],
) <ray-slabs>
...
#block(sticky: true)[#raw("<<Bounds3 Inline Functions>>+=")] <fragment-Bounds3InlineFunctions-9>
```cpp
template <typename T>
...
#parec[
  ...
  对于每对平面，此例程需要分别求出射线与两个平面的交点。例如，由两个垂直于 $x$ 轴的平面描述的夹层可以由通过点 $(x_0 , 0 , 0)$ 和 $(x_1 , 0 , 0)$ 的平面描述，每个平面的法向量为 $(1 , 0 , 0)$。考虑第一个平面相交的 $t$ 值， $t_0$。起点为 $upright(bold(o))$、方向为 $upright(bold(d))$ 的射线与平面 $a x + b y + c z + d = 0$ 的相交的参数 $t$ 值可以通过将光线方程代入平面方程来找到：
]

$
  0 &= a (o_x + t upright(bold(d))_x) + b (o_y + t upright(bold(d))_y) + c (o_z + t upright(bold(d))_z) + d \
...
$ t_0 = (x_0 - o_x) / upright(bold(d))_x $
#parec[
  The code to compute the $t$ values of the slab intersections starts by computing the reciprocal of the corresponding component of the ray direction so that it can multiply by this factor instead of performing multiple divisions. Note that, although it divides by this component, it is not necessary to verify that it is nonzero. If it is zero, then `invRayDir` will hold an infinite value, either $- oo$ or $oo$, and the rest of the algorithm still works correctly.#footnote[This assumes that the architecture being used supports IEEE floating-point arithmetic, which is universal on modern systems. The relevant properties of IEEE floating-point arithmetic are that for all $v > 0$, $v / 0 = infinity$ and for all $w < 0$, $w / 0 = -infinity$, where $infinity$ is a special value such that any positive number multiplied by $infinity$ gives $infinity$ and any negative number multiplied by $infinity$ gives $-infinity$ and so on. See @floating-point-arithmetic for more information about floating-point arithmetic.]
][
  计算夹层交点的 $t$ 值的代码首先通过计算光线方向相应分量的倒数来开始，这样它可以乘以这个因子而不是执行多次除法。注意，虽然它除以这个分量，但没有必要验证它是否为零。如果它为零，那么 `invRayDir` 将持有一个无穷大值，可能是 $- oo$ 或 $oo$， 并且算法的其余部分仍然可以正确工作。#footnote[这里假定所用架构支持 IEEE 浮点运算，现代系统普遍如此。相关性质是：对所有 $v > 0$，$v / 0 = infinity$；对所有 $w < 0$，$w / 0 = -infinity$。其中 $infinity$ 是特殊值，任意正数乘以它得到 $infinity$，任意负数乘以它得到 $-infinity$，等等。关于浮点运算的更多信息，参见 @floating-point-arithmetic 。]
]

#block(sticky: true)[#raw("<<Update interval for ith bounding box slab>>=")] <fragment-Updateintervalformonoithboundingboxslab-0>
```cpp
...
#parec[
  The two distances are reordered so that `tNear` holds the closer intersection and `tFar` the farther one. This gives a parametric range $[upright("tNear") , upright("tFar")]$, which is used to compute the set intersection with the current range $[t_0 , t_1]$ to compute a new range. If this new range is empty (i.e., $t_0 > t_1$ ), then the code can immediately return failure.
][
  两个距离被重新排序，使得 `tNear` 保存较近的交点，`tFar` 保存较远的交点。这给出了一个参数范围 $[upright("tNear") , upright("tFar")]$，用于计算与当前范围 $[t_0 , t_1]$ 的集合交集以计算新范围。如果这个新范围为空（即 $t_0 > t_1$ ），那么代码可以立即返回失败。
]
...
#parec[
  ...
  这里还有另一个与浮点相关的微妙之处：在光线起点位于一个包围盒夹层的平面上并且光线位于夹层的平面上的情况下，可能会通过 $0 \/ 0$ 形式的表达式计算出 `tNear` 或 `tFar`，这会导致浮点“非数值”（NaN）值。 像无穷大值一样，NaN 有明确的语义：例如，任何涉及 NaN 的逻辑比较总是评估为假。 因此，更新 `t0` 和 `t1` 值的代码被仔细编写，以确保如果 `tNear` 或 `tFar` 是 NaN，那么 `t0` 或 `t1` 永远不会取 NaN 值，而总是保持不变。
]

#block(sticky: true)[#raw("<<Update parametric interval from slab intersection t values>>=")] <fragment-Updateparametricintervalfromslabintersectiontvalues-0>
```cpp
...
#block(sticky: true)[#raw("<<Bounds3 Inline Functions>>+=")] <fragment-Bounds3InlineFunctions-10>
```cpp
template <typename T>
...
#parec[
  ...
  若射线方向在某轴上的分量为负，则近交点位于该轴边界值较大的平面，远交点位于边界值较小的平面。 实现可以利用这一观察来直接计算每个方向的近和远参数值。
]

#block(sticky: true)[#raw("<<Check for ray intersection against x and y slabs>>=")] <fragment-Checkforrayintersectionagainstxandyslabs-0>
```cpp
...
#parec[
  ...
  ⟨Check for ray intersection against $z$ slab⟩ 片段的实现类似，正文不再列出。
]
...
#parec[
  ...
  此求交测试是遍历 BVHAggregate 加速结构的核心，该结构在@bounding-volume-hierarchies 中介绍。 因为在遍历 BVH 树时会执行许多光线与包围盒的求交测试，我们发现这个优化方法比使用没有预先计算方向倒数和符号的 `Bounds3::IntersectP()` 变体提供了大约 15% 的整体渲染时间性能提升。
]
...
#block(sticky: true)[#raw("<<Shape Interface>>+=")] <fragment-ShapeInterface-2>
```cpp
...
#block(sticky: true)[#raw("<<ShapeIntersection Definition>>=")] <fragment-ShapeIntersectionDefinition-0>
```cpp
...
#parec[
  ...
  - 传递给求交例程的射线是在渲染空间中，因此如果需要进行求交测试，形状负责将其转换为对象空间。但返回的交点信息应在渲染空间中。
]
...
#parec[
  ...
  第二个求交测试方法 `Shape::IntersectP()` 是一个谓词函数，用于判断是否存在交点，而不返回关于交点本身的任何细节。 该测试通常比完整的求交测试更高效。此方法特别用于测试从场景中的某一点到光源的可见性的阴影射线。
]

#block(sticky: true)[#raw("<<Shape Interface>>+=")] <fragment-ShapeInterface-3>
```cpp
bool IntersectP(const Ray &ray, Float tMax = Infinity) const;
```

=== #ez_caption[Intersection Coordinate Spaces][求交的坐标空间]
...
#parec[
  ...
  对于某些形状，求交例程最自然地在其对象空间中表达。例如，以下球体形状计算与位于原点的给定半径球体的交点。 球体位于原点允许对求交算法进行各种简化。其他形状，如三角形，将其表示转换为渲染空间并在那里执行求交测试。
]
...
#parec[
  ...
  如果射线-形状求交算法在射线的距离 $t$ 处找到对象空间交点，则对象空间交点为
]
...
#parec[
  ...
  在 `pbrt` 中，面光源是通过将发射特性附加到 `Shape` 来定义的。要将 `Shape` 用作面光源，必须能够在渲染空间中返回形状的表面积。
]

#block(sticky: true)[#raw("<<Shape Interface>>+=")] <fragment-ShapeInterface-4>
```cpp
Float Area() const;
```
...
#parec[
  ...
  有两种形状的采样方法，均命名为 `Sample()`。第一种方法使用以表面积为测度的采样分布选择形状表面上的点，并返回关于采样点的局部几何信息，存储在 #link(<ShapeSample>)[`ShapeSample`] 中。提供的样本值 `u` 是 $[0, 1)^2$ 中的均匀样本，用于定位几何形状上的点。
]

#block(sticky: true)[#raw("<<Shape Interface>>+=")] <fragment-ShapeInterface-5>
```cpp
...
#parec[
  The `ShapeSample` structure that is returned stores an #link("https://pbr-book.org/4ed/Geometry_and_Transformations/Interactions.html#Interaction")[`Interaction`] corresponding to a sampled point on the surface as well as the probability density with respect to surface area on the shape for sampling that point.
][
  返回的 `ShapeSample` 结构存储了对应于表面上采样点的 #link("https://pbr-book.org/4ed/Geometry_and_Transformations/Interactions.html#Interaction")[`Interaction`] 以及该点相对于形状表面积的采样概率密度。
]

#block(sticky: true)[#raw("<<ShapeSample Definition>>=")] <fragment-ShapeSampleDefinition-0>
```cpp
struct ShapeSample {
    Interaction intr;
    Float pdf;
...
#parec[
  Shapes must also provide an associated `PDF()` method that returns probability density for sampling the specified point on the shape that corresponds to the given #link("https://pbr-book.org/4ed/Geometry_and_Transformations/Interactions.html#Interaction")[`Interaction`]. This method should only be called with interactions that are on the shape's surface. Although `Sample()` already returns the probability density for the point it samples, this method is useful when using multiple importance sampling, in which case it is necessary to compute the probability density for samples generated using other sampling techniques. An important detail is that implementations are allowed to assume that the provided point is on their surface; callers are responsible for ensuring that this is the case.
][
  形状还必须提供一个关联的 `PDF()` 方法，该方法返回与 #link("https://pbr-book.org/4ed/Geometry_and_Transformations/Interactions.html#Interaction")[`Interaction`] 在几何形状上对应点的采样概率密度。调用此方法时，传入的交互点必须位于该形状表面上。尽管 `Sample()` 已经返回了它采样的点的概率密度，但在使用多重重要性采样时，此方法很有用，此时需要计算使用其他采样技术生成的样本的概率密度。一个重要的细节是，允许该方法的实现假设提供的点在它们的表面上；调用者有责任确保这种情况。
]

#block(sticky: true)[#raw("<<Shape Interface>>+=")] <fragment-ShapeInterface-6>
```cpp
...
#parec[
  Information about the reference point and its geometric and shading normals is provided by the `ShapeSampleContext` structure. The reference point position is specified using the #link("https://pbr-book.org/4ed/Utilities/Mathematical_Infrastructure.html#Point3fi")[`Point3fi`] class, which can represent the numerical uncertainty in a ray intersection point computed using floating-point arithmetic. Discussion of related topics is in @managing-rounding-error . For points in participating media that are not associated with a surface, the normal and shading normal are left with their default values of $(0 , 0 , 0)$.
][
  `ShapeSampleContext` 结构提供了参考点及其几何和着色法向量的信息。参考点位置使用 #link("https://pbr-book.org/4ed/Utilities/Mathematical_Infrastructure.html#Point3fi")[`Point3fi`] 类指定，该类可表示浮点运算求得的射线交点位置中的数值不确定性。相关主题的讨论在@managing-rounding-error 中。对于不与表面相关联的参与介质中的点，法向量和着色法向量保持其默认值 $(0 , 0 , 0)$。
]

#block(sticky: true)[#raw("<<ShapeSampleContext Definition>>=")] <fragment-ShapeSampleContextDefinition-0>
```cpp
...
#parec[
  `ShapeSampleContext` provides a variety of convenience constructors that allow specifying the member variable values directly or from various types of #link("https://pbr-book.org/4ed/Geometry_and_Transformations/Interactions.html#Interaction")[`Interaction`].
][
  `ShapeSampleContext` 提供了多种便捷构造函数，允许直接或从各种 #link("https://pbr-book.org/4ed/Geometry_and_Transformations/Interactions.html#Interaction")[`Interaction`] 类型指定成员变量值。
]

#block(sticky: true)[#raw("<<ShapeSampleContext Public Methods>>=")] <fragment-ShapeSampleContextPublicMethods-0>
```cpp
...
#parec[
  For code that does not need to be aware of numeric error in the intersection point, a method provides it as a regular #link("https://pbr-book.org/4ed/Geometry_and_Transformations/Points.html#Point3")[`Point3`].
][
  对于不需要了解交点数值误差的代码，提供了一种方法返回常规 #link("https://pbr-book.org/4ed/Geometry_and_Transformations/Points.html#Point3")[`Point3`]的坐标。
]

#block(sticky: true)[#raw("<<ShapeSampleContext Public Methods>>+=")] <fragment-ShapeSampleContextPublicMethods-1>
```cpp
Point3f p() const { return Point3f(pi); }
``` <ShapeSampleContext::p>


#parec[
  A second `PDF()` method comes along with this sampling approach. It returns the shape's probability of sampling a point on the light such that the incident direction $omega_i$ at the reference point is `wi`. As with the corresponding sampling method, this density should be with respect to solid angle at the reference point. As with the other #link(<Shape>)[`Shape`] `PDF()` method, this should only be called for a direction that is known to intersect the shape from the reference point; as such, implementations are not responsible for checking that case.
][
  这种采样方法还附带了第二个 `PDF()` 方法。它返回在形状上采样到使参考点处入射方向 $omega_i$ 等于 `wi` 的点时所对应的概率密度。与相应的采样方法一样，此密度应相对于参考点的立体角。与其他 #link(<Shape>)[`Shape`] 中的`PDF()` 方法一样，假设从参考点沿着给定的方向一定与形状相交；因此，实现不负责检查这种情况。
]

#block(sticky: true)[#raw("<<Shape Interface>>+=")] <fragment-ShapeInterface-8>
```cpp
...
#block(sticky: true)[#raw("<<ShapeSampleContext Public Methods>>+=")] <fragment-ShapeSampleContextPublicMethods-2>
```cpp
...
#parec[Source notes: the source describes all logical comparisons involving NaN as false, whereas the algorithm here relies specifically on the ordered comparisons `<` and `>`. The sentence introducing the transformed ray writes $bold(M) o_o$ and $bold(M) bold(d)_o$; given its definition of $bold(M)$, the input subscripts should be the rendering-space subscript $r$. The source expressions above are preserved and the subsequent derivation is unchanged.][原文校注：原文将涉及 NaN 的逻辑比较概括为均为假，而本算法实际依赖的是 `<` 与 `>` 这两种有序比较。引入变换后射线的句子写成 $bold(M) o_o$ 与 $bold(M) bold(d)_o$；按矩阵 $bold(M)$ 的定义，输入应使用表示渲染空间的下标 $r$。上文保留源式，后续推导未改。]

#include "supplements/6.1-expanded.typ"
