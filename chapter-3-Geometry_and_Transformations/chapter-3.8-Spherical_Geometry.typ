...
== #ez_caption[Spherical Geometry][球面几何]
<Spherical_Geometry>
...
#parec[
  ...
  在渲染中，单位球面上的几何也常常是有用的。三维单位方向向量可以等效地表示为单位球面上的点，方向的集合可以表示为单位球面上的区域。为一组方向确定包围范围等操作，往往可以简洁地表示为单位球面上的包围范围。因此，我们将在本节中介绍一些球面几何的有用原理以及相关的类和函数。
]

=== #ez_caption[Solid Angles][立体角]
<solid-angles>
...
#parec[
  ...
  在二维中，#emph[平面角] 是某个物体相对于给定位置所张的总角度（@fig:plane-angle）。考虑点 $p$ 周围的单位圆；如果我们将阴影物体投影到该圆上，圆的一段弧 $s$ 将被其投影覆盖。弧长 $s$（即角度 $theta$ ）是物体所形成的角度。平面角以#emph[弧度]为单位测量，整个单位圆覆盖 $2pi$ 弧度。
]
...
#parec[
  ...
  立体角将二维单位圆扩展到三维单位球（@fig:solid-angle ）。总面积 $s$ 就是物体所张的立体角。立体角以#emph[球面度]（sr）为单位测量。整个球形成的立体角为 $4pi$ sr，半球形成的立体角为 $2 pi$ sr。
]
...
#figure(
  ...
  caption: [
    #ez_caption[Solid Angle. The solid angle $s$ subtended by a 3D object is computed by projecting the object onto the unit sphere and measuring the area of its projection.][立体角。三维物体所张的立体角 $s$，等于将物体投影到单位球面后所得区域的面积。]
  ],
)<solid-angle>
...
#parec[
  ...
  通过提供一种在单位球面上测量面积的方法（从而在单位方向上），立体角也为球面函数积分所需的测度提供了基础；#emph[微分立体角] $d omega$ 对应于单位球面上的微分面积测度。
]

=== #ez_caption[Spherical Polygons][球面多边形]
<Spherical_Polygons>
#parec[
  ...
  有时我们会发现考虑从一点到多边形表面的方向集合是有用的。（例如，在计算从发光多边形到一点的照明时，这样做可能是有用的。）如果将一个常规平面多边形投影到单位球面上，它就形成了一个#emph[球面多边形]。
]
...
#figure(
  ...
  caption: [
    #ez_caption[A spherical polygon corresponds to the projection of a polygon onto the unit sphere. Its vertices correspond to the unit vectors to the original polygon's vertices and its edges are defined by the intersection of the sphere and the planes that go through the sphere's center and two vertices of the polygon.][球面多边形对应于多边形在单位球面上的投影。它的顶点对应于原始多边形顶点的单位向量，其边由球面和穿过球体中心和多边形两个顶点的平面的交集定义。]
  ],
)<spherical-polygon>
...
#parec[
  ...
  球面多边形的顶点可以通过将从球心到原始多边形相应顶点的向量归一化来找到。球面多边形的每条边由单位球与通过球心和多边形相应两个顶点的平面的交线给出。相交得到球面上的一个#emph[大圆]；两个顶点之间的球面最短路径沿这个大圆连接（@fig:spherical-polygon）。
]
...
#parec[
  ...
  每个顶点的角度由在该顶点相交的两条边对应的平面之间的角度给出（@fig:spherical-triangle ）。（两个平面之间的角度称为它们的#emph[二面角]。）我们将用与其标签对应的希腊字母标记每个顶点的角度（顶点 $bold(a)$ 的角度为 $alpha$ 等）。与平面三角形不同，球面三角形的三个角度之和不等于 $pi$ 弧度；而是它们的和为 $pi + A$，其中 $A$ 是球面三角形的面积。 给定角度 $alpha$、$beta$ 和 $gamma$，可以使用#emph[吉拉德定理]计算球面三角形的面积，该定理指出单位球面上三角形的表面积 $A$ 由下式的“角盈”给出。
]
...
#parec[
  ...
  直接实现@eqt:girards-theorem 需要多次调用计算量大的反三角函数，并且由于浮点消减，其计算可能容易出错。更高效且准确的方法是应用以下关系
]

$
  tan(frac(1, 2) A) = frac(upright(bold(a)) dot.op lr((upright(bold(b)) times upright(bold(c)))), 1 +(upright(bold(a)) dot.op upright(bold(b))) +(upright(bold(a)) dot.op upright(bold(c))) +(upright(bold(b)) dot.op upright(bold(c)))),
...
#parec[
  ...
  这一关系可以用球面三角恒等式从 @eqt:girards-theorem 推导出来。`SphericalTriangleArea()` 采用该方法，该函数接收单位球面上对应于球面三角形顶点的三个向量。
]

#block(sticky: true)[#raw("<<Spherical Geometry Inline Functions>>=") #link(<fragment-SphericalGeometryInlineFunctions-1>)[▼]] <fragment-SphericalGeometryInlineFunctions-0>
```cpp
...
#parec[
  ...
  投影到单位球面上的四边形的面积为 $alpha + beta + gamma + delta - 2 pi$，其中 $alpha$、$beta$、$gamma$ 和 $delta$ 是其内角。这个值由`SphericalQuadArea()` 计算，该函数接收单位球面上的顶点位置。其实现与#link(<SphericalTriangleArea>)[`SphericalTriangleArea()`] 非常相似，因此这里不包括。
]

#block(sticky: true)[#raw("<<Spherical Geometry Inline Functions>>+=") #link(<fragment-SphericalGeometryInlineFunctions-0>)[▲] #link(<fragment-SphericalGeometryInlineFunctions-2>)[▼]] <fragment-SphericalGeometryInlineFunctions-1>
```cpp
Float SphericalQuadArea(Vector3f a, Vector3f b, Vector3f c, Vector3f d);
```

=== #ez_caption[Spherical Parameterizations][球面参数化]
...
#parec[
  ...
  在单位球面上，一个点的三维笛卡尔坐标并不总是表示方向的最方便方式。例如，如果要将球面函数制成表格，利用球面本身是二维曲面的二维参数化会更合适。
]
...
==== #ez_caption[Spherical Coordinates][球坐标]
<spherical-coordinates-section>


#parec[
  Spherical coordinates $(theta, phi.alt)$ are a well-known parameterization of the sphere. For a general sphere of radius $r$, they are related to Cartesian coordinates by
][
  球坐标 $(theta, phi.alt)$ 是球面的一种著名参数化。对于一般半径为 $r$ 的球，它们与笛卡尔坐标的关系为
]
...
#figure(
  ...
  caption: [#ez_caption[
      A direction vector can be written in terms of spherical coordinates $(theta, phi.alt)$ if the $x$, $y$, and $z$ basis vectors are given as well. The spherical angle formulae make it easy to convert between the two representations.

    ][
      给定 $x$、$y$ 和 $z$ 基向量后，可以用球坐标 $(theta, phi.alt)$ 表示方向向量。球面角公式使得在两种表示之间的转换变得容易。
    ]
  ],
)<spherical-angles>
...
#parec[
  For convenience, we will define a `SphericalDirection()` function that converts a $theta$ and $phi.alt$ pair into a unit $(x, y, z)$ vector, applying these equations directly. Notice that the function is given the sine and cosine of $theta$, rather than $theta$ itself. This is because the sine and cosine of $theta$ are often already available to the caller. This is not normally the case for $phi.alt$, however, so $phi.alt$ is passed in as is.
][
  为了方便起见，我们将定义一个 `SphericalDirection()` 函数，该函数将 $theta$ 和 $phi.alt$ 对应的值转换为一个单位 $(x, y, z)$ 向量，直接应用这些方程。注意，该函数接收的是 $theta$ 的正弦值和余弦值，而不是 $theta$ 本身。这是因为调用者通常已经拥有 $theta$ 的正弦值和余弦值。然而， $phi.alt$ 的情况通常不是这样，所以 $phi.alt$ 会直接传入。
]

#block(sticky: true)[#raw("<<Spherical Geometry Inline Functions>>+=") #link(<fragment-SphericalGeometryInlineFunctions-1>)[▲] #link(<fragment-SphericalGeometryInlineFunctions-3>)[▼]] <fragment-SphericalGeometryInlineFunctions-2>
```cpp
...
#parec[
  The corresponding functions follow. Note that #link(<SphericalTheta>)[`SphericalTheta()`] assumes that the vector `v` has been normalized before being passed in; using #link("https://pbr-book.org/4ed/Utilities/Mathematical_Infrastructure.html#SafeACos")[`SafeACos()`] in place of `std::acos()` avoids errors if $|v.z|$ is slightly greater than 1 due to floating-point round-off error.
][
  对应的函数如下。注意#link(<SphericalTheta>)[`SphericalTheta()`] 假设向量`v`在传入之前已经被归一化；使用#link("https://pbr-book.org/4ed/Utilities/Mathematical_Infrastructure.html#SafeACos")[`SafeACos()`] 代替`std::acos()`可以避免由于浮点舍入误差导致的 $|v.z|$ 略大于1的错误。
]

#block(sticky: true)[#raw("<<Spherical Geometry Inline Functions>>+=") #link(<fragment-SphericalGeometryInlineFunctions-2>)[▲] #link(<fragment-SphericalGeometryInlineFunctions-4>)[▼]] <fragment-SphericalGeometryInlineFunctions-3>
```cpp
...
#parec[
  `SphericalPhi()` returns an angle in $[0, 2pi]$ , which sometimes requires an adjustment to the value returned by `std::atan2()`.
][
  `SphericalPhi()`返回一个在 $[0, 2pi]$ 范围内的角度，有时需要对`std::atan2()`返回的值进行调整。
]


#block(sticky: true)[#raw("<<Spherical Geometry Inline Functions>>+=") #link(<fragment-SphericalGeometryInlineFunctions-3>)[▲] #link(<fragment-SphericalGeometryInlineFunctions-5>)[▼]] <fragment-SphericalGeometryInlineFunctions-4>
```cpp
...
$
  cos theta =((0, 0, 1) dot.op omega) = omega_z .
$
...
#block(sticky: true)[#raw("<<Spherical Geometry Inline Functions>>+=") #link(<fragment-SphericalGeometryInlineFunctions-4>)[▲] #link(<fragment-SphericalGeometryInlineFunctions-6>)[▼]] <fragment-SphericalGeometryInlineFunctions-5>
```cpp
...
#block(sticky: true)[#raw("<<Spherical Geometry Inline Functions>>+=") #link(<fragment-SphericalGeometryInlineFunctions-5>)[▲] #link(<fragment-SphericalGeometryInlineFunctions-7>)[▼]] <fragment-SphericalGeometryInlineFunctions-6>
```cpp
...
#block(sticky: true)[#raw("<<Spherical Geometry Inline Functions>>+=") #link(<fragment-SphericalGeometryInlineFunctions-6>)[▲] #link(<fragment-SphericalGeometryInlineFunctions-8>)[▼]] <fragment-SphericalGeometryInlineFunctions-7>
```cpp
...
$ cos phi.alt &= x/r = x/(sin theta) \
  sin phi.alt &= y/r = y/(sin theta). $
...
#block(sticky: true)[#raw("<<Spherical Geometry Inline Functions>>+=") #link(<fragment-SphericalGeometryInlineFunctions-7>)[▲] #link(<fragment-SphericalGeometryInlineFunctions-9>)[▼]] <fragment-SphericalGeometryInlineFunctions-8>
```cpp
...
#block(sticky: true)[#raw("<<Spherical Geometry Inline Functions>>+=") #link(<fragment-SphericalGeometryInlineFunctions-8>)[▲] #link("https://pbr-book.org/4ed/Reflection_Models/BSDF_Representation.html#fragment-SphericalGeometryInlineFunctions-10")[▼]] <fragment-SphericalGeometryInlineFunctions-9>
```cpp
...
#parec[
  ...
  使用球坐标对球体进行参数化对应于球体的#emph[等距矩形]映射。由于在球体的极点处存在显著的变形，它不是表示球体上规则采样数据的特别好的参数化方法。
]

==== #ez_caption[Octahedral Encoding][八面体编码]
<octahedral-encoding>
#parec[
  While #link("https://pbr-book.org/4ed/Geometry_and_Transformations/Vectors.html#Vector3f")[`Vector3f`] is a convenient representation for computation using unit vectors, it does not use storage efficiently: not only does it use 12 bytes of memory (assuming 4-byte #link("https://pbr-book.org/4ed/Introduction/pbrt_System_Overview.html#Float")[`Float`]s), but it is capable of representing 3D direction vectors of arbitrary length. Normalized vectors are a small subset of all the possible #link("https://pbr-book.org/4ed/Geometry_and_Transformations/Vectors.html#Vector3f")[`Vector3f`]s, however, which means that the storage represented by those 12 bytes is not well allocated for them. When many normalized vectors need to be stored in memory, a more space-efficient representation can be worthwhile.
][
  虽然#link("https://pbr-book.org/4ed/Geometry_and_Transformations/Vectors.html#Vector3f")[`Vector3f`] 是使用单位向量进行计算的便捷表示，但它的存储效率不高：不仅占用 12 字节的内存（假设 4 字节的#link("https://pbr-book.org/4ed/Introduction/pbrt_System_Overview.html#Float")[`Float`]），而且能够表示任意长度的三维方向向量。然而，归一化向量只是所有可能的#link("https://pbr-book.org/4ed/Geometry_and_Transformations/Vectors.html#Vector3f")[`Vector3f`] 中的一个小子集，这意味着这 12 字节的存储对它们来说分配不当。当需要在内存中存储许多归一化向量时，使用更节省空间的表示可能是值得的。
]

#parec[
  Spherical coordinates could be used for this task. Doing so would reduce the storage required to two `Float`s, though with the disadvantage that relatively expensive trigonometric and inverse trigonometric functions would be required to convert to and from #link("https://pbr-book.org/4ed/Geometry_and_Transformations/Vectors.html#Vector3")[`Vector3`]s. Further, spherical coordinates provide more precision near the poles and less near the equator; a more equal distribution of precision across all unit vectors is preferable. (Due to the way that floating-point numbers are represented, #link("https://pbr-book.org/4ed/Geometry_and_Transformations/Vectors.html#Vector3f")[`Vector3f`] suffers from providing different precision in different parts of the unit sphere as well.)
][
  可以使用球坐标来完成此任务。这样做可以将存储需求减少到两个`Float`，但缺点是需要相对昂贵的三角函数和反三角函数来进行转换。此外，球坐标在极点附近提供了更多的精度，而在赤道附近则较少；在所有单位向量中更均匀地分布精度是更可取的。（由于浮点数的表示方式，#link("https://pbr-book.org/4ed/Geometry_and_Transformations/Vectors.html#Vector3f")[`Vector3f`]在单位球面的不同部分也提供了不同的精度。）
]

#parec[
  #link(<OctahedralVector>)[`OctahedralVector`] provides a compact representation for unit vectors with an even distribution of precision and efficient encoding and decoding routines. Our implementation uses just 4 bytes of memory for each unit vector; all the possible values of those 4 bytes correspond to a valid unit vector. Its representation is not suitable for computation, but it is easy to convert between it and #link("https://pbr-book.org/4ed/Geometry_and_Transformations/Vectors.html#Vector3f")[`Vector3f`], which makes it an appealing option for in-memory storage of normalized vectors.
][
  #link(<OctahedralVector>)[`OctahedralVector`] 提供了一种紧凑的单位向量表示，具有均匀的精度分布和高效的编码和解码例程。我们的实现为每个单位向量仅使用 4 字节的内存；这 4 字节的所有可能值都对应于一个有效的单位向量。它的表示不适合计算，但很容易在它和#link("https://pbr-book.org/4ed/Geometry_and_Transformations/Vectors.html#Vector3f")[`Vector3f`] 之间转换，这使得它成为在内存中存储归一化向量的一个有吸引力的选项。
]

#block(sticky: true)[#raw("<<OctahedralVector Definition>>=")] <fragment-OctahedralVectorDefinition-0>
```cpp
...
#parec[
  ...
  正如其名称所示，这个单位向量基于 @fig:octahedral-vec-parameterization 中所示的单位球面的八面体映射。
]
...
#figure(
  ...
  caption: [
    #ez_caption[ The
      ...
      Figure 2 in Meyer et al. (@meyer2010floating).)

    ][
      #link(<OctahedralVector>)[`OctahedralVector`] 对单位球面的参数化可以通过首先考虑(a)一个内接于球体的八面体来理解。然后通过(b)将上半部的棱锥展平到$z = 0$平面，并(c)展开底部并将其三角形投影到同一平面上定义其二维参数化。(d)结果允许一个简单的$[-1,1]^2$参数化。（图改编自Meyer 等人（@meyer2010floating）的图 2。）
    ]
  ],
...
#parec[
  ...
  将单位向量转换为这种表示的算法出乎意料地简单。第一步是将向量投影到三维八面体的各个面上；这可以通过将向量的各个分量除以向量的 L1 范数 $| upright(bold(v))_x | + | upright(bold(v))_y | + | upright(bold(v))_z |$ 来完成。对于上半球中的点（即 $ upright(bold(v))_z gt.eq 0$ ），投影到 $z = 0$ 平面只需要直接取 $x$ 和 $y$ 分量即可。
]

#block(sticky: true)[#raw("<<OctahedralVector Public Methods>>=") #link(<fragment-OctahedralVectorPublicMethods-1>)[▼]] <fragment-OctahedralVectorPublicMethods-0>
```cpp
...
#block(sticky: true)[#raw("<<Encode octahedral vector with z < 0>>=")] <fragment-Encodeoctahedralvectorwithz0-0>
```cpp
...
#block(sticky: true)[#raw("<<OctahedralVector Private Methods>>=") #link(<fragment-OctahedralVectorPrivateMethods-1>)[▼]] <fragment-OctahedralVectorPrivateMethods-0>
```cpp
static Float Sign(Float v) { return std::copysign(1.f, v); }
``` <OctahedralVector::Sign>
...
#parec[
  ...
  @fig:octahedral-vec-parameterization (d)中的二维参数化然后使用每个坐标的16 位值表示，将 $[-1,1]$ 的范围量化为$2^16$个量化级。
]

#block(sticky: true)[#raw("<<OctahedralVector Private Members>>=")] <fragment-OctahedralVectorPrivateMembers-0>
```cpp
uint16_t x, y;
```
...
#block(sticky: true)[#raw("<<OctahedralVector Private Methods>>+=") #link(<fragment-OctahedralVectorPrivateMethods-0>)[▲]] <fragment-OctahedralVectorPrivateMethods-1>
```cpp
...
#parec[
  The mapping back to a #link("https://pbr-book.org/4ed/Geometry_and_Transformations/Vectors.html#Vector3f")[`Vector3f`] follows the same steps in reverse. For directions in the upper hemisphere, the $z$ value on the octahedron face is easily found. Normalizing that vector then gives the corresponding unit vector.
][
  映射回#link("https://pbr-book.org/4ed/Geometry_and_Transformations/Vectors.html#Vector3f")[`Vector3f`]时，遵循相同步骤的逆过程。对于上半球的方向，很容易找到八面体面上的 $z$ 值。然后归一化该向量以得到相应的单位向量。
]

#block(sticky: true)[#raw("<<OctahedralVector Public Methods>>+=") #link(<fragment-OctahedralVectorPublicMethods-0>)[▲]] <fragment-OctahedralVectorPublicMethods-1>
```cpp
...
#block(sticky: true)[#raw("<<Reparameterize directions in the z < 0 portion of the octahedron>>=")] <fragment-Reparameterizedirectionsinthez0portionoftheoctahedron-0>
```cpp
if (v.z < 0) {
    Float xo = v.x;
...
==== #ez_caption[Equal-Area Mapping][等面积映射]
<equal-area-mapping>
#parec[
  The third spherical parameterization used in `pbrt` is carefully designed to preserve area: any area on the surface of the sphere maps to a proportional area in the parametric domain. This representation is a good choice for tabulating functions on the sphere, as it is continuous, has reasonably low distortion, and all values stored represent the same solid angle. It combines the octahedral mapping used in the #link(<OctahedralVector>)[`OctahedralVector`] class with a variant of the square-to-disk mapping from #link("https://pbr-book.org/4ed/Sampling_Algorithms/Sampling_Multidimensional_Functions.html#sec:unit-disk-sample")[Section A.5.1], which maps the unit square to the hemisphere in a way that preserves area.
][
  `pbrt` 中使用的第三种球面参数化经过精心设计，以保持面积：球面上的任何面积都映射到参数域中的一个比例面积。 这种表示法是对球面函数进行制表的理想选择，因为它是连续的，具有相对较低的失真，并且存储的所有值代表相同的立体角。 它结合了#link(<OctahedralVector>)[`OctahedralVector`]类中使用的八面体映射和#link("https://pbr-book.org/4ed/Sampling_Algorithms/Sampling_Multidimensional_Functions.html#sec:unit-disk-sample")[第 A.5.1 节] 中的方形到圆盘映射的变体，该映射以保持面积的方式将单位方形映射到半球。
]
...
#parec[
  ...
  得到一个保面积的映射，其中 $phi.alt in [- pi \/ 4 , pi \/ 4]$。其他三个扇区也可以找到类似的映射。
]
...
$ x &= (cos phi.alt) r sqrt(2 - r^2) \
 y &= (sin phi.alt) r sqrt(2 - r^2) \
 z &= 1 - r^2 . $ <concentric-hemi-mapping>
...
#parec[
  #block[
    ...
    + For directions in the upper hemisphere, the concentric hemisphere
      mapping, @eqt:concentric-hemi-mapping, is applied to the inner square of the
      octahedral mapping.
  ]
][
  #block[
    #set enum(numbering: "1.", start: 2)
    + 对于上半球的方向，应用@eqt:concentric-hemi-mapping 中的同心半球映射到八面体映射的内部方形。
  ]
]
...
#parec[
  ...
  这种方法的以下实现经过仔细设计以实现#emph[无分支]：无论输入值是什么，函数中都有一条单一的控制流路径。 在可能的情况下，这种特性通常有助于提高性能，尤其是在 GPU 上，尽管我们注意到此函数通常只占`pbrt` 执行时间的一小部分，因此这种特性不会影响系统的整体性能。
]

#block(sticky: true)[#raw("<<Square–Sphere Mapping Function Definitions>>=")] <fragment-Square--SphereMappingFunctionDefinitions-0>
```cpp
...
#parec[
  After transforming the original point `p` in $[0 , 1]^2$ to $(u , v) in [- 1 , 1]^2$, the implementation also computes the absolute value of these coordinates $u' = lr(|u|)$ and $v' = lr(|v|)$. Doing so remaps the three quadrants with one or two negative coordinate values to the positive quadrant, flipping each quadrant so that its upper hemisphere is mapped to $u' + v' < 1$, which corresponds to the upper hemisphere in the original positive quadrant. (Each lower hemisphere is also mapped to the $u' + v' > 1$ region, corresponding to the original negative quadrant.)
][
  将原始点 `p` 从 $[0 , 1]^2$ 转换为 $(u , v) in [- 1 , 1]^2$ 后，实现还计算了这些坐标的绝对值 $u' = lr(|u|)$ 和 $v' = lr(|v|)$。 这样做将一个或两个负坐标值的三个象限重新映射到正象限，翻转每个象限，使其上半球映射到 $u' + v' < 1$，这对应于原始正象限中的上半球。 （每个下半球也映射到 $u' + v' > 1$ 区域，对应于原始负象限。）
]


#block(sticky: true)[#raw("<<Transform p to [-1, 1]^2 and compute absolute values>>=")] <fragment-Transformmonopto-112andcomputeabsolutevalues-0>
```cpp
...
#parec[
  Most of this function's implementation operates using $(u' , v')$ in the positive quadrant. Its next step is to compute the radius $r$ for the mapping to the disk by computing the signed distance to the $u + v = 1$ diagonal that splits the upper and lower hemispheres where the lower hemisphere's signed distance is negative (@fig:square-to-disk-r).
][
  此函数的大部分实现使用正象限中的 $(u' , v')$。 其下一步是通过计算到分割上半球和下半球的 $u + v = 1$ 对角线的有符号距离来计算映射到圆盘的半径 $r$，其中下半球的有符号距离为负（@fig:square-to-disk-r）。
]
...
#figure(
  ...
  caption: [
    #ez_caption[Computation of the Radius $r$ for the Square-to-Disk
      ...
      One minus its absolute value gives a radius between 0 and 1.][方形到圆盘映射的半径 $r$ 的计算。计算到$u' + v' = 1$线的有符号距离。用 1 减去其绝对值，得到 0 到 1 之间的半径。]
  ],
) <square-to-disk-r>


#block(sticky: true)[#raw("<<Compute radius r as signed distance from diagonal>>=")] <fragment-Computeradiusmonorassigneddistancefromdiagonal-0>
```cpp
...
#block(sticky: true)[#raw("<<Compute angle phi for square to sphere mapping>>=")] <fragment-Computeanglephiforsquaretospheremapping-0>
```cpp
...
#parec[
  ...
  先前计算的有符号距离的符号指示 $(u' , v')$ 点是否在下半球；返回的 $z$ 坐标取其符号。
]

#block(sticky: true)[#raw("<<Find z coordinate for spherical direction>>=")] <fragment-Findzcoordinateforsphericaldirection-0>
```cpp
...
#block(sticky: true)[#raw("<<Compute cos phi and sin phi for original quadrant and return vector>>=")] <fragment-Computecosphiandsinphifororiginalquadrantandreturnvector-0>
```cpp
...
#parec[
  ...
  逆映射由`EqualAreaSphereToSquare()`函数执行，该函数实际上按相反顺序执行相同的操作，因此在此不包括。 此外，`WrapEqualAreaSquare()`处理略微超出 $[0 , 1]^2$ 的点`p`的边界情况（如在图像纹理查找的双线性插值期间可能发生的情况），并将它们环绕到可以传递给#link(<EqualAreaSquareToSphere>)[`EqualAreaSquareToSphere()`] 的适当有效坐标。
]



=== #ez_caption[Bounding Directions][包围方向集合]
<bounding-directions>
#parec[
  ...
  除了界定空间区域，有时界定一组方向也是有用的。例如，如果一个光源在某些方向上发出光而在其他方向上不发光，这些信息可以用来从光照计算中排除该光源，因为它们肯定不会照亮某些点。 `pbrt` 提供了 #link(<DirectionCone>)[DirectionCone] 类用于这种用途；它表示一个由中心方向和张角（即圆锥半角）参数化的圆锥（见@fig:cone-bound-directions）。
]
#figure(
...
#block(sticky: true)[#raw("<<DirectionCone Definition>>=")] <fragment-DirectionConeDefinition-0>
```cpp
class DirectionCone {
  public:
...
#block(sticky: true)[#raw("<<DirectionCone Public Methods>>=") #link(<fragment-DirectionConePublicMethods-1>)[▼]] <fragment-DirectionConePublicMethods-0>
```cpp
...
#block(sticky: true)[#raw("<<DirectionCone Public Members>>=")] <fragment-DirectionConePublicMembers-0>
```cpp
Vector3f w;
...
#block(sticky: true)[#raw("<<DirectionCone Public Methods>>+=") #link(<fragment-DirectionConePublicMethods-0>)[▲] #link(<fragment-DirectionConePublicMethods-2>)[▼]] <fragment-DirectionConePublicMethods-1>
```cpp
...
#block(sticky: true)[#raw("<<DirectionCone Public Methods>>+=") #link(<fragment-DirectionConePublicMethods-1>)[▲]] <fragment-DirectionConePublicMethods-2>
```cpp
...
#block(sticky: true)[#raw("<<DirectionCone Inline Functions>>=") #link(<fragment-DirectionConeInlineFunctions-1>)[▼]] <fragment-DirectionConeInlineFunctions-0>
```cpp
...
#parec[
  ...
  `BoundSubtendedDirections()` 返回一个 `DirectionCone`，它界定了相对于点 `p` 的给定包围盒所覆盖的方向。
]

#block(sticky: true)[#raw("<<DirectionCone Inline Functions>>+=") #link(<fragment-DirectionConeInlineFunctions-0>)[▲]] <fragment-DirectionConeInlineFunctions-1>
```cpp
...
#parec[
  ...
  首先，为边界 `b` 找到一个包围球。如果给定点 `p` 在球内，则返回所有方向的方向界限。注意，点 `p` 可能在球内但在 `b` 外部，在这种情况下返回的包围范围会过于保守。本章末尾的一道习题将进一步讨论这一问题。
]

#block(sticky: true)[#raw("<<Compute bounding sphere for b and check if p is inside>>=")] <fragment-Computeboundingsphereformonobandcheckifmonopisinside-0>
```cpp
Float radius;
Point3f pCenter;
...
#figure(
  ...
  caption: [
    #ez_caption[Finding the Angle That a Bounding Sphere Subtends from a Point $p$. Given a bounding sphere and a reference point $p$ outside of the sphere, the cosine of the angle $theta$ can be found by first computing $sin theta$ by dividing the sphere's radius $r$ by the distance $d$ between $p$ and the sphere's center and then using the identity $sin^2 theta + cos^2 theta = 1$.][求包围球相对于点 $p$ 所张的角。给定一个包围球和一个位于球外的参考点 $p$，可以通过首先计算角度 $theta$ 的 $sin theta$ 来找到余弦值。$sin theta$ 可以通过将球的半径 $r$ 除以 $p$ 与球心之间的距离 $d$ 来计算，接着使用恒等式 $sin^2 theta + cos^2 theta = 1$ 来求得余弦值。]
  ],
) <bounding-sphere-angle>

#block(sticky: true)[#raw("<<Compute and return DirectionCone for bounding sphere>>=")] <fragment-ComputeandreturnmonoDirectionConeforboundingsphere-0>
```cpp
...
#block(sticky: true)[#raw("<<DirectionCone Function Definitions>>=")] <fragment-DirectionConeFunctionDefinitions-0>
```cpp
...
#block(sticky: true)[#raw("<<Handle the cases where one or both cones are empty>>=")] <fragment-Handlethecaseswhereoneorbothconesareempty-0>
```cpp
...
#parec[
  ...
  否则，实现先计算几个有用的角度，包括每个圆锥的实际张角以及它们两个中心方向向量之间的角度。 这些值提供了足够的信息来确定一个圆锥是否完全被另一个圆锥包围（见@fig:cone-inside-another）。
]
...
#block(sticky: true)[#raw("<<Handle the cases where one cone is inside the other>>=")] <fragment-Handlethecaseswhereoneconeisinsidetheother-0>
```cpp
...
#block(sticky: true)[#raw("<<Compute the spread angle of the merged cone, theta_o>>=")] <fragment-Computethespreadangleofthemergedconetheta_o-0>
```cpp
...
#parec[
  ...
  新圆锥的方向向量_不应该_通过两个圆锥方向向量的平均值来设定；该向量和张角 $theta_o$ 不一定能包围给定的两个圆锥。使用该向量将需要一个张角 $frac(theta_d, 2) + max(2 theta_a, 2 theta_b)$，这个角度从不小于 $theta_o$。（值得在纸上画出几个例子来让自己确信这一点。）
]

#parec[
  Instead, we find the vector perpendicular to the cones' direction vectors using the cross product and rotate `a.w` by the angle around that axis that causes it to bound both cones' angles. (The `Rotate()` function used for this will be introduced shortly, in @rotation-around-an-arbitrary-axis). In the case that `LengthSquared(wr) == 0` , the vectors face in opposite directions and a bound of the entire sphere is returned.#footnote[A tighter bound is possible in this case, but it occurs very rarely and so we have not bothered with handling it more effectively.]
][
  相反，我们通过使用叉积找到垂直于两个圆锥方向向量的向量，并将 `a.w` 绕该轴旋转，使其能够包围两个圆锥的角度。（用于此操作的 `Rotate()` 函数将在@rotation-around-an-arbitrary-axis 中引入。）如果 `LengthSquared(wr) == 0` ，则向量朝相反方向，并返回整个球的包围。#footnote[在这种情况下可以有一个更紧的边界，但这种情况非常罕见，因此我们没有为此增加更精细的处理。]
]

#block(sticky: true)[#raw("<<Find the merged cone’s axis and return cone union>>=")] <fragment-Findthemergedconesaxisandreturnconeunion-0>
```cpp
...
#include "supplements/3.8-expanded.typ"

#parec[][
  #translator[固定原书图 3.12 的平面角图注写作 unit sphere，而正文讨论的是单位圆；此处忠实保留并记录来源差异。式 3.8 的 arctan(y/x) 也未写出象限处理，随后实现采用 atan2。合并圆锥段的平均轴张角公式含 max(2 theta_a, 2 theta_b)，同样来自固定原书，待统一核实，未擅自修改公式。Inside的文字说明使用严格“大于”，而代码使用 >=，包含圆锥边界；这里保留原文并记录这一差异。]
]
