...
#parec[
  ...
  渲染（Rendering）是根据 3D 场景的描述生成图像的过程。显然，这是一项广泛的任务，有很多方法可以实现。其中，基于物理的技术（Physically Based Techniques）试图模拟现实，即利用物理原理来模拟光与物质的相互作用。虽然基于物理的方法似乎是最直观的渲染方法，但实际上，它只是在过去 15 年左右的时间里才被广泛采用。
]
...
#parec[
  ...
  这本书介绍了 `pbrt` ，一个基于光线追踪算法（Ray-Tracing Algorithm）的基于物理的渲染系统（Physically Based Rendering System）。它能够渲染复杂场景的逼真图像，如 @fig:pbrt-kroken-view 所示。（除了本章中的一些特别注明的图像外，本书中的所有图像均使用`pbrt`渲染而成.）
]
...
#parec[
  ...
  大多数计算机图形学书籍通常介绍算法和理论，有时会附带一些代码片段。而本书的不同之处在于，它不仅讲解了相关理论，还完整呈现了一个可正常工作的渲染系统的实现。此外，该系统的完整源代码可在开源许可下获取，并且自 2023 年 11 月 1 日起，可在 #link("https://pbr-book.org/4ed")[pbr-book.org/4ed] 在线免费获取本书的全文。更多信息，包括示例场景和附加信息 pbrt ，可以在网站#link("https://pbr-book.org")[pbr-book.org]上找到。
]
...
#figure(
  ...
  caption: [
    ...
    ][渲染的场景 pbrt 。 Kroken 场景具有复杂的几何形状、材质和光传输。在渲染系统中很好地处理所有这些效果使得渲染像这样的逼真图像成为可能。该场景和许多其他场景可以从 pbrt 网站。_（场景由安吉洛·费雷蒂提供）_
    ]
  ],
) <pbrt-kroken-view>
